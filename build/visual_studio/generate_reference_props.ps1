...
function Get-ProjectGuid
{
	...
	if ($content -match '<ProjectGuid>\{([A-Fa-f0-9\-]+)\}</ProjectGuid>')
	{
...
"@
	...
	{
		$guid = Get-ProjectGuid -VcxprojPath $dependency.VcxprojPath

		if ($null -eq $guid)
		{
			Write-Warning "  Could not find GUID for $($dependency.VcxprojPath)"
			continue
		}

		# Calculate relative path from the output directory to the vcxproj
		$outputDirectory = Split-Path $OutputPath -Parent
		$relativePath = Get-RelativePath -FromPath $outputDirectory -ToPath $dependency.VcxprojPath
...
function Invoke-ProcessDirectory
{
	...
	foreach ($commonProps in $commonPropsFiles)
	{
		...
		$generated = New-ReferenceProps -SourcePropsPath $commonProps.FullName -OutputPath $referencePropsPath -BaseName $baseName
	...
	foreach ($buildProps in $buildPropsFiles)
	{
		...
		$generated = New-ReferenceProps -SourcePropsPath $buildProps.FullName -OutputPath $referencePropsPath -BaseName $baseName -DisplayNamePrefix "Application"
...
if ([string]::IsNullOrEmpty($RootPath))
{
	...
	foreach ($directory in $directories)
	{
		...
		{
			$result = Invoke-ProcessDirectory -DirectoryPath $directory -UpdateBuildProps:$UpdateBuildProps
			$totalGenerated += $result.Generated
			$totalUpdated += $result.Updated
		}
		else
		{
			Write-Host "Skipping (not found): $directory"
		}
	}
}
else
{
	...
	$result = Invoke-ProcessDirectory -DirectoryPath $RootPath -UpdateBuildProps:$UpdateBuildProps
	$totalGenerated = $result.Generated
	$totalUpdated = $result.Updated
}

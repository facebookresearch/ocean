...
tasks.register("copyTemporaryFiles") {
  ...
  copy {
    from(
        file(
            "$oceanDevelopmentPath/impl/application/ocean/demo/tracking/pointtracker/android/AndroidManifest.xml"))
    into(sourcePath)
  }
  ...
  val javaSourcesByPackage =
      mapOf(
          "com/meta/ocean/app/demo/tracking/pointtracker/android" to
              listOf(
                  "$oceanDevelopmentPath/impl/application/ocean/demo/tracking/pointtracker/android/PointTrackerActivity.java"),
  ...
  copy {
    from("$oceanDevelopmentPath/res/application/ocean/demo/tracking/pointtracker/android/res/")
    into("$resourcePath")
  }
...
android {
  namespace = "com.meta.ocean.app.demo.tracking.pointtracker.android"
  compileSdk = 33

  defaultConfig {
    applicationId = "com.meta.ocean.app.demo.tracking.pointtracker.android"
    minSdk = 33
    targetSdk = 33
    versionCode = 1
    versionName = "1.0"
    ...
    externalNativeBuild {
      cmake {
        ...
        targets += "application_ocean_demo_tracking_pointtracker_android_native"
      }
    }
  }
  ...
  lint {
    ...
    // The PointTracker demo doesn't actually exercise mic capture, so we don't
    // declare RECORD_AUDIO in its manifest; suppress just this one check.

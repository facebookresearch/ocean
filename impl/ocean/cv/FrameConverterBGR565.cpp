...
void FrameConverterBGR565::convertRowBGR565ToBGR24(const uint16_t* source, uint8_t* target, const size_t width, const void* unusedOptions)
{
	...
	if (width >= size_t(8))
	{
		convertRowBGR565ToBGR24NEON(source, target, (unsigned int)width);
		return;
	}
	...
	{
		const uint8_t blue5 = (*source & 0x1Fu);
		const uint8_t green6 = (*source >> 5u) & 0x3Fu;
		const uint8_t red5 = (*source >> 11u);
...
void FrameConverterBGR565::convertRowBGR565ToRGB24(const uint16_t* source, uint8_t* target, const size_t width, const void* unusedOptions)
{
	ocean_assert(source != nullptr && target != nullptr);
	ocean_assert(width >= 1u);
	...
	if (width >= size_t(8))
	{
		convertRowBGR565ToRGB24NEON(source, target, (unsigned int)width);
		return;
	}
...
void FrameConverterBGR565::convertRowBGR565ToY8FullRange(const uint16_t* source, uint8_t* target, const size_t width, const void* unusedOptions)
{
	ocean_assert(source != nullptr && target != nullptr);
	ocean_assert(width >= 1u);
	...
	if (width >= size_t(8))
	{
		convertRowBGR565ToY8FullRangeNEON(source, target, (unsigned int)width);
		return;
	}
...
void FrameConverterBGR565::convertRowBGR565ToBGR24NEON(const uint16_t* source, uint8_t* target, const unsigned int width)
{
	ocean_assert(source != nullptr && target != nullptr);
	ocean_assert(width >= 8u);

	uint8x8x3_t blue_green_red_u_8x8x3;
	...
	for (unsigned int x = 0u; x < width; x += 8u)
	{
		...
		// BBBBBGGG GGGRRRRR    BBBBBGGG GGGRRRRR    BBBBBGGG GGGRRRRR    BBBBBGGG GGGRRRRR
		// bbbBBBBB ????????    bbbBBBBB ????????    bbbBBBBB ????????    bbbBBBBB ????????
		const uint8x16_t blue_u_8x16 = vsliq_n_u8(source_u_8x16, source_u_8x16, 3);
		blue_green_red_u_8x8x3.val[0] = vmovn_u16(vreinterpretq_u16_u8(blue_u_8x16));

		// shift (with insert) green:
		// BBBBBGGG GGGRRRRR    BBBBBGGG GGGRRRRR    BBBBBGGG GGGRRRRR    BBBBBGGG GGGRRRRR
		// GGGGGGRR RRR00000    GGGGGGRR RRR00000    GGGGGGRR RRR00000    GGGGGGRR RRR00000
		// ggGGGGGG ????????    ggGGGGGG ????????    ggGGGGGG ????????    ggGGGGGG ????????
		const uint8x8_t green_intermediate_u_8x8 = vshrn_n_u16(vreinterpretq_u16_u8(source_u_8x16), 5);
		blue_green_red_u_8x8x3.val[1] = vsli_n_u8(green_intermediate_u_8x8, green_intermediate_u_8x8, 2);

		// shift (with insert) red:
		// BBBBBGGG GGGRRRRR    BBBBBGGG GGGRRRRR    BBBBBGGG GGGRRRRR    BBBBBGGG GGGRRRRR
		// RRRRR000 00000000    RRRRR000 00000000    RRRRR000 00000000    RRRRR000 00000000
		// rrrRRRRR ????????    rrrRRRRR ????????    rrrRRRRR ????????    rrrRRRRR ????????
		const uint16x8_t red_intermediate_u_16x8 = vshrq_n_u16(vreinterpretq_u16_u8(source_u_8x16), 11);
		const uint16x8_t red_u_16x8 = vsliq_n_u16(red_intermediate_u_16x8, red_intermediate_u_16x8, 3);
		blue_green_red_u_8x8x3.val[2] = vmovn_u16(red_u_16x8);

		vst3_u8(target + x * 3u, blue_green_red_u_8x8x3);
	}
}

void FrameConverterBGR565::convertRowBGR565ToRGB24NEON(const uint16_t* source, uint8_t* target, const unsigned int width)
{
	ocean_assert(source != nullptr && target != nullptr);
	ocean_assert(width >= 8u);
	...
	for (unsigned int x = 0u; x < width; x += 8u)
	{
		...
		// bbbBBBBB ????????    bbbBBBBB ????????    bbbBBBBB ????????    bbbBBBBB ????????
		const uint8x16_t blue_u_8x16 = vsliq_n_u8(source_u_8x16, source_u_8x16, 3);
		red_green_blue_u_8x8x3.val[2] = vmovn_u16(vreinterpretq_u16_u8(blue_u_8x16));
		...
		const uint16x8_t red_intermediate_u_16x8 = vshrq_n_u16(vreinterpretq_u16_u8(source_u_8x16), 11);
		const uint16x8_t red_u_16x8 = vsliq_n_u16(red_intermediate_u_16x8, red_intermediate_u_16x8, 3);
		red_green_blue_u_8x8x3.val[0] = vmovn_u16(red_u_16x8);

		vst3_u8(target + x * 3u, red_green_blue_u_8x8x3);
	}
}

void FrameConverterBGR565::convertRowBGR565ToY8FullRangeNEON(const uint16_t* source, uint8_t* target, const unsigned int width)
{
	ocean_assert(source != nullptr && target != nullptr);
	ocean_assert(width >= 8u);
	...
	for (unsigned int x = 0u; x < width; x += 8u)
	{
		...
		// bbbBBBBB ????????    bbbBBBBB ????????    bbbBBBBB ????????    bbbBBBBB ????????
		const uint8x16_t blue_u_8x16 = vsliq_n_u8(source_u_8x16, source_u_8x16, 3);
		const uint8x8_t blue_u_8x8 = vmovn_u16(vreinterpretq_u16_u8(blue_u_8x16));
		...
		const uint16x8_t red_intermediate_u_16x8 = vshrq_n_u16(vreinterpretq_u16_u8(source_u_8x16), 11);
		const uint16x8_t red_u_16x8 = vsliq_n_u16(red_intermediate_u_16x8, red_intermediate_u_16x8, 3);
		const uint8x8_t red_u_8x8 = vmovn_u16(red_u_16x8);

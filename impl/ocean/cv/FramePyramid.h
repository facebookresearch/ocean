...
class OCEAN_CV_EXPORT FramePyramid
{
	...
	public:
		 ...
		 * Beware: Due to performance issues the new frame should have the same frame type as already defined in this pyramid.<br>
		 * Further, the number of layers to be created should be the same as already defined.
		 * Beware: This template-based function can be used instead of the corresponding non-template-based function if the size of the resulting binary matters.<br>
		 * As this function is more restrictive (and does not allow to apply a blur filter) the resulting code is significantly smaller.
		 ...
		 * @see isOwner(), replace().
		 */
		bool replace8BitPerChannel(const uint8_t* frame, const unsigned int width, const unsigned int height, const unsigned int channels, const FrameType::PixelOrigin pixelOrigin, const unsigned int layers, const unsigned int framePaddingElements, Worker* worker, const Timestamp timestamp = Timestamp(false));
		 ...
		 * @param invalidWidthOrHeight Width or the height of a coarse layer not valid any more, with range [0, std::min(width, height) - 1]
		 * @return Resulting layers so that the invalid frame size will not be reached, with range [1, infinity)
		 */
		 ...
		 * @param width The width of the finest layer in pixel, with range [0, infinity)
		 * @param height The height of the finest layer in pixel, with range [0, infinity)
		 * @param invalidWidth Width of a coarse layer not valid any more, with range [0, width - 1]
		 * @param invalidHeight Height of a coarse layer not valid any more, with range [0, height -1]
		 * @return Resulting layers so that the invalid frame size will not be reached, with range [1, infinity)
		 */
		 ...
		 * @param invalidWidth Width of a coarse layer not valid any more, with range [0, width)
		 * @param invalidHeight Height of a coarse layer not valid any more, with range [0, height)
...
inline FramePyramid FramePyramid::create8BitPerChannel(const uint8_t* frame, const unsigned int width, const unsigned int height, const unsigned int channels, const FrameType::PixelOrigin pixelOrigin, const unsigned int layers, const unsigned int framePaddingElements, Worker* worker)
{
	...
	FramePyramid newPyramid;
	newPyramid.replace8BitPerChannel(frame, width, height, channels, pixelOrigin, layers, framePaddingElements, worker);

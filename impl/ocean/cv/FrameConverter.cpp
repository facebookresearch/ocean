...
bool FrameConverter::Comfort::convert(const LegacyFrame& source, const FrameType& targetType, LegacyFrame& target, const bool forceCopy, Worker* worker)
{
	...
	constexpr unsigned int nonGenericPaddingElements = 0u;

	const ConversionFlag flag = source.pixelOrigin() == targetType.pixelOrigin() ? CONVERT_NORMAL : CONVERT_FLIPPED;

	if ((source.pixelFormat() == targetType.pixelFormat() && FrameType::formatIsGeneric(source.pixelFormat()))
		 || (FrameType::formatIsPureGeneric(source.pixelFormat()) && source.pixelFormat() == FrameType::makeGenericPixelFormat(targetType.pixelFormat())))
	{
		ocean_assert(source.numberPlanes() == 1u);

		if (source.pixelOrigin() == targetType.pixelOrigin() && forceCopy == false)
		{
			target = LegacyFrame(targetType, source.constdata(), false);
			return true;
		}

		target.set(targetType);

		switch (source.bytesPerDataType())
		{
			case 1u:
			{
				switch (FrameType::formatGenericNumberChannels(source.pixelFormat()))
				{
					case 1u:
						FrameChannels::transformGeneric<uint8_t, 1u>(source.constdata<uint8_t>(), target.data<uint8_t>(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
						return true;

					case 2u:
						FrameChannels::transformGeneric<uint8_t, 2u>(source.constdata<uint8_t>(), target.data<uint8_t>(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
						return true;

					case 3u:
						FrameChannels::transformGeneric<uint8_t, 3u>(source.constdata<uint8_t>(), target.data<uint8_t>(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
						return true;

					case 4u:
						FrameChannels::transformGeneric<uint8_t, 4u>(source.constdata<uint8_t>(), target.data<uint8_t>(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
						return true;
				}
			...
			case 2u:
			{
				...
				{
					case 1u:
						FrameChannels::transformGeneric<uint16_t, 1u>(source.constdata<uint16_t>(), target.data<uint16_t>(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
						return true;

					case 2u:
						FrameChannels::transformGeneric<uint16_t, 2u>(source.constdata<uint16_t>(), target.data<uint16_t>(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
						return true;

					case 3u:
						FrameChannels::transformGeneric<uint16_t, 3u>(source.constdata<uint16_t>(), target.data<uint16_t>(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
						return true;

					case 4u:
						FrameChannels::transformGeneric<uint16_t, 4u>(source.constdata<uint16_t>(), target.data<uint16_t>(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
						return true;
				}
			...
			case 4u:
			{
				...
				{
					case 1u:
						FrameChannels::transformGeneric<uint32_t, 1u>(source.constdata<uint32_t>(), target.data<uint32_t>(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
						return true;

					case 2u:
						FrameChannels::transformGeneric<uint32_t, 2u>(source.constdata<uint32_t>(), target.data<uint32_t>(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
						return true;

					case 3u:
						FrameChannels::transformGeneric<uint32_t, 3u>(source.constdata<uint32_t>(), target.data<uint32_t>(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
						return true;

					case 4u:
						FrameChannels::transformGeneric<uint32_t, 4u>(source.constdata<uint32_t>(), target.data<uint32_t>(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
						return true;
				}
			...
			case 8u:
			{
				...
				{
					case 1u:
						FrameChannels::transformGeneric<uint64_t, 1u>(source.constdata<uint64_t>(), target.data<uint64_t>(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
						return true;

					case 2u:
						FrameChannels::transformGeneric<uint64_t, 2u>(source.constdata<uint64_t>(), target.data<uint64_t>(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
						return true;

					case 3u:
						FrameChannels::transformGeneric<uint64_t, 3u>(source.constdata<uint64_t>(), target.data<uint64_t>(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
						return true;

					case 4u:
						FrameChannels::transformGeneric<uint64_t, 4u>(source.constdata<uint64_t>(), target.data<uint64_t>(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
						return true;
				}
	...
	/**
	 * In the following we will set the target frame with the following conditions before each individual conversion:
	 *
	 * target.set(targetType, false, true);
	 *
	 * Thus, we do not require that the target frame must be the owner of the memory as long as the memory is writable
	 * This condition allows to call the convert function with an already defined target frame not owning the memory:#
	 *
	 * uint8_t* targetData = malloc(width * height * 3u);
	 *
	 * LegacyFrame targetFrame(FrameType(width, height, FrameType::FORMAT_RGB24, FrameType::ORIGIN_UPPER_LEFT), targetData, false);
	 * FrameConverter::Comfort::convert(sourceFrame, targetFrame, targetFrame, true);
	 */
	...
	switch (source.pixelFormat())
	{
		...
		{
			switch (targetType.pixelFormat())
			{
				case FrameType::FORMAT_BGR24:
				{
					target.set(targetType, false, true);
					FrameConverterABGR32::convertABGR32ToBGR24(source.constdata(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
				...
				{
					target.set(targetType, false, true);
					FrameConverterABGR32::convertABGR32ToRGBA32(source.constdata(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
		...
		case FrameType::FORMAT_ARGB32:
		{
			switch (targetType.pixelFormat())
			{
				...
				{
					target.set(targetType, false, true);
					FrameConverterARGB32::convertARGB32ToBGRA32(source.constdata(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
				...
				{
					target.set(targetType, false, true);
					FrameConverterARGB32::convertARGB32ToRGB24(source.constdata(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
				...
				{
					target.set(targetType, false, true);
					FrameConverterARGB32::convertARGB32ToRGBA32(source.constdata(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
		...
		case FrameType::FORMAT_BGR24:
		{
			switch (targetType.pixelFormat())
			{
				...
				{
					target.set(targetType, false, true);
					FrameConverterBGR24::convertBGR24ToRGB24(source.constdata(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
				...
				{
					target.set(targetType, false, true);
					FrameConverterBGR24::convertBGR24ToY8(source.constdata(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
				...
				{
					target.set(targetType, false, true);
					FrameConverterBGR24::convertBGR24ToYUV24(source.constdata(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
		...
		case FrameType::FORMAT_BGR32:
		{
			switch (targetType.pixelFormat())
			{
				...
				{
					target.set(targetType, false, true);
					FrameConverterBGR32::convertBGR32ToRGB24(source.constdata(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
				...
				{
					target.set(targetType, false, true);
					FrameConverterBGR32::convertBGR32ToRGBA32(source.constdata(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, 0xFF, worker);
					return true;
				}
		...
		case FrameType::FORMAT_BGR565:
		{
			switch (targetType.pixelFormat())
			{
				...
				{
					target.set(targetType, false, true);
					FrameConverterBGR565::convertBGR565ToBGR24(source.constdata<uint16_t>(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
				...
				{
					target.set(targetType, false, true);
					FrameConverterBGR565::convertBGR565ToY8(source.constdata<uint16_t>(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
		...
		case FrameType::FORMAT_BGRA32:
		{
			switch (targetType.pixelFormat())
			{
				...
				{
					target.set(targetType, false, true);
					FrameConverterBGRA32::convertBGRA32ToARGB32(source.constdata(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
				...
				{
					target.set(targetType, false, true);
					FrameConverterBGRA32::convertBGRA32ToBGR24(source.constdata(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
				...
				{
					target.set(targetType, false, true);
					FrameConverterBGRA32::convertBGRA32ToRGB24(source.constdata(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
				...
				{
					target.set(targetType, false, true);
					FrameConverterBGRA32::convertBGRA32ToRGBA32(source.constdata(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
				...
				{
					target.set(targetType, false, true);
					FrameConverterBGRA32::convertBGRA32ToY8(source.constdata(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
				...
				{
					target.set(targetType, false, true);
					FrameConverterBGRA32::convertBGRA32ToYA16(source.constdata(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
				...
				{
					target.set(targetType, false, true);
					FrameConverterBGRA32::convertBGRA32ToYUV24(source.constdata(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
		...
		case FrameType::FORMAT_RGB24:
		{
			switch (targetType.pixelFormat())
			{
				...
				{
					target.set(targetType, false, true);
					FrameConverterRGB24::convertRGB24ToBGR24(source.constdata(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
				...
				{
					target.set(targetType, false, true);
					FrameConverterRGB24::convertRGB24ToBGR32(source.constdata(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
				...
				{
					target.set(targetType, false, true);
					FrameConverterRGB24::convertRGB24ToRGB32(source.constdata(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
				...
				{
					target.set(targetType, false, true);
					FrameConverterRGB24::convertRGB24ToY8(source.constdata(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
				...
				{
					target.set(targetType, false, true);
					FrameConverterRGB24::convertRGB24ToYUV24(source.constdata(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
		...
		case FrameType::FORMAT_RGB32:
		{
			switch (targetType.pixelFormat())
			{
				...
				{
					target.set(targetType, false, true);
					FrameConverterRGB32::convertRGB32ToRGB24(source.constdata(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
				...
				{
					target.set(targetType, false, true);
					FrameConverterRGB32::convertRGB32ToRGBA32(source.constdata(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, 0xFF, worker);
					return true;
				}
		...
		case FrameType::FORMAT_RGB565:
		{
			switch (targetType.pixelFormat())
			{
				...
				{
					target.set(targetType, false, true);
					FrameConverterRGB565::convertRGB565ToRGB24(source.constdata<uint16_t>(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
				...
				{
					target.set(targetType, false, true);
					FrameConverterRGB565::convertRGB565ToY8(source.constdata<uint16_t>(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
		...
		case FrameType::FORMAT_RGBA32:
		{
			switch (targetType.pixelFormat())
			{
				...
				{
					target.set(targetType, false, true);
					FrameConverterRGBA32::convertRGBA32ToABGR32(source.constdata(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
				...
				{
					target.set(targetType, false, true);
					FrameConverterRGBA32::convertRGBA32ToARGB32(source.constdata(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
				...
				{
					target.set(targetType, false, true);
					FrameConverterRGBA32::convertRGBA32ToBGR24(source.constdata(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
				...
				{
					target.set(targetType, false, true);
					FrameConverterRGBA32::convertRGBA32ToBGR32(source.constdata(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
				...
				{
					target.set(targetType, false, true);
					FrameConverterRGBA32::convertRGBA32ToBGRA32(source.constdata(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
				...
				{
					target.set(targetType, false, true);
					FrameConverterRGBA32::convertRGBA32ToRGB24(source.constdata(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
				...
				{
					target.set(targetType, false, true);
					FrameConverterRGBA32::convertRGBA32ToRGB32(source.constdata(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
				...
				{
					target.set(targetType, false, true);
					FrameConverterRGBA32::convertRGBA32ToY8(source.constdata(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
				...
				{
					target.set(targetType, false, true);
					FrameConverterRGBA32::convertRGBA32ToYA16(source.constdata(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
				...
				{
					target.set(targetType, false, true);
					FrameConverterRGBA32::convertRGBA32ToYUV24(source.constdata(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
		...
		case FrameType::FORMAT_Y8:
		{
			switch (targetType.pixelFormat())
			{
				...
				{
					target.set(targetType, false, true);
					FrameConverterY8::convertY8ToBGR24(source.constdata(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
				...
				{
					target.set(targetType, false, true);
					FrameConverterY8::convertY8ToRGB24(source.constdata(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
		...
		case FrameType::FORMAT_Y32:
		{
			switch (targetType.pixelFormat())
			{
				...
				{
					target.set(targetType, false, true);
					FrameConverterY32::convertY32ToY8(source.constdata<uint32_t>(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
				...
				{
					target.set(targetType, false, true);
					FrameConverterY32::convertY32ToY16(source.constdata<uint32_t>(), target.data<uint16_t>(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
		...
		case FrameType::FORMAT_YA16:
		{
			switch (targetType.pixelFormat())
			{
				...
				{
					target.set(targetType, false, true);
					FrameConverterYA16::convertYA16ToBGRA32(source.constdata(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
				...
				{
					target.set(targetType, false, true);
					FrameConverterYA16::convertYA16ToRGBA32(source.constdata(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
				...
				{
					target.set(targetType, false, true);
					FrameConverterYA16::convertYA16ToY8(source.constdata(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
		...
		case FrameType::FORMAT_Y_U_V12:
		{
			switch (targetType.pixelFormat())
			{
				case FrameType::FORMAT_Y_U_V12:
				{
					if (source.pixelOrigin() == targetType.pixelOrigin())
					{
						if (forceCopy)
						{
							target.set(targetType, false, true);
							ocean_assert(source.size() == target.size());

							memcpy(target.data(), source.constdata(), source.size());
						}
						else
							target = LegacyFrame(targetType, source.constdata(), false);
				...
				case FrameType::FORMAT_Y8:
				{
					if (source.pixelOrigin() != targetType.pixelOrigin() || forceCopy)
					{
						target.set(targetType, false, true);
						...
						const uint8_t* const ySource = source.constdata();
						const uint8_t* const uSource = ySource + (source.width() + nonGenericPaddingElements) * source.height();
						const uint8_t* const vSource = uSource + (width_2 + nonGenericPaddingElements) * height_2;

						FrameConverterY_U_V12::convertY_U_V12ToY8(ySource, uSource, vSource, target.data(), source.width(), source.height(), flag, nonGenericPaddingElements, nonGenericPaddingElements, nonGenericPaddingElements, nonGenericPaddingElements, worker);
					}
					else
						target = LegacyFrame(targetType, source.constdata(), false);
				...
				case FrameType::FORMAT_BGR24:
				{
					...
					FrameConverterY_U_V12::convertY_U_V12LimitedRangeToBGR24FullRange(ySource, uSource, vSource, target.data(), source.width(), source.height(), flag, nonGenericPaddingElements, nonGenericPaddingElements, nonGenericPaddingElements, nonGenericPaddingElements, worker);
					return true;
				}
				...
				case FrameType::FORMAT_RGB24:
				{
					...
					FrameConverterY_U_V12::convertY_U_V12LimitedRangeToRGB24FullRange(ySource, uSource, vSource, target.data(), source.width(), source.height(), flag, nonGenericPaddingElements, nonGenericPaddingElements, nonGenericPaddingElements, nonGenericPaddingElements, worker);
					return true;
				}
				...
				case FrameType::FORMAT_YUV24:
				{
					...
					FrameConverterY_U_V12::convertY_U_V12ToYUV24(ySource, uSource, vSource, target.data(), source.width(), source.height(), flag, nonGenericPaddingElements, nonGenericPaddingElements, nonGenericPaddingElements, nonGenericPaddingElements, worker);
					return true;
				}
				...
				case FrameType::FORMAT_YVU24:
				{
					...
					FrameConverterY_U_V12::convertY_U_V12ToYVU24(ySource, uSource, vSource, target.data(), source.width(), source.height(), flag, nonGenericPaddingElements, nonGenericPaddingElements, nonGenericPaddingElements, nonGenericPaddingElements, worker);
					return true;
				}
		...
		case FrameType::FORMAT_YUV24:
		{
			switch (targetType.pixelFormat())
			{
				...
				{
					target.set(targetType, false, true);
					FrameConverterYUV24::convertYUV24ToBGR24(source.constdata(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
				...
				{
					target.set(targetType, false, true);
					FrameConverterYUV24::convertYUV24ToRGB24(source.constdata(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
				...
				{
					target.set(targetType, false, true);
					FrameConverterYUV24::convertYUV24ToY8(source.constdata(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
				...
				{
					target.set(targetType, false, true);
					FrameConverterYUV24::convertYUV24ToYVU24(source.constdata(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
				...
				case FrameType::FORMAT_Y_U_V12:
				{
					...
					uint8_t* const yTarget = target.data();
					uint8_t* const uTarget = yTarget + (source.width() + nonGenericPaddingElements) * source.height();
					uint8_t* const vTarget = uTarget + (width_2 + nonGenericPaddingElements) * height_2;

					FrameConverterYUV24::convertYUV24ToY_U_V12(source.constdata(), yTarget, uTarget, vTarget, source.width(), source.height(), flag, nonGenericPaddingElements, nonGenericPaddingElements, nonGenericPaddingElements, nonGenericPaddingElements, worker);
					return true;
				}
		...
		case FrameType::FORMAT_YUVA32:
		{
			switch (targetType.pixelFormat())
			{
				// case FrameType::FORMAT_YUVA32:  (the identity, including mirroring etc. has been handled above already)

				case FrameType::FORMAT_YUV24:
				{
					target.set(targetType, false, true);
					FrameChannels::removeLastChannel<uint8_t, 4u>(source.constdata(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
				...
				{
					target.set(targetType, false, true);
					FrameConverterYUVA32::convertYUVA32ToY8(source.constdata(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
		...
		case FrameType::FORMAT_YUVT32:
		{
			switch (targetType.pixelFormat())
			{
				// case FrameType::FORMAT_YUVT32: (the identity, including mirroring etc. has been handled above already)
		...
		case FrameType::FORMAT_YVU24:
		{
			switch (targetType.pixelFormat())
			{
				...
				{
					target.set(targetType, false, true);
					FrameConverterYVU24::convertYVU24ToBGR24(source.constdata(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
				...
				{
					target.set(targetType, false, true);
					FrameConverterYVU24::convertYVU24ToRGB24(source.constdata(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
				...
				{
					target.set(targetType, false, true);
					FrameConverterYVU24::convertYVU24ToY8(source.constdata(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
				...
				{
					target.set(targetType, false, true);
					FrameConverterYVU24::convertYVU24ToYUV24(source.constdata(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
		...
		case FrameType::FORMAT_YUYV16:
		{
			switch (targetType.pixelFormat())
			{
				...
				{
					target.set(targetType, false, true);
					FrameConverterYUYV16::convertYUYV16ToBGR24(source.constdata(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
				...
				{
					target.set(targetType, false, true);
					FrameConverterYUYV16::convertYUYV16ToRGB24(source.constdata(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
				...
				{
					target.set(targetType, false, true);
					FrameConverterYUYV16::convertYUYV16ToY8(source.constdata(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
				...
				{
					target.set(targetType, false, true);
					FrameConverterYUYV16::convertYUYV16ToYVU24(source.constdata(), target.data(), source.width(), source.height(), flag, sourcePaddingElements, targetPaddingElements, worker);
					return true;
				}
		...
		case FrameType::FORMAT_Y_V_U12: // FORMAT_Y_V_U12_LIMITED_RANGE
		{
			switch (targetType.pixelFormat())
			{
				...
				case FrameType::FORMAT_Y8:
				{
					if (source.pixelOrigin() != targetType.pixelOrigin() || forceCopy)
					{
						...
						const uint8_t* const ySource = source.constdata();
						const uint8_t* const vSource = ySource + (source.width() + nonGenericPaddingElements) * source.height();
						const uint8_t* const uSource = vSource + (width_2 + nonGenericPaddingElements) * height_2;

						FrameConverterY_V_U12::convertY_V_U12ToY8(ySource, vSource, uSource, target.data(), source.width(), source.height(), flag, nonGenericPaddingElements, nonGenericPaddingElements, nonGenericPaddingElements, nonGenericPaddingElements, worker);
					}
					else
						target = LegacyFrame(targetType, source.constdata(), false);
				...
				case FrameType::FORMAT_BGR24:
				{
					...
					FrameConverterY_V_U12::convertY_V_U12LimitedRangeToBGR24FullRange(ySource, vSource, uSource, target.data(), source.width(), source.height(), flag, nonGenericPaddingElements, nonGenericPaddingElements, nonGenericPaddingElements, nonGenericPaddingElements, worker);
					return true;
				}
				...
				case FrameType::FORMAT_RGB24:
				{
					...
					FrameConverterY_V_U12::convertY_V_U12LimitedRangeToRGB24FullRange(ySource, vSource, uSource, target.data(), source.width(), source.height(), flag, nonGenericPaddingElements, nonGenericPaddingElements, nonGenericPaddingElements, nonGenericPaddingElements, worker);
					return true;
				}
				...
				case FrameType::FORMAT_YVU24:
				{
					...
					FrameConverterY_V_U12::convertY_V_U12ToYVU24(ySource, vSource, uSource, target.data(), source.width(), source.height(), flag, nonGenericPaddingElements, nonGenericPaddingElements, nonGenericPaddingElements, nonGenericPaddingElements, worker);
					return true;
				}
				...
				case FrameType::FORMAT_YUV24:
				{
					...
					FrameConverterY_V_U12::convertY_V_U12ToYUV24(ySource, vSource, uSource, target.data(), source.width(), source.height(), flag, nonGenericPaddingElements, nonGenericPaddingElements, nonGenericPaddingElements, nonGenericPaddingElements, worker);
					return true;
				}
		...
		case FrameType::FORMAT_Y_UV12: // FORMAT_Y_UV12_LIMITED_RANGE
		{
			switch (targetType.pixelFormat())
			{
				...
				case FrameType::FORMAT_BGR24:
				{
					...
					const uint8_t* const ySource = source.constdata();
					const uint8_t* const uvSource = source.constdata() + (source.width() + nonGenericPaddingElements) * source.height();

					FrameConverterY_UV12::convertY_UV12LimitedRangeToBGR24FullRange(ySource, uvSource, target.data(), source.width(), source.height(), flag, nonGenericPaddingElements, nonGenericPaddingElements, nonGenericPaddingElements, worker);
					return true;
				}
				...
				case FrameType::FORMAT_RGB24:
				{
					...
					FrameConverterY_UV12::convertY_UV12LimitedRangeToRGB24FullRange(ySource, uvSource, target.data(), source.width(), source.height(), flag, nonGenericPaddingElements, nonGenericPaddingElements, nonGenericPaddingElements, worker);
					return true;
				}

				case FrameType::FORMAT_Y8: // **TODO** this should actually be FORMAT_Y8_LIMITED_RANGE, we keep it for now to be backward compatible
				{
					if (source.pixelOrigin() != targetType.pixelOrigin() || forceCopy)
					{
						target.set(targetType, false, true);
						...
						FrameConverterY_UV12::convertY_UV12ToY8(ySource, uvSource, target.data(), source.width(), source.height(), flag, nonGenericPaddingElements, nonGenericPaddingElements, nonGenericPaddingElements, worker);
					}
					else
					{
						target = LegacyFrame(targetType, source.constdata(), false);
				...
				case FrameType::FORMAT_YUV24:
				{
					...
					FrameConverterY_UV12::convertY_UV12ToYUV24(ySource, uvSource, target.data(), source.width(), source.height(), flag, nonGenericPaddingElements, nonGenericPaddingElements, nonGenericPaddingElements, worker);
					return true;
				}
				...
				case FrameType::FORMAT_YVU24:
				{
					...
					FrameConverterY_UV12::convertY_UV12ToYVU24(ySource, uvSource, target.data(), source.width(), source.height(), flag, nonGenericPaddingElements, nonGenericPaddingElements, nonGenericPaddingElements, worker);
					return true;
				}
		...
		case FrameType::FORMAT_Y_VU12: // FORMAT_Y_VU12_LIMITED_RANGE
		{
			switch (targetType.pixelFormat())
			{
				...
				case FrameType::FORMAT_BGR24:
				{
					...
					const uint8_t* const ySource = source.constdata();
					const uint8_t* const vuSource = source.constdata() + (source.width() + nonGenericPaddingElements) * source.height();

					FrameConverterY_VU12::convertY_VU12LimitedRangeToBGR24FullRange(ySource, vuSource, target.data(), source.width(), source.height(), flag, nonGenericPaddingElements, nonGenericPaddingElements, nonGenericPaddingElements, worker);
					return true;
				}
				...
				case FrameType::FORMAT_RGB24:
				{
					...
					FrameConverterY_VU12::convertY_VU12LimitedRangeToRGB24FullRange(ySource, vuSource, target.data(), source.width(), source.height(), flag, nonGenericPaddingElements, nonGenericPaddingElements, nonGenericPaddingElements, worker);
					return true;
				}
				...
				case FrameType::FORMAT_Y8: // **TODO** this should actually be FORMAT_Y8_LIMITED_RANGE, we keep it for now to be backward compatible
				{
					if (source.pixelOrigin() != targetType.pixelOrigin() || forceCopy)
					{
						...
						FrameConverterY_VU12::convertY_VU12ToY8(ySource, vuSource, target.data(), source.width(), source.height(), flag, nonGenericPaddingElements, nonGenericPaddingElements, nonGenericPaddingElements, worker);
					}
					else
					{
						target = LegacyFrame(targetType, source.constdata(), false);
				...
				case FrameType::FORMAT_YUV24:
				{
					...
					FrameConverterY_VU12::convertY_VU12ToYUV24(ySource, vuSource, target.data(), source.width(), source.height(), flag, nonGenericPaddingElements, nonGenericPaddingElements, nonGenericPaddingElements, worker);
					return true;
				}
				...
				case FrameType::FORMAT_YVU24:
				{
					...
					FrameConverterY_VU12::convertY_VU12ToYVU24(ySource, vuSource, target.data(), source.width(), source.height(), flag, nonGenericPaddingElements, nonGenericPaddingElements, nonGenericPaddingElements, worker);
					return true;
				}
...
bool FrameConverter::Comfort::change(LegacyFrame& frame, const FrameType& targetType, const bool forceCopy, Worker* worker)
{
	if (frame == targetType)
		return true;

	if (!forceCopy || (forceCopy && frame.isOwner()))
	{
		switch (frame.pixelFormat())
		{
			case FrameType::FORMAT_RGB24:
			{
				switch (targetType.pixelFormat())
				{
					case FrameType::FORMAT_BGR24:
					{
						if (frame.pixelOrigin() == targetType.pixelOrigin())
						{
							FrameConverterRGB24::changeRGB24ToBGR24(frame.data<uint8_t>(), frame.width(), frame.height(), worker);
							frame.setPixelFormat(FrameType::FORMAT_BGR24);
					...
					case FrameType::FORMAT_YUV24:
					{
						if (frame.pixelOrigin() == targetType.pixelOrigin())
						{
							FrameConverterRGB24::changeRGB24ToYUV24(frame.data<uint8_t>(), frame.width(), frame.height(), worker);
							frame.setPixelFormat(FrameType::FORMAT_YUV24);
	...
	if (!convert(frame, targetType, tmpFrame, forceCopy, worker))
	{
		return false;
	}

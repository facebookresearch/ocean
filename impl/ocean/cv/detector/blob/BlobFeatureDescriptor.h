...
class OCEAN_CV_DETECTOR_BLOB_EXPORT BlobFeatureDescriptor
{
	...
	public:
		 ...
		 * @param type Type of the orientation determination
		 * @param feature Feature to calculate the dominant orientation for
		 * @param forceCalculation Determines whether the calculation will be enforced even if the feature has been descripted already
		 ...
		 * @param pixelOrigin Origin of the original frame, to date UPPER_LEFT is supported only
		 * @param type Type of the orientation determination
		 * @param features Features to calculate the dominant orientation for
		 * @param forceCalculation Determines whether the calculation will be enforced even if the feature has been descripted already
		 * @param worker Optional worker object used for computation distribution, NULL if no worker is defined
		 * @see BlobFeatureDescriptor::calculateOrientation(), BlobFeatureDetector::detectFeatures().
		 */
		inline static void calculateOrientations(const uint32_t* linedIntegralImage, const unsigned int width, const unsigned int height, const FrameType::PixelOrigin pixelOrigin, const OrientationType type, BlobFeatures& features, const bool forceCalculation, Worker* worker);
		 ...
		 * @param forceCalculation Determines whether the calculation will be enforced even if the feature has been descripted already
		 * @param worker Optional worker object used for computation distribution, NULL if no worker object is available
		 ...
		 */
		inline static void calculateDescriptors(const uint32_t* linedIntegralImage, const unsigned int width, const unsigned int height, const FrameType::PixelOrigin pixelOrigin, BlobFeatures& features, const bool forceCalculation, Worker* worker = nullptr);
		 ...
		 * @param pixelOrigin Origin of the original frame, to date UPPER_LEFT is supported only
		 * @param type Type of the orientation determination
		 * @param features Features to calculate the orientation and descriptor for
		 * @param forceCalculation Determines whether the calculation will be enforced even if the feature has been descripted already
		 * @param worker Optional worker object used for computation distribution, NULL if no worker object is available
		 * @see calculateOrientation(), calculateDescriptor().
		 */
		inline static void calculateOrientationsAndDescriptors(const uint32_t* linedIntegralImage, const unsigned int width, const unsigned int height, const FrameType::PixelOrigin pixelOrigin, const OrientationType type, BlobFeatures& features, const bool forceCalculation, Worker* worker = nullptr);
		 ...
		 * @see calculateOrientation(), calculateDescriptor().
		 */
		inline static void calculateOrientationsAndDescriptors(const uint32_t* linedIntegralImage, const unsigned int width, const unsigned int height, const FrameType::PixelOrigin pixelOrigin, const OrientationType type, BlobFeatures& features, const FeatureIndices& featureIndices, const bool forceCalculation, Worker* worker = nullptr);
		 ...
		 * @param type Type of the orientation determination
		 * @param features Features to calculate the orientation and descriptor for
		 * @param number Number of features to be oriented and descripted (defined from the beginning)
		 * @param forceCalculation Determines whether the calculation will be enforced even if the feature has been descripted already
		 * @param worker Optional worker object used for computation distribution, NULL if no worker object is available
		 * @see calculateOrientation(), calculateDescriptor().
		 */
		inline static void calculateOrientationsAndDescriptors(const uint32_t* linedIntegralImage, const unsigned int width, const unsigned int height, const FrameType::PixelOrigin pixelOrigin, const OrientationType type, BlobFeatures& features, const unsigned int number, const bool forceCalculation, Worker* worker = nullptr);
		 ...
		 * @param height The height of the original frame in pixel
		 * @param pixelOrigin Origin of the original frame, to date UPPER_LEFT is supported only
		 * @param features Features to calculate the not oriented descriptor for
		 * @param forceCalculation Determines whether the calculation will be enforced even if the feature has been descripted already
		 * @param worker Optional worker object used for computation distribution, NULL if no worker object is available
		 * @see BlobFeatureDetector::detectFeatures(), BlobFeatureDetector::calculateNotOrientedDescriptor(), IntegralImage::createLinedImage().
		 */
		inline static void calculateNotOrientedDescriptors(const uint32_t* linedIntegralImage, const unsigned int width, const unsigned int height, const FrameType::PixelOrigin pixelOrigin, BlobFeatures& features, const bool forceCalculation, Worker* worker = nullptr);
		 ...
		 * @see BlobFeatureDetector::detectFeatures(), BlobFeatureDetector::calculateNotOrientedDescriptor(), IntegralImage::createLinedImage().
		 */
		inline static void calculateNotOrientedDescriptors(const uint32_t* linedIntegralImage, const unsigned int width, const unsigned int height, const FrameType::PixelOrigin pixelOrigin, BlobFeatures& features, const FeatureIndices& featureIndices, const bool forceCalculation, Worker* worker = nullptr);
		 ...
		 * @param pixelOrigin Origin of the original frame, to date UPPER_LEFT is supported only
		 * @param features Features to calculate the not oriented descriptor for
		 * @param number Number of features to be descripted (defined from the beginning)
		 * @param forceCalculation Determines whether the calculation will be enforced even if the feature has been descripted already
		 * @param worker Optional worker object used for computation distribution, NULL if no worker object is available
		 * @see BlobFeatureDetector::detectFeatures(), BlobFeatureDetector::calculateNotOrientedDescriptor(), IntegralImage::createLinedImage().
		 */
		inline static void calculateNotOrientedDescriptors(const uint32_t* linedIntegralImage, const unsigned int width, const unsigned int height, const FrameType::PixelOrigin pixelOrigin, BlobFeatures& features, const unsigned int number, const bool forceCalculation, Worker* worker = nullptr);
	...
	private:
		 ...
		 * @param numberFeatures Specifies the number of features to be handled, with startpoint as specified
		 */
		...
		/**
		 * Calculates the orientation and (oriented) descriptor for a specifed subset of given features.
		 * Beware: No range check will be done!
		 * @param linedIntegralImage Integral image of the original (grayscale) frame added by one additional column and row with zero entries
		 * @param width The width of the original frame in pixel
		 * @param height The height of the original frame in pixel
		 ...
		 * @param featureIndices Indices of all features to be oriented and descripted
		 * @param forceCalculation Determines whether the calculation will be enforced even if the feature has been descripted already
		 * @param firstFeature Specifies the index of the first feature to be handled
		 * @param numberFeatures Specifies the number of features to be handled, with startpoint as specified
		 */
		...
		/**
		 * Calculates the not oriented descriptor for a specifed subset of given features.
		 * Beware: No range check will be done!
		 * @param linedIntegralImage Integral image of the original (grayscale) frame added by one additional column and row with zero entries
		 * @param width The width of the original frame in pixel
		 * @param height The height of the original frame in pixel

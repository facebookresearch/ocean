...
MappingI1::MappingI1(const MappingI1& pixelMapping) :
	MappingI(pixelMapping)
{
	// nothing to do here
}

MappingI1::MappingI1(MappingI1&& mapping) noexcept :
	MappingI(std::move(mapping))
{
	// nothing to do here
}

MappingI1::MappingI1(const unsigned int width, const unsigned int height) :
...
void MappingI1::applyMapping(Frame& frame, const Frame& mask, const unsigned int xStart, const unsigned int xWidth, const unsigned int yStart, const unsigned int yHeight, Worker* worker) const
{
	ocean_assert(frame.isValid() && mask.isValid());
	ocean_assert(frame.width() == mask.width() && frame.height() == mask.height());
	ocean_assert(mask.pixelFormat() == FrameType::FORMAT_Y8);

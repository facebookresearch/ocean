...
#ifndef META_OCEAN_CV_SYNTHESIS_OPTIMIZER_4_NEIGHBORHOOD_HIGH_PERFORMANCE_SKIPPING_I_H
#define META_OCEAN_CV_SYNTHESIS_OPTIMIZER_4_NEIGHBORHOOD_HIGH_PERFORMANCE_SKIPPING_I_H
	...
	virtual public Optimizer1
{
	public:
		 ...
		 */
		inline Optimizer4NeighborhoodHighPerformanceSkippingI1(LayerI1& layer, RandomGenerator& randomGenerator);
...
template <unsigned int tWeightFactor, unsigned int tBorderFactor, bool tUpdateFrame>
inline Optimizer4NeighborhoodHighPerformanceSkippingI1<tWeightFactor, tBorderFactor, tUpdateFrame>::Optimizer4NeighborhoodHighPerformanceSkippingI1(LayerI1& layer, RandomGenerator& randomGenerator) :
	Optimizer(layer),
	OptimizerI(layer),
...
template <unsigned int tWeightFactor, unsigned int tBorderFactor, bool tUpdateFrame>
void Optimizer4NeighborhoodHighPerformanceSkippingI1<tWeightFactor, tBorderFactor, tUpdateFrame>::optimizeSubset(const unsigned int radii, const unsigned int maxSpatialCost, const unsigned int boundingBoxTop, const unsigned int boundingBoxHeight, const bool downIsMain, const unsigned int firstColumn, const unsigned int numberColumns, const unsigned int rowOffset, const unsigned int firstRow, const unsigned int numberRows, const unsigned int threadIndex) const
{
	ocean_assert(layerI1_.frame().numberPlanes() == 1u);
	ocean_assert(layerI1_.frame().dataType() == FrameType::DT_UNSIGNED_INTEGER_8);
...
template <unsigned int tWeightFactor, unsigned int tBorderFactor, bool tUpdateFrame>
template <unsigned int tChannels>
void Optimizer4NeighborhoodHighPerformanceSkippingI1<tWeightFactor, tBorderFactor, tUpdateFrame>::optimizeSubsetChannels(const unsigned int radii, const unsigned int maxSpatialCost, const unsigned int boundingBoxTop, const unsigned int boundingBoxHeight, const bool downIsMain, const unsigned int firstColumn, const unsigned int numberColumns, const unsigned int rowOffset, const unsigned int firstRow, const unsigned int numberRows, const unsigned int threadIndex) const
{
	...
	if (down)
	{
		...
		for (unsigned int yy = yStart; yy < yEnd; ++yy)
		{
			...
			for (unsigned int x = xStart; x < xEnd; ++x)
			{
				...
				if (*maskRow != 0xFFu && (x == 0u || y == 0u || (positionRow - 1)->east() != *positionRow || (positionRow - layerWidth)->south() != *positionRow))
				{
					...
					if (x > 0u && *(maskRow - 1) != 0xFFu)
					{
						...
						if (y > 0u && *(maskRow - layerMaskStrideElements) != 0xFF
	...
	else // up
	{
		...
		for (unsigned int yy = yEnd - 1; yy != yStart - 1; --yy)
		{
			...
			for (unsigned int x = xEnd - 1; x != xStart - 1 ; --x)
			{
				...
				ocean_assert(maskRow == layerMask.constpixel<uint8_t>(x, y));
				if (*maskRow != 0xFFu && (x == layerWidth - 1u || y == layerHeight - 1u || (positionRow + 1)->west() != *positionRow || (positionRow + layerWidth)->north() != *positionRow))
				{
					unsigned int newPositionX = positionRow->x();
					unsigned int newPositionY = positionRow->y();
...
#endif // META_OCEAN_CV_SYNTHESIS_OPTIMIZER_4_NEIGHBORHOOD_HIGH_PERFORMANCE_SKIPPING_I_H

...
#ifndef META_OCEAN_CV_SYNTHESIS_OPTIMIZER_4_NEIGHBORHOOD_HIGH_PERFORMANCE_SKIPPING_BY_COST_MASK_I_H
#define META_OCEAN_CV_SYNTHESIS_OPTIMIZER_4_NEIGHBORHOOD_HIGH_PERFORMANCE_SKIPPING_BY_COST_MASK_I_H
...
inline Optimizer4NeighborhoodHighPerformanceSkippingByCostMaskI1<tWeightFactor, tBorderFactor, tUpdateFrame>::Optimizer4NeighborhoodHighPerformanceSkippingByCostMaskI1(LayerI1& layer, RandomGenerator& randomGenerator, const LegacyFrame& skippingMask) :
	...
	Optimizer1(layer),
	layerI1_(layer),
	skippingMask_(skippingMask)
{
	static_assert(tBorderFactor != 0u, "Invalid border factor!");
}

template <unsigned int tWeightFactor, unsigned int tBorderFactor, bool tUpdateFrame>
void Optimizer4NeighborhoodHighPerformanceSkippingByCostMaskI1<tWeightFactor, tBorderFactor, tUpdateFrame>::optimizeSubset(const unsigned int radii, const unsigned int maxSpatialCost, const unsigned int boundingBoxTop, const unsigned int boundingBoxHeight, const bool downIsMain, const unsigned int firstColumn, const unsigned int numberColumns, const unsigned int rowOffset, const unsigned int firstRow, const unsigned int numberRows, const unsigned int threadIndex) const
{
...
template <unsigned int tWeightFactor, unsigned int tBorderFactor, bool tUpdateFrame>
template <unsigned int tChannels>
void Optimizer4NeighborhoodHighPerformanceSkippingByCostMaskI1<tWeightFactor, tBorderFactor, tUpdateFrame>::optimizeSubsetChannels(const unsigned int radii, const unsigned int maxSpatialCost, const unsigned int boundingBoxTop, const unsigned int boundingBoxHeight, const bool downIsMain, const unsigned int firstColumn, const unsigned int numberColumns, const unsigned int rowOffset, const unsigned int firstRow, const unsigned int numberRows, const unsigned int threadIndex) const
{
	...
	const uint8_t* const layerSkippingMaskData = skippingMask_.constdata<uint8_t>();
	...
	if (down)
	{
		...
		for (unsigned int yy = yStart; yy < yEnd; ++yy)
		{
			...
			const uint8_t* skipping = layerSkippingMaskData + y * layerWidth + xStart - 1;
	...
	else // up
	{
		...
		for (unsigned int yy = yEnd - 1; yy != yStart - 1; --yy)
		{
			...
			const uint8_t* skipping = layerSkippingMaskData + y * layerWidth + xEnd;
...
#endif // META_OCEAN_CV_SYNTHESIS_OPTIMIZER_4_NEIGHBORHOOD_HIGH_PERFORMANCE_SKIPPING_BY_COST_MASK_I_H

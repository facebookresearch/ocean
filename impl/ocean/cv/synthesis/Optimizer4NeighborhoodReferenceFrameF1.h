...
#ifndef META_OCEAN_CV_SYNTHESIS_OPTIMIZER_4_NEIGHBORHOOD_REFERENCE_FRAME_F_1_H
#define META_OCEAN_CV_SYNTHESIS_OPTIMIZER_4_NEIGHBORHOOD_REFERENCE_FRAME_F_1_H
...
/**
 * This class implements an optimizer for a mapping with float accuracy that uses a reference frame as additional optimization constraint.
...
inline Optimizer4NeighborhoodReferenceFrameF1<tWeightFactor, tBorderFactor, tUpdateFrame>::Optimizer4NeighborhoodReferenceFrameF1(LayerF1& layer, RandomGenerator& randomGenerator, const LegacyFrame& reference) :
	...
	Optimizer1(layer),
	layerF1_(layer),
	referenceFrame_(reference)
{
	// nothing to do here
}

template <unsigned int tWeightFactor, unsigned int tBorderFactor, bool tUpdateFrame>
void Optimizer4NeighborhoodReferenceFrameF1<tWeightFactor, tBorderFactor, tUpdateFrame>::optimizeSubset(const unsigned int radii, const unsigned int maxSpatialCost, const unsigned int boundingBoxTop, const unsigned int boundingBoxHeight, const bool downIsMain, const unsigned int firstColumn, const unsigned int numberColumns, const unsigned int rowOffset, const unsigned int firstRow, const unsigned int numberRows, const unsigned int threadIndex) const
{
...
template <unsigned int tWeightFactor, unsigned int tBorderFactor, bool tUpdateFrame>
template <unsigned int tChannels>
void Optimizer4NeighborhoodReferenceFrameF1<tWeightFactor, tBorderFactor, tUpdateFrame>::optimizeSubsetChannels(const unsigned int radii, const unsigned int maxSpatialCost, const unsigned int boundingBoxTop, const unsigned int boundingBoxHeight, const bool downIsMain, const unsigned int firstColumn, const unsigned int numberColumns, const unsigned int rowOffset, const unsigned int firstRow, const unsigned int numberRows, const unsigned int threadIndex) const
{
	...
	ocean_assert(referenceFrame_.width() == layerWidth);
	ocean_assert(referenceFrame_.height() == layerHeight);
	ocean_assert(referenceFrame_.pixelOrigin() == layerFrame.pixelOrigin());
	...
	if (down)
	{
		...
		for (unsigned int yy = yStart; yy < yEnd; ++yy)
		{
			...
			const uint8_t* mask = layerMaskData + int(y * layerWidth + xStart) - 1;
			Vector2* position = layerMapping() + int(y * layerWidth + xStart) - 1;
			...
			for (unsigned int x = xStart; x < xEnd; ++x)
			{
				...
				if (*mask != 0xFF)
				{
					Scalar newPositionX = position->x();
					Scalar newPositionY = position->y();
					...
					ocean_assert(*(mask - 1) == layerMaskData[y * layerWidth + x - 1]);
					if (x > 0 && *(mask - 1) != 0xFF)
					{
						...
						if (testPositionX < Scalar(layerWidth - 3u) && layerMaskData[Numeric::round32(testPositionY) * layerWidth + Numeric::round32(testPositionX)] == 0xFF)
						{
					...
					ocean_assert(*(mask - layerWidth) == layerMaskData[(y - 1) * layerWidth + x]);
					if (y > 0 && *(mask - layerWidth) != 0xFF)
					{
						...
						if (testPositionY < Scalar(layerHeight - 3u) && layerMaskData[Numeric::round32(testPositionY) * layerWidth + Numeric::round32(testPositionX)] == 0xFF)
						{
							// the structure cost is 0 due to the neighbor condition
							const unsigned int testCost = layerMapping.appearanceReferenceCost5x5<tChannels>(x, y, testPositionX, testPositionY, layerFrameData, layerMaskData, reference, layerFramePaddingElements, layerMaskPaddingElements, referencePaddingElements, tBorderFactor);
					...
					for (unsigned int n = 0; n < radii; ++n)
					{
						...
						if ((testPositionX == newPositionX && testPositionY == newPositionY) || testPositionX < Scalar(2) || testPositionX >= Scalar(layerWidth - 3u)
								...
								|| layerMaskData[Numeric::round32(testPositionY) * layerWidth + Numeric::round32(testPositionX)] != 0xFF)
							continue;
					...
					if (tUpdateFrame && foundBetter)
					{
						...
						ocean_assert(layerMaskData[Numeric::round32(newPositionY) * layerWidth + Numeric::round32(newPositionX)] == 0xFF);
	...
	else // up
	{
		...
		for (unsigned int yy = yEnd - 1; yy != yStart - 1; --yy)
		{
			...
			Vector2* position = layerMapping() + y * layerWidth + xEnd;
			...
			for (unsigned int x = xEnd - 1; x != xStart - 1 ; --x)
			{
				...
				if (*mask != 0xFF)
				{
					...
					if (x < layerWidth - 1 && *(mask + 1) != 0xFF)
					{
						...
						if (testPositionX >= Scalar(2) && layerMaskData[Numeric::round32(testPositionY) * layerWidth + Numeric::round32(testPositionX)] == 0xFF)
						{
							// the structure cost is 0 due to the neighbor condition
							const unsigned int testCost = layerMapping.appearanceReferenceCost5x5<tChannels>(x, y, testPositionX, testPositionY, layerFrameData, layerMaskData, reference, layerFramePaddingElements, layerMaskPaddingElements, referencePaddingElements, tBorderFactor);
					...
					if (y < layerHeight - 1 && *(mask + layerWidth) != 0xFF)
					{
						...
						if (testPositionY >= Scalar(2) && layerMaskData[Numeric::round32(testPositionY) * layerWidth + Numeric::round32(testPositionX)] == 0xFF)
						{
							// the structure cost is 0 due to the neighbor condition
							const unsigned int testCost = layerMapping.appearanceReferenceCost5x5<tChannels>(x, y, testPositionX, testPositionY, layerFrameData, layerMaskData, reference, layerFramePaddingElements, layerMaskPaddingElements, referencePaddingElements, tBorderFactor);
...
#endif // META_OCEAN_CV_SYNTHESIS_OPTIMIZER_4_NEIGHBORHOOD_REFERENCE_FRAME_F_1_H

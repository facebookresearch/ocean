...
bool FrameInterpolatorNearestPixel::Comfort::affine(const Frame& input, Frame& output, const SquareMatrix3& affineTransform, const uint8_t* borderColor, Worker* worker, const PixelPositionI& outputOrigin)
{
	...
	if (input.dataType() == FrameType::DT_UNSIGNED_INTEGER_8 && output.dataType() == FrameType::DT_UNSIGNED_INTEGER_8 && input.numberPlanes() == 1u && output.isValid() && output.pixelFormat() == input.pixelFormat() && output.pixelOrigin() == input.pixelOrigin())
	{
		...
		{
			case 1u:
				FrameInterpolatorNearestPixel::affine8BitPerChannel<1u>(input.constdata<uint8_t>(), input.width(), input.height(), affineTransform, borderColor, output.data<uint8_t>(), outputOrigin, output.width(), output.height(), input.paddingElements(), output.paddingElements(), worker);
				return true;

			case 2u:
				FrameInterpolatorNearestPixel::affine8BitPerChannel<2u>(input.constdata<uint8_t>(), input.width(), input.height(), affineTransform, borderColor, output.data<uint8_t>(), outputOrigin, output.width(), output.height(), input.paddingElements(), output.paddingElements(), worker);
				return true;

			case 3u:
				FrameInterpolatorNearestPixel::affine8BitPerChannel<3u>(input.constdata<uint8_t>(), input.width(), input.height(), affineTransform, borderColor, output.data<uint8_t>(), outputOrigin, output.width(), output.height(), input.paddingElements(), output.paddingElements(), worker);
				return true;

			case 4u:
				FrameInterpolatorNearestPixel::affine8BitPerChannel<4u>(input.constdata<uint8_t>(), input.width(), input.height(), affineTransform, borderColor, output.data<uint8_t>(), outputOrigin, output.width(), output.height(), input.paddingElements(), output.paddingElements(), worker);
				return true;
		}
	}
...
bool FrameInterpolatorNearestPixel::Comfort::homography(const Frame& input, Frame& output, const SquareMatrix3& homography, const void* borderColor, Worker* worker, const PixelPositionI& outputOrigin)
{
	ocean_assert(input.isValid() && output.isValid());
	...
	if (FrameType::arePixelFormatsCompatible(input.pixelFormat(), output.pixelFormat()) && input.numberPlanes() == 1u && input.pixelOrigin() == output.pixelOrigin())
	{
		if (input.dataType() == FrameType::DT_UNSIGNED_INTEGER_8 || input.dataType() == FrameType::DT_SIGNED_INTEGER_8)
		{
			...
			{
				case 1u:
					FrameInterpolatorNearestPixel::homography<uint8_t, 1u>(input.constdata<uint8_t>(), input.width(), input.height(), homography, borderColor_u8, output.data<uint8_t>(), outputOrigin, output.width(), output.height(), input.paddingElements(), output.paddingElements(), worker);
					return true;

				case 2u:
					FrameInterpolatorNearestPixel::homography<uint8_t, 2u>(input.constdata<uint8_t>(), input.width(), input.height(), homography, borderColor_u8, output.data<uint8_t>(), outputOrigin, output.width(), output.height(), input.paddingElements(), output.paddingElements(), worker);
					return true;

				case 3u:
					FrameInterpolatorNearestPixel::homography<uint8_t, 3u>(input.constdata<uint8_t>(), input.width(), input.height(), homography, borderColor_u8, output.data<uint8_t>(), outputOrigin, output.width(), output.height(), input.paddingElements(), output.paddingElements(), worker);
					return true;

				case 4u:
					FrameInterpolatorNearestPixel::homography<uint8_t, 4u>(input.constdata<uint8_t>(), input.width(), input.height(), homography, borderColor_u8, output.data<uint8_t>(), outputOrigin, output.width(), output.height(), input.paddingElements(), output.paddingElements(), worker);
					return true;
			}
		}
		else if (input.dataType() == FrameType::DT_SIGNED_FLOAT_32 || input.dataType() == FrameType::DT_UNSIGNED_INTEGER_32 || input.dataType() == FrameType::DT_SIGNED_INTEGER_32)
		{
			...
			{
				case 1u:
					FrameInterpolatorNearestPixel::homography<uint32_t, 1u>(input.constdata<uint32_t>(), input.width(), input.height(), homography, borderColor_u32, output.data<uint32_t>(), outputOrigin, output.width(), output.height(), input.paddingElements(), output.paddingElements(), worker);
					return true;

				case 2u:
					FrameInterpolatorNearestPixel::homography<uint32_t, 2u>(input.constdata<uint32_t>(), input.width(), input.height(), homography, borderColor_u32, output.data<uint32_t>(), outputOrigin, output.width(), output.height(), input.paddingElements(), output.paddingElements(), worker);
					return true;

				case 3u:
					FrameInterpolatorNearestPixel::homography<uint32_t, 3u>(input.constdata<uint32_t>(), input.width(), input.height(), homography, borderColor_u32, output.data<uint32_t>(), outputOrigin, output.width(), output.height(), input.paddingElements(), output.paddingElements(), worker);
					return true;

				case 4u:
					FrameInterpolatorNearestPixel::homography<uint32_t, 4u>(input.constdata<uint32_t>(), input.width(), input.height(), homography, borderColor_u32, output.data<uint32_t>(), outputOrigin, output.width(), output.height(), input.paddingElements(), output.paddingElements(), worker);
					return true;
			}
		}
	}
...
bool FrameInterpolatorNearestPixel::Comfort::homographyMask(const Frame& input, Frame& output, Frame& outputMask, const SquareMatrix3& homography, Worker* worker, const uint8_t maskValue, const PixelPositionI& outputOrigin)
{
	...
	if (input.dataType() == FrameType::DT_UNSIGNED_INTEGER_8 && input.numberPlanes() == 1u && output.isValid() && output.pixelFormat() == input.pixelFormat() && output.pixelOrigin() == input.pixelOrigin())
	{
		...
		{
			case 1u:
				FrameInterpolatorNearestPixel::homographyMask8BitPerChannel<1u>(input.constdata<uint8_t>(), input.width(), input.height(), input.paddingElements(), homography, output.data<uint8_t>(), output.paddingElements(), outputMask.data<uint8_t>(), outputMask.paddingElements(), outputOrigin, output.width(), output.height(), worker, maskValue);
				return true;

			case 2u:
				FrameInterpolatorNearestPixel::homographyMask8BitPerChannel<2u>(input.constdata<uint8_t>(), input.width(), input.height(), input.paddingElements(), homography, output.data<uint8_t>(), output.paddingElements(),outputMask.data<uint8_t>(), outputMask.paddingElements(), outputOrigin, output.width(), output.height(), worker, maskValue);
				return true;

			case 3u:
				FrameInterpolatorNearestPixel::homographyMask8BitPerChannel<3u>(input.constdata<uint8_t>(), input.width(), input.height(), input.paddingElements(), homography, output.data<uint8_t>(), output.paddingElements(),outputMask.data<uint8_t>(), outputMask.paddingElements(), outputOrigin, output.width(), output.height(), worker, maskValue);
				return true;

			case 4u:
				FrameInterpolatorNearestPixel::homographyMask8BitPerChannel<4u>(input.constdata<uint8_t>(), input.width(), input.height(), input.paddingElements(), homography, output.data<uint8_t>(), output.paddingElements(),outputMask.data<uint8_t>(), outputMask.paddingElements(), outputOrigin, output.width(), output.height(), worker, maskValue);
				return true;
		}
	}

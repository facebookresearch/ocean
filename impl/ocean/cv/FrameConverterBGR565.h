...
class OCEAN_CV_EXPORT FrameConverterBGR565 : public FrameConverter
{
	public:
		...
		/**
		 * Converts a BGR565 (16 bit) frame to a Y8 bit frame with full value range [0, 255].
		 * @param source The source frame buffer, must be valid
		 * @param target The target frame buffer, must be valid
		 * @param width The width of the frame in pixel, with range (0, infinity)
		 * @param height The height of the frame in pixel, with range (0, infinity)
		 ...
		 * @param worker Optional worker object to distribute the computational load
		 */
		static inline void convertBGR565ToY8FullRange(const uint16_t* source, uint8_t* target, const unsigned int width, const unsigned int height, const ConversionFlag flag, const unsigned int sourcePaddingElements, const unsigned int targetPaddingElements, Worker* worker = nullptr);
	...
	protected:
		...
		/**
		 * Converts a row of a BGR565 frame to a row of a Y8 full range frame.
		 * @param source The source row, must be valid
		 * @param target The target row, must be valid
		 * @param width The width of the row in pixel, with range [1, infinity)
		 * @param unusedOptions Unused options parameter, must be nullptr
		 */
		static void convertRowBGR565ToY8FullRange(const uint16_t* source, uint8_t* target, const size_t width, const void* unusedOptions);
		...
		/**
		 * Converts a row of a BGR565 frame to a row of a Y8 full range frame using NEON instructions.
		 * @param source The source row, must be valid
		 * @param target The target row, must be valid
		 * @param width The width of the row in pixel, with range [8, infinity)
		 */
		static void convertRowBGR565ToY8FullRangeNEON(const uint16_t* source, uint8_t* target, const unsigned int width);
...
inline void FrameConverterBGR565::convertBGR565ToY8FullRange(const uint16_t* source, uint8_t* target, const unsigned int width, const unsigned int height, const ConversionFlag flag, const unsigned int sourcePaddingElements, const unsigned int targetPaddingElements, Worker* worker)
{
	ocean_assert(source != nullptr && target != nullptr);
	ocean_assert(width >= 1u && height >= 1u);
	...
	FrameConverter::convertGenericPixelFormat<uint16_t, uint8_t>(source, target, width, height, sourceStrideElements, targetStrideElements, flag, CV::FrameConverterBGR565::convertRowBGR565ToY8FullRange, CV::FrameChannels::reverseRowPixelOrderInPlace<uint8_t, 1u>, areContinuous, nullptr, worker);
}

...
bool FrameInterpolator::affine(const Frame& input, Frame& output, const SquareMatrix3& affineTransform, const InterpolationMethod interpolationMethod, const uint8_t* borderColor, Worker* worker, const PixelPositionI& outputOrigin)
{
	switch (interpolationMethod)
	{
		case IM_NEAREST_PIXEL:
			return FrameInterpolatorNearestPixel::Comfort::affine(input, output, affineTransform, borderColor, worker, outputOrigin);

		default:
			ocean_assert(interpolationMethod == IM_BILINEAR);
			return FrameInterpolatorBilinear::Comfort::affine(input, output, affineTransform, borderColor, worker, outputOrigin);
	}
}

bool FrameInterpolator::homography(const Frame& input, Frame& output, const SquareMatrix3& homography, const InterpolationMethod interpolationMethod, const uint8_t* borderColor, Worker* worker, const PixelPositionI& outputOrigin)
{
	switch (interpolationMethod)
	{
		case IM_NEAREST_PIXEL:
			return FrameInterpolatorNearestPixel::Comfort::homography(input, output, homography, borderColor, worker, outputOrigin);

		default:
			ocean_assert(interpolationMethod == IM_BILINEAR);
			return FrameInterpolatorBilinear::Comfort::homography(input, output, homography, borderColor, worker, outputOrigin);
	}
}

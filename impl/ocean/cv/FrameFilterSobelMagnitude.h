...
#ifndef META_OCEAN_CV_FRAME_FILTER_SOBEL_MAGNITUDE_H
#define META_OCEAN_CV_FRAME_FILTER_SOBEL_MAGNITUDE_H
...
/**
 * This class implements a Sobel filter based on (or respecting) the magnitude of the response.
 * The 3x3 Sobel box filter is defined by:
 * <pre>
 * | -1  0  1 |
 * | -2  0  2 |
 * | -1  0  1 |
 * </pre>
 * @see FrameFilterSobel, FrameFilterScharrMagnitude.
 * @ingroup cv
 */
class OCEAN_CV_EXPORT FrameFilterSobelMagnitude
{
	public:
		...
		class OCEAN_CV_EXPORT Comfort
		{
			public:
				...
				/**
				 * Horizontal and vertical Sobel filter returning the response for the channel with largest magnitude.
				...
				/**
				 * Diagonal (45 and 135 degree) Sobel filter returning the response for the channel with largest magnitude.
				 * The target frame must be valid, must have data type 'DT_SIGNED_INTEGER_8` or `DT_SIGNED_INTEGER_16`, and 2 channels, one for each filter direction.
				 * @param source The source frame to filter, must be valid
				 * @param target The target frame receiving the filtered frame, will be adjusted if the frame type or pixel format is not compatible or read-only, can be invalid
				 * @param worker Optional worker object to distribute the computational load
				...
				/**
				 * Horizontal, vertical, and diagonal (45 and 135 degree) Sobel filter returning the response for the channel with largest magnitude.
				...
				/**
				 * Horizontal and vertical Sobel filter returning the highest absolute response for any direction.
				 * The target frame must be valid, must have data type 'DT_SIGNED_INTEGER_8` or `DT_SIGNED_INTEGER_16`, and 1 channel.
				 * @param source The source frame to filter, must be valid
				 * @param target The target frame receiving the filtered frame, will be adjusted if the frame type or pixel format is not compatible or read-only, can be invalid
				 * @param worker Optional worker object to distribute the computational load
				 * @return True, if succeeded
				 */
				static bool filterHorizontalVerticalTo1Response(const Frame& source, Frame& target, Worker* worker = nullptr);
		};
	...
	protected:
		...
		class PixelResponse
		{
			public:
				 ...
				 * @return The horizontal Sobel response, normalized by 32
				 ...
				 * @param source1 The center left location of the 3x3 box filter, must be valid
				 * @param source2 the bottom left location of the 3x3 box filter, must be valid
				 * @return The vertical Sobel response, normalized by 32
				 * @tparam TTarget The data type oft he target response frame, must be `int8_t` or `int16_t`
				 */
				template <typename TTarget>
				 ...
				 * @param source1 The center left location of the 3x3 box filter, must be valid
				 * @param source2 the bottom left location of the 3x3 box filter, must be valid
				 * @param responses The maximal horizontal and vertical Sobel response across all channels based on the magnitude, normalized by 32
				 * @tparam TTarget The data type oft he target response frame, must be `int8_t` or `int16_t`
				 * @tparam TResponseA
				 * @tparam TResponseB
				 */
	...
	public:
		...
		/**
		 * Horizontal and vertical Sobel filter for a multi-channel frame, the channel response with largest magnitude is stored as filter response.
		...
		/**
		 * Diagonal (45 and 135 degree) Sobel filter for a multi-channel frame, the channel response with largest magnitude is stored as filter response.
		 * If the target response data type is selected to be `int8_t`, each filter response  is normalized by 1/8 to fit into the value range [-128, 127].<br>
		 * If the target response data type is selected to be `int16_t` no normalization will be applied.
		 * The border pixels are set to zero.
		 * @param source The source frame to filter, must be valid
		...
		/**
		 * Horizontal, vertical, and diagonal (45 and 135 degree) Sobel filter for a multi-channel frame, the channel response with largest magnitude is stored as filter response.
		 * If the target response data type is selected to be `int8_t`, each filter response  is normalized by 1/8 to fit into the value range [-128, 127].<br>
		 * If the target response data type is selected to be `int16_t` no normalization will be applied.
		 * The border pixels are set to zero.
		 * @param source The source frame to filter, must be valid
		...
		/**
		 * Horizontal and vertical Sobel filter for a multi-channel frame, the maximal absolute response either horizontal or vertical is stored in a 1-channel frame.
		 * If the target response data type is selected to be `int8_t`, each filter response  is normalized by 1/4 to fit into the value range [0, 255].<br>
		 * If the target response data type is selected to be `int16_t` no normalization will be applied.
		 * The border pixels are set to zero.
		 * @param source The source frame to filter, must be valid
		 * @param target The target (response) frame receiving the filter responses, must be valid
		 ...
		 */
		template <typename TTarget, unsigned int tChannels>
		static inline void filterHorizontalVerticalTo1Response8Bit(const uint8_t* source, TTarget* target, const unsigned int width, const unsigned int height, const unsigned int sourcePaddingElements, const unsigned int targetPaddingElements, Worker* worker = nullptr);

	private:

		/**
		 * Applies the horizontal and vertical Sobel filter for one row of a source frame, the channel response with largest magnitude is stored as filter response.
		...
		/**
		 * Applies the diagonal (45 and 135 degree) Sobel filter for one row of a source frame, the channel response with largest magnitude is stored as filter response.
		 * @param sourceRow The row of the source frame, must be valid
		 * @param targetRow The row of the target response frame, must be valid
		 * @param width The width of the source and target frame in pixel, with range [3, infinity)
		 * @param height The height of the source and target frame in pixel, with range [3, infinity)
		...
		/**
		 * Applies the horizontal, vertical, and diagonal (45 and 135 degree) Sobel filter for one row of a source frame, the channel response with largest magnitude is stored as filter response.
		 * @param sourceRow The row of the source frame, must be valid
		 * @param targetRow The row of the target response frame, must be valid
		 * @param width The width of the source and target frame in pixel, with range [3, infinity)
		 * @param height The height of the source and target frame in pixel, with range [3, infinity)
		...
		/**
		 * Applies the horizontal and vertical Sobel filter for one row of a source frame, the maximal absolute response either horizontal or vertical is stored as a 1-value response.
		 * @param sourceRow The row of the source frame, must be valid
		 * @param targetRow The row of the target response frame, must be valid
		 * @param width The width of the source and target frame in pixel, with range [3, infinity)
		 * @param height The height of the source and target frame in pixel, with range [3, infinity)
		 ...
		 */
		template <typename TSource, typename TTarget, unsigned int tChannels>
		static void filterHorizontalVerticalTo1ResponseRow(const TSource* sourceRow, TTarget* targetRow, const unsigned int width, const unsigned int height, unsigned int rowIndex, const unsigned int sourceStrideElements, const unsigned int targetStrideElements);
};

template <unsigned int tChannels>
template <typename TTarget>
OCEAN_FORCE_INLINE TTarget FrameFilterSobelMagnitude::PixelResponse<tChannels>::response0(const uint8_t* const source0, const uint8_t* const source1, const uint8_t* const source2)
{
	...
	{
		return TTarget((-int16_t(source0[0]) + int16_t(source0[tChannels * 2u]) - int16_t(source2[0]) + int16_t(source2[tChannels * 2u]) + (-int16_t(source1[0]) + int16_t(source1[tChannels * 2u])) * int16_t(2)) / int16_t(8));
	}
	else
	{
		ocean_assert((std::is_same<TTarget, int16_t>::value));

		return TTarget(-int16_t(source0[0]) + int16_t(source0[tChannels * 2u]) - int16_t(source2[0]) + int16_t(source2[tChannels * 2u]) + (-int16_t(source1[0]) + int16_t(source1[tChannels * 2u])) * int16_t(2));
	}
}

template <unsigned int tChannels>
template <typename TTarget>
OCEAN_FORCE_INLINE TTarget FrameFilterSobelMagnitude::PixelResponse<tChannels>::response90(const uint8_t* const source0, const uint8_t* const /*source1*/, const uint8_t* const source2)
{
	static_assert(std::is_same<TTarget, int8_t>::value || std::is_same<TTarget, int16_t>::value, "Invalid target data type!");
	static_assert(tChannels >= 1u, "Invalid channels!");
	...
	if constexpr (std::is_same<TTarget, int8_t>::value)
	{
		return TTarget((-int16_t(source0[0]) - int16_t(source0[tChannels * 2u]) + int16_t(source2[0]) + int16_t(source2[tChannels * 2u]) + (-int16_t(source0[tChannels]) + int16_t(source2[tChannels])) * int16_t(2)) / int16_t(8));
	}
	else
	{
		ocean_assert((std::is_same<TTarget, int16_t>::value));

		return TTarget(-int16_t(source0[0]) - int16_t(source0[tChannels * 2u]) + int16_t(source2[0]) + int16_t(source2[tChannels * 2u]) + (-int16_t(source0[tChannels]) + int16_t(source2[tChannels])) * int16_t(2));
	}
}

template <unsigned int tChannels>
template <typename TTarget>
OCEAN_FORCE_INLINE TTarget FrameFilterSobelMagnitude::PixelResponse<tChannels>::response45(const uint8_t* const source0, const uint8_t* const source1, const uint8_t* const source2)
{
	static_assert(std::is_same<TTarget, int8_t>::value || std::is_same<TTarget, int16_t>::value, "Invalid target data type!");
	static_assert(tChannels >= 1u, "Invalid channels!");
	...
	if constexpr (std::is_same<TTarget, int8_t>::value)
	{
		return TTarget(((int16_t(source2[tChannels * 2u]) - int16_t(source0[0])) * int16_t(2) + int16_t(source2[tChannels]) + int16_t(source1[tChannels * 2u]) - int16_t(source1[0]) - int16_t(source0[tChannels])) / int16_t(8));
	}
	else
	{
		ocean_assert((std::is_same<TTarget, int16_t>::value));

		return TTarget((int16_t(source2[tChannels * 2u]) - int16_t(source0[0])) * int16_t(2) + int16_t(source2[tChannels]) + int16_t(source1[tChannels * 2u]) - int16_t(source1[0]) - int16_t(source0[tChannels]));
	}
}

template <unsigned int tChannels>
template <typename TTarget>
OCEAN_FORCE_INLINE TTarget FrameFilterSobelMagnitude::PixelResponse<tChannels>::response135(const uint8_t* const source0, const uint8_t* const source1, const uint8_t* const source2)
{
	static_assert(std::is_same<TTarget, int8_t>::value || std::is_same<TTarget, int16_t>::value, "Invalid target data type!");
	static_assert(tChannels >= 1u, "Invalid channels!");
	...
	if constexpr (std::is_same<TTarget, int8_t>::value)
	{
		return TTarget(((int16_t(source2[0]) - int16_t(source0[tChannels * 2u])) * int16_t(2) + int16_t(source1[0]) + int16_t(source2[tChannels]) - int16_t(source0[tChannels]) - int16_t(source1[tChannels * 2u])) / int16_t(8));
	}
	else
	{
		ocean_assert((std::is_same<TTarget, int16_t>::value));

		return TTarget((int16_t(source2[0]) - int16_t(source0[tChannels * 2u])) * int16_t(2) + int16_t(source1[0]) + int16_t(source2[tChannels]) - int16_t(source0[tChannels]) - int16_t(source1[tChannels * 2u]));
	}
}

template <>
template <typename TTarget, TTarget(*tResponseA)(const uint8_t* const, const uint8_t*, const uint8_t* const), TTarget(*tResponseB)(const uint8_t* const, const uint8_t*, const uint8_t* const)>
OCEAN_FORCE_INLINE void FrameFilterSobelMagnitude::PixelResponse<2u>::maxMagnitudeResponse(const uint8_t* const source0, const uint8_t* const source1, const uint8_t* const source2, TTarget* const response)
{
...
template <>
template <typename TTarget, TTarget(*tResponseA)(const uint8_t* const, const uint8_t*, const uint8_t* const), TTarget(*tResponseB)(const uint8_t* const, const uint8_t*, const uint8_t* const)>
OCEAN_FORCE_INLINE void FrameFilterSobelMagnitude::PixelResponse<3u>::maxMagnitudeResponse(const uint8_t* const source0, const uint8_t* const source1, const uint8_t* const source2, TTarget* const response)
{
	ocean_assert(source0 != nullptr && source1 != nullptr && source2 != nullptr && response != nullptr);
...
template <>
template <typename TTarget, TTarget(*tResponseA)(const uint8_t* const, const uint8_t*, const uint8_t* const), TTarget(*tResponseB)(const uint8_t* const, const uint8_t*, const uint8_t* const)>
OCEAN_FORCE_INLINE void FrameFilterSobelMagnitude::PixelResponse<4u>::maxMagnitudeResponse(const uint8_t* const source0, const uint8_t* const source1, const uint8_t* const source2, TTarget* const response)
{
	ocean_assert(source0 != nullptr && source1 != nullptr && source2 != nullptr && response != nullptr);
...
template <typename TTarget, unsigned int tChannels>
inline void FrameFilterSobelMagnitude::filterHorizontalVerticalAs1Channel8Bit(const uint8_t* source, TTarget* target, const unsigned int width, const unsigned int height, const unsigned int sourcePaddingElements, const unsigned int targetPaddingElements, Worker* worker)
{
	static_assert(std::is_same<TTarget, int8_t>::value || std::is_same<TTarget, int16_t>::value, "Invalid target data type!");
	...
	{
		FrameFilterSobel::filterHorizontalVertical8BitPerChannel<TTarget, 1u>(source, target, width, height, sourcePaddingElements, targetPaddingElements, worker);
	}
	else
	{
...
template <typename TTarget, unsigned int tChannels>
inline void FrameFilterSobelMagnitude::filterDiagonalAs1Channel8Bit(const uint8_t* source, TTarget* target, const unsigned int width, const unsigned int height, const unsigned int sourcePaddingElements, const unsigned int targetPaddingElements, Worker* worker)
{
	static_assert(std::is_same<TTarget, int8_t>::value || std::is_same<TTarget, int16_t>::value, "Invalid target data type!");
	static_assert(tChannels >= 1u, "Invalid channel number!");
	...
	if constexpr (tChannels == 1u)
	{
		FrameFilterSobel::filterDiagonal8BitPerChannel<TTarget, 1u>(source, target, width, height, sourcePaddingElements, targetPaddingElements, worker);
	}
	else
	{
		constexpr unsigned int tTargetChannels = 2u;
...
template <typename TTarget, unsigned int tChannels>
inline void FrameFilterSobelMagnitude::filterAs1Channel8Bit(const uint8_t* source, TTarget* target, const unsigned int width, const unsigned int height, const unsigned int sourcePaddingElements, const unsigned int targetPaddingElements, Worker* worker)
{
	static_assert(std::is_same<TTarget, int8_t>::value || std::is_same<TTarget, int16_t>::value, "Invalid target data type!");
	static_assert(tChannels >= 1u, "Invalid channel number!");
	...
	if constexpr (tChannels == 1u)
	{
		FrameFilterSobel::filter8BitPerChannel<TTarget, 1u>(source, target, width, height, sourcePaddingElements, targetPaddingElements, worker);
	}
	else
	{
...
template <typename TTarget, unsigned int tChannels>
inline void FrameFilterSobelMagnitude::filterHorizontalVerticalTo1Response8Bit(const uint8_t* source, TTarget* target, const unsigned int width, const unsigned int height, const unsigned int sourcePaddingElements, const unsigned int targetPaddingElements, Worker* worker)
{
	...
	FrameChannels::applyRowOperator<uint8_t, TTarget, tChannels, tTargetChannels>(source, target, width, height, sourcePaddingElements, targetPaddingElements, &filterHorizontalVerticalTo1ResponseRow<uint8_t, TTarget, tChannels>, worker);
}

template <typename TSource, typename TTarget, unsigned int tSourceChannels>
void FrameFilterSobelMagnitude::filterHorizontalVerticalAs1ChannelRow(const TSource* sourceRow, TTarget* targetRow, const unsigned int width, const unsigned int height, unsigned int rowIndex, const unsigned int sourceStrideElements, const unsigned int /*targetStrideElements*/)
{
...
template <typename TSource, typename TTarget, unsigned int tSourceChannels>
void FrameFilterSobelMagnitude::filterDiagonalAs1ChannelRow(const TSource* sourceRow, TTarget* targetRow, const unsigned int width, const unsigned int height, unsigned int rowIndex, const unsigned int sourceStrideElements, const unsigned int /*targetStrideElements*/)
{
	static_assert(std::is_same<TSource, uint8_t>::value, "Invalid source data type!");
	static_assert(std::is_same<TTarget, int8_t>::value || std::is_same<TTarget, int16_t>::value, "Invalid target data type!");
...
template <typename TSource, typename TTarget, unsigned int tSourceChannels>
void FrameFilterSobelMagnitude::filterAs1ChannelRow(const TSource* sourceRow, TTarget* targetRow, const unsigned int width, const unsigned int height, unsigned int rowIndex, const unsigned int sourceStrideElements, const unsigned int /*targetStrideElements*/)
{
	static_assert(std::is_same<TSource, uint8_t>::value, "Invalid source data type!");
	static_assert(std::is_same<TTarget, int8_t>::value || std::is_same<TTarget, int16_t>::value, "Invalid target data type!");
...
template <typename TSource, typename TTarget, unsigned int tSourceChannels>
void FrameFilterSobelMagnitude::filterHorizontalVerticalTo1ResponseRow(const TSource* sourceRow, TTarget* targetRow, const unsigned int width, const unsigned int height, unsigned int rowIndex, const unsigned int sourceStrideElements, const unsigned int /*targetStrideElements*/)
{
	static_assert(std::is_same<TSource, uint8_t>::value, "Invalid source data type!");
	static_assert(std::is_same<TTarget, uint8_t>::value || std::is_same<TTarget, uint16_t>::value, "Invalid target data type!");
	...
	for (unsigned int x = 1u; x < width - 1u; ++x)
	{
		int16_t maxResponse = int16_t(0);

		for (unsigned int n = 0u; n < tSourceChannels; ++n)
		{
			const int16_t response = int16_t(std::max(std::abs(PixelResponse<tSourceChannels>::template response0<int16_t>(source0 + n, source1 + n, source2 + n)), abs(PixelResponse<tSourceChannels>::template response90<int16_t>(source0 + n, source1 + n, source2 + n))));
		...
		{
			*targetRow++ = uint8_t((uint16_t(maxResponse) + 2u) / 4u);
		}
		else
		{
			*targetRow++ = uint16_t(maxResponse);
		}
...
#endif // META_OCEAN_CV_FRAME_FILTER_SOBEL_MAGNITUDE_H

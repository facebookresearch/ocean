...
inline void LineT2<T>::decomposeAngleDistance(T& angle, T& distance) const
{
	...
	ocean_assert_accuracy((std::is_same<T, float>::value) || NumericT<T>::isEqual(Numeric::abs(distance), this->distance(VectorT2<T>(0, 0))));
...
inline VectorT3<T> LineT2<T>::decomposeNormalDistance(const bool forcePositiveDistance) const
{
	...
	// ensure that the absolute distance is correct
	ocean_assert((std::is_same<T, float>::value) || NumericT<T>::isWeakEqual(Numeric::abs(distance), this->distance(VectorT2<T>(0, 0))));

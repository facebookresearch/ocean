...
class TriangleT2 : public TriangleT<T>
{
	public:
		 ...
		 * @param triangles Triangles that are tested
		 * @param point Point to be checked
		 * @return True, if so
		 */
		 ...
		 * @param cartesian Cartesian coordinate to convert to a barycentric coordinate
	...
	private:
		...
		VectorT2<T> trianglePoints[3];

		/// Convert factor for barycentric coordinates.
		T triangleBarycentricFactor;
};

template <typename T>
inline TriangleT2<T>::TriangleT2() :
	triangleBarycentricFactor(T(0))
{
	trianglePoints[0] = VectorT2<T>(T(0), T(0));
	trianglePoints[1] = VectorT2<T>(T(0), T(0));
	trianglePoints[2] = VectorT2<T>(T(0), T(0));
}
	...
	triangleBarycentricFactor(T(0))
{
	...
	const T factor = (trianglePoints[1].y() - trianglePoints[2].y()) * (trianglePoints[0].x() - trianglePoints[2].x())
								+ (trianglePoints[2].x() - trianglePoints[1].x()) * (trianglePoints[0].y() - trianglePoints[2].y());

	if (NumericT<T>::isNotEqualEps(factor))
	{
		triangleBarycentricFactor = T(1) / factor;
	}
}
...
{
	return trianglePoints[0].sqrDistance(trianglePoints[1]);
}

template <typename T>
inline T TriangleT2<T>::sqrDistance02() const
{
	return trianglePoints[0].sqrDistance(trianglePoints[2]);
}

template <typename T>
inline T TriangleT2<T>::sqrDistance12() const
{
	return trianglePoints[1].sqrDistance(trianglePoints[2]);
}
...
{
	const T a2 = trianglePoints[0].sqrDistance(trianglePoints[1]);
	const T b2 = trianglePoints[0].sqrDistance(trianglePoints[2]);
	const T c2 = trianglePoints[1].sqrDistance(trianglePoints[2]);
...
void TriangleT2<T>::cosines(T& cosine0, T& cosine1, T& cosine2) const
{
	...
	const T sqrDistance01(trianglePoints[1].sqrDistance(trianglePoints[0]));
	const T sqrDistance02(trianglePoints[2].sqrDistance(trianglePoints[0]));
	const T sqrDistance12(trianglePoints[2].sqrDistance(trianglePoints[1]));
...
VectorT3<T> TriangleT2<T>::barycentricCircumcenter() const
{
	...
	const T a2 = trianglePoints[1].sqrDistance(trianglePoints[2]);
	const T b2 = trianglePoints[0].sqrDistance(trianglePoints[2]);
	const T c2 = trianglePoints[0].sqrDistance(trianglePoints[1]);
...
VectorT3<T> TriangleT2<T>::barycentricIncenter() const
{
	...
	const T a = trianglePoints[1].distance(trianglePoints[2]);
	const T b = trianglePoints[0].distance(trianglePoints[2]);
	const T c = trianglePoints[0].distance(trianglePoints[1]);
...
TriangleT2<T> TriangleT2<T>::padded(const T padWidth) const
{
	...
	const VectorT3<T> hPoint0(trianglePoints[0], T(1.0));
	const VectorT3<T> hPoint1(trianglePoints[1], T(1.0));
	const VectorT3<T> hPoint2(trianglePoints[2], T(1.0));
...
{
	const T sqrLength01(trianglePoints[0].sqrDistance(trianglePoints[1]));
	const T sqrLength02(trianglePoints[0].sqrDistance(trianglePoints[2]));
	const T sqrLength12(trianglePoints[1].sqrDistance(trianglePoints[2]));
...
{
	return Numeric::sqrt(minSqrLength());
}
...
inline bool TriangleT2<T>::isCounterClockwise(const bool yAxisDownwards) const
{
	...
	return analyzePoints(trianglePoints[0], trianglePoints[1], trianglePoints[2], yAxisDownwards) < T(0);
}
...
inline VectorT2<T> TriangleT2<T>::barycentric2cartesian(const VectorT3<T>& barycentric) const
{
	...
	return VectorT2<T>((trianglePoints[0].x() * barycentric[0] + trianglePoints[1].x() * barycentric[1] + trianglePoints[2].x() * barycentric[2]),
...
inline VectorT3<T> TriangleT2<T>::cartesian2barycentric(const VectorT2<T>& cartesian) const
{
	...
	const T barycentric0 = ((trianglePoints[1].y() - trianglePoints[2].y()) * (cartesian.x() - trianglePoints[2].x())
							+ (trianglePoints[2].x() - trianglePoints[1].x()) * (cartesian.y() -trianglePoints[2].y())) * triangleBarycentricFactor;

	const T barycentric1 = ((trianglePoints[2].y() - trianglePoints[0].y()) * (cartesian.x() - trianglePoints[2].x())
							+ (trianglePoints[0].x() - trianglePoints[2].x()) * (cartesian.y() - trianglePoints[2].y())) * triangleBarycentricFactor;
...
inline bool TriangleT2<T>::isValid() const
{
	...
	{
		return NumericT<T>::isNotEqualEps(triangleBarycentricFactor);
	}
	else
	{
...
{
	return TriangleT2<T>(trianglePoints[0] + offset, trianglePoints[1] + offset, trianglePoints[2] + offset);
}

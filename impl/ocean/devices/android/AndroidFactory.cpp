		...
		switch (type)
		{
			...
			case AndroidEventDevice::AST_GRAVITY:
				...
				if (sensorGravity_ == nullptr)
				{
					...
					registerDevice(AndroidGravityTracker3DOF::deviceNameAndroidGravityTracker3DOF(), AndroidGravityTracker3DOF::deviceTypeGravityTracker3DOF(), InstanceFunction::create(*this, &AndroidFactory::createAndroidGravityTracker3DOF));
				}
...
bool AndroidFactory::registerCustomDevice(const std::string& deviceName, const std::string& stringType, const Device::DeviceType& deviceType)
{
	...
	Log::error() << "Android API level 21 is required to register custom devices";

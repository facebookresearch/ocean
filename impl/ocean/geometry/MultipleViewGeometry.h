...
class OCEAN_GEOMETRY_EXPORT MultipleViewGeometry
{
	...
	public:
		 ...
		 * @param flippedCamera1_P_world Resulting camera projection matrix of first view (inverted flipped)
		 * @param flippedCamera2_P_world Resulting camera projection matrix of second view (inverted flipped)
		 * @param flippedCamera3_P_world Resulting camera projection matrix of third view (inverted flipped)
		 ...
		 * @param flippedCameras_P_world The camera projection matrices (at least 3, inverted flipped)
		 * @param imageWidth Width of image
		 * @param imageHeight Height of image
		 * @param cameraIntrinsic The resulting intrinsic camera matrix
		 * @param flippedCameras_T_world Resulting inverted and flipped camera poses, one for each given camera projection matrix
		 * @return True, if succeeded
		 * @see calibrateFromProjectionsMatricesIF()
		 */
		 ...
		 * @param flippedCamera1_P_world First camera projection matrices (inverted flipped)
		 * @param flippedCamera2_P_world Second camera projection matrices (inverted flipped)
		 * @param flippedCamera3_P_world Third camera projection matrices (inverted flipped)
		 * @param imageWidth Width of image
		 * @param imageHeight Height of image
		 * @param cameraIntrinsics Resulting camera intrinsics matrix
		 * @param flippedCamera1_T_world Resulting first camera pose (inverted flipped)
		 * @param flippedCamera2_T_world Resulting second camera pose (inverted flipped)
		 * @param flippedCamera3_T_world Resulting third camera pose (inverted flipped)
		 * @return True, if succeeded
		 * @see calibrateFromProjectionsMatricesIF()
		 */
	...
	protected:
		 ...
		 * @param flippedCameraB_P_world projection matrix P_2 in homogeneous format
		 * @param flippedCameraC_P_world projection matrix P_3 in homogeneous format
		 * @param trifocal Resulting trifocal tensor matrices
		 * @return True, if succeeded
		 */
		 ...
		 * @param flippedCameraA_P_world projection matrix P_1 in homogeneous format
		 * @param flippedCameraB_P_world projection matrix P_2 in homogeneous format
		 * @param flippedCameraC_P_world projection matrix P_3 in homogeneous format
		 * @param trifocal Resulting trifocal tensor matrices
		 * @return True, if succeeded
		 ...
		 * @param trifocal Trifocal tensor matrices
		 * @param flippedCamera2NormedEpipole Resulting (normalized) epipole in second view
		 * @param flippedCamera3NormedEpipole Resulting (normalized) epipole in third view
		 * @return True, if succeeded
		 */
		 ...
		 * @param trifocal Trifocal tensor matrices
		 * @param flippedCamera2Epipole Epipole in second view
		 * @param flippedCamera3Epipole Epipole in third view
		 ...
		 * @param trifocal Trifocal tensor matrices
		 * @param flippedCamera2NormedEpipole Normalized epipole in second view
		 * @param flippedCamera3NormedEpipole Normalized epipole in third view
		 * @param flippedCamera2_P_world Resulting camera projection matrices of second view (determined only up to a common projective 3d transformation)
		 * @param flippedCamera3_P_world Resulting camera projection matrices of third view (determined only up to a common projective 3d transformation)
		 * @return True, if succeeded
		 */
...
class OCEAN_GEOMETRY_EXPORT AutoCalibration
{
	public:
		 ...
		 * @param flippedCameras_P_world The inverted and flipped camera projection matrices [K * P0], [K * P1], [K * P2], ..., with Pi = [Ri | ti], at least 3
		 ...
		 */
		static bool findCommonIntrinsicsFromProjectionMatricesIF(const ConstIndexedAccessor<HomogenousMatrix4>& flippedCameras_P_world, SquareMatrix3& cameraIntrinsics, SquareMatrix4* Q = nullptr, SquareMatrix3* omega = nullptr);
		 ...
		 * @param flippedCameras_P_world Inverted and flipped camera projection matrices [K | P0], [K | P1], [K | P2], ..., with Pi = [Ri | ti], at least 3
		 ...
		 * @see findAbsoluteDualQuadricLinear()
		 */
		static bool findCommonIntrinsicsFromProjectionMatricesIF(const ConstIndexedAccessor<HomogenousMatrix4>& flippedCameras_P_world, const unsigned int imageWidth, const unsigned int imageHeight, SquareMatrix3& cameraIntrinsics, SquareMatrix4* Q = nullptr, SquareMatrix3* omega = nullptr);
		 ...
		 */
		static bool determineAbsoluteDualQuadricLinearIF(const ConstIndexedAccessor<HomogenousMatrix4>& flippedCameras_P_world, SquareMatrix4& matrixQ, const unsigned int imageWidth, const unsigned int imageHeight, bool equalFxFy = true);
		 ...
		 * @param flippedCameras_P_world The inverted and flipped camera projection pose matrices
		 ...
		 */
		static bool intrinsicsFromAbsoluteDualQuadricIF(const SquareMatrix4& symmetricQ, const ConstIndexedAccessor<HomogenousMatrix4>& flippedCameras_P_world, SquareMatrix3* intrinsics);
		 ...
		 * @param flippedCameras_P_world The camera projection matrices (inverted flipped)
		 * @param metricFlippedCameras_P_world Resulting metric projection matrices (inverted flipped)
		 * @param transformation Optional resulting perspective transformation matrix
		 * @return True, if succeeded
		 * @see getTransformProjectiveToMetricMatrix()
		 */
		static bool transformProjectiveToMetricIF(const SquareMatrix4& Q, const ConstIndexedAccessor<HomogenousMatrix4>& flippedCameras_P_world, HomogenousMatrix4* metricFlippedCameras_P_world, SquareMatrix4* transformation = nullptr);
		 ...
		 * @param metricFlippedCameras_P_world The inverted and flipped metric camera projection matrices
		 * @param intrinsic The already known intrinsic camera matrix K
		 * @param flippedCameras_T_world The resulting inverted and flipped camera poses, one for each given camera projection matrix
		 * @return True, if succeeded
		 * @see transformProjectiveToMetric().
		 */
		static bool metricProjectionMatricesToPosesIF(const ConstIndexedAccessor<HomogenousMatrix4>& metricFlippedCameras_P_world, const SquareMatrix3& intrinsic, HomogenousMatrix4* flippedCameras_T_world);
	...
	protected:
		 ...
		 * P' = T * P.
		 * @param flippedCameras_P_world Inverted and flipped camera projection matrices, at least one
		 ...
		 * @param normalizedFlippedCameras_P_world Resulting normalized inverted and flipped camera projection matrices, one for each given camera projection matrix
		 * @param backTransformation Optional resulting back transformation matrix T^-1
		 * @return True, if succeeded
		 * @see transformProjectiveToMetric()
		 */
		static bool transformProjectionsZeroPrinciplePoint(const ConstIndexedAccessor<HomogenousMatrix4>& flippedCameras_P_world, const unsigned int imageWidth, const unsigned int imageHeight, HomogenousMatrix4* normalizedFlippedCameras_P_world, SquareMatrix3* backTransformation = nullptr);
	...
	private:
		 ...
		 * @param flippedCamera_P_world The inverted and flipped camera projection matrix, must be valid
		 * @return A line matrix [1x10] containing the values of the upper triangle entities of the symmetric matrix Q&infin;
		 */
		static Matrix createLinearSystemForAbsoluteDualQuadric(const unsigned int omegaRowIndex, const unsigned int omegaColumnIndex, const HomogenousMatrix4& flippedCamera_P_world);
};

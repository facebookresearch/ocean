...
SquareMatrix3 Normalization::normalizedTransformation(const SquareMatrix3& transformation)
{
	ocean_assert(Numeric::isNotEqualEps(transformation[8]));

	if (Numeric::isNotEqualEps(transformation[8]))
	{
		const Scalar factor = Scalar(1) / transformation[8];

		return SquareMatrix3(transformation[0] * factor, transformation[1] * factor, transformation[2] * factor,
...
void Normalization::normalizeTransformation(SquareMatrix3& transformation)
{
	ocean_assert(Numeric::isNotEqualEps(transformation[8]));

	if (Numeric::isNotEqualEps(transformation[8]))
	{
		transformation *= Scalar(1) / transformation[8];
	}
}

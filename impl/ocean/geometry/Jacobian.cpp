...
{
	ocean_assert(jx && jy);
	ocean_assert(!invertedFlippedOrientation.isSingular());
	ocean_assert(objectPointDistance > Numeric::eps());

	if (!distortImagePoint)
	{
		/**
		 * This functions uses four concatenated jacobian matrices to receive one final jacobian matrix with size 2x2.
		 * The final jacobian is defined by: j = jDeh * jProj * jTrans * jO
		 *
		 ...
		 * | 0    0   1 |
		 *
		 * jTrans (3x3) the jacobian for the orientation of the camera:
		...
		const Vector3 objectPoint(sphericalObjectPoint.quaternion() * Vector3(0, 0, -objectPointDistance));

		const Vector3 transformedObjectPoint(invertedFlippedOrientation * objectPoint);
		...
		const Scalar m02 = factor * pinholeCamera.principalPointX() - (transformedObjectPoint.x() * pinholeCamera.focalLengthX() + transformedObjectPoint.z() * pinholeCamera.principalPointX()) * Numeric::sqr(factor);
		const Scalar m12 = factor * pinholeCamera.principalPointY() - (transformedObjectPoint.y() * pinholeCamera.focalLengthY() + transformedObjectPoint.z() * pinholeCamera.principalPointY()) * Numeric::sqr(factor);

		StaticMatrix<Scalar, 2, 3> jacobianDehomogenousProjectionRotation;
		jacobianDehomogenousProjectionRotation[0] = m00 * invertedFlippedOrientation[0] + m02 * invertedFlippedOrientation[2]; // first row of the sub-jacobian
		jacobianDehomogenousProjectionRotation[1] = m00 * invertedFlippedOrientation[3] + m02 * invertedFlippedOrientation[5];
		jacobianDehomogenousProjectionRotation[2] = m00 * invertedFlippedOrientation[6] + m02 * invertedFlippedOrientation[8];

		jacobianDehomogenousProjectionRotation[3] = m11 * invertedFlippedOrientation[1] + m12 * invertedFlippedOrientation[2]; // second row of the sub-jacobian
		jacobianDehomogenousProjectionRotation[4] = m11 * invertedFlippedOrientation[4] + m12 * invertedFlippedOrientation[5];
		jacobianDehomogenousProjectionRotation[5] = m11 * invertedFlippedOrientation[7] + m12 * invertedFlippedOrientation[8];

		StaticMatrix<Scalar, 3, 3> jacobianObjectPoint;
		...
		StaticMatrix<Scalar, 2, 3> finalJacobian(jacobianDehomogenousProjectionRotation * jacobianObjectPoint);
		...
		// slow implementation not utilizing the zeros in the matrices:
		/*const Vector3 objectPoint(sphericalObjectPoint.quaternion() * Vector3(0, 0, -objectPointDistance));
		...
		const StaticMatrix<Scalar, 3, 3> jacobianTransformation(invertedFlippedOrientation.data(), false);

		const Vector3 transformedObjectPoint(invertedFlippedOrientation * objectPoint);
		ocean_assert(Numeric::isNotEqualEps(transformedObjectPoint.z()));

		const Scalar factor = 1 / transformedObjectPoint.z();
		const Vector2 projectedObjectPoint(transformedObjectPoint.x() * pinholeCamera.focalLengthX() + transformedObjectPoint.z() * pinholeCamera.principalPointX(),
												transformedObjectPoint.y() * pinholeCamera.focalLengthY() + transformedObjectPoint.z() * pinholeCamera.principalPointY());
		...
		const StaticMatrix<Scalar, 2, 3> finalJacobian(((jacobianDehomogenous * jacobianProjection) * jacobianTransformation) * jacobianObjectPoint);
	...
	else
	{
		...
		jacobianDehomogenous(0, 1) = 0;
		jacobianDehomogenous(1, 1) = factor;
		jacobianDehomogenous(0, 2) = -transformedObjectPoint.x() * factor * factor;
		jacobianDehomogenous(1, 2) = -transformedObjectPoint.y() * factor * factor;

		StaticMatrix<Scalar, 2, 2> jacobianFocal_Dist;

		const Vector2 normalizedObjectPoint(transformedObjectPoint.x() * factor, transformedObjectPoint.y() * factor);

		const Scalar& x = normalizedObjectPoint.x();
		const Scalar& y = normalizedObjectPoint.y();
		...
		// Fx (1+6 p2 x+2 p1 y+k1 (3 x^2+y^2)+k2 (x^2+y^2) (5 x^2+y^2))
		jacobianFocal_Dist[0] = Fx * (1 + 6 * p2 * x + 2 * p1 * y + k1 * (3 * x * x + y * y) + k2 * (x * x + y * y) * (5 * x * x + y * y));

		// 2 Fy (p1 x + y (p2 + x (k1 + 2 k2 (x^2 + y^2))))
		jacobianFocal_Dist[2] = 2 * Fy * (p1 * x + y * (p2 + x * (k1 + 2 * k2 * (x * x + y * y))));

		// 2 Fx (p1 x + y (p2 + x (k1 + 2 k2 (x^2 + y^2))))
		jacobianFocal_Dist[1] = 2 * Fx * (p1 * x + y * (p2 + x * (k1 + 2 * k2 * (x * x + y * y))));

		// Fy (1 + 2 p2 x + 6 p1 y + k1 (x^2 + 3 y^2) + k2 (x^2 + y^2) (x^2 + 5 y^2))
		jacobianFocal_Dist[3] = Fy * (1 + 2 * p2 * x + 6 * p1 * y + k1 * (x * x + 3 * y * y) + k2 * (x * x + y * y) * (x * x + 5 * y * y));

		const StaticMatrix<Scalar, 2, 3> finalJacobian(((jacobianFocal_Dist * jacobianDehomogenous) * jacobianTransformation) * jacobianObjectPoint);

...
bool RANSAC::p3p(const AnyCamera& anyCamera, const ConstIndexedAccessor<Vector3>& objectPointAccessor, const ConstIndexedAccessor<Vector2>& imagePointAccessor, RandomGenerator& randomGenerator, HomogenousMatrix4& pose_world_T_camera, const unsigned int minimalValidCorrespondences, const bool refine, const unsigned int iterations, const Scalar sqrPixelErrorThreshold, Indices32* usedIndices, Scalar* sqrAccuracy)
{
	...
	HomogenousMatrix4 cameraPoses_world_T_camera[4];
	HomogenousMatrix4 internalPose_world_T_camera;
	...
	for (unsigned int i = 0u; i < adpativeIterations; ++i)
	{
		...
		const unsigned int numberPoses = P3P::poses(permutationObjectPoints, permutationImageRays, cameraPoses_world_T_camera);
	...
	pose_world_T_camera = internalPose_world_T_camera;
	...
	if (refine)
	{
		...
		if (!NonLinearOptimizationPose::optimizePose(anyCamera, internalPose_world_T_camera, ConstArraySubsetAccessor<Vector3, unsigned int>(objectPoints.data(), bestIndices), ConstArraySubsetAccessor<Vector2, unsigned int>(imagePoints.data(), bestIndices), pose_world_T_camera, 20u, Estimator::ET_SQUARE, Scalar(0.001), Scalar(5), nullptr, sqrAccuracy))
		{
			return false;
		}
		...
		if (usedIndices && bestIndices.size() != correspondences)
		{
			...
			if (bestIndices.size() != bestIndicesUsedForOptimization)
			{
				...
				internalPose_world_T_camera = pose_world_T_camera;
...
bool RANSAC::objectPoint(const PinholeCamera& pinholeCamera, const ConstIndexedAccessor<HomogenousMatrix4>& poses, const ConstIndexedAccessor<ImagePoint>& imagePoints, RandomGenerator& randomGenerator, ObjectPoint& objectPoint, const bool distortImagePoints, const unsigned int iterations, const Scalar maximalSqrError, const unsigned int minValidCorrespondences, const bool onlyFrontObjectPoint, const Estimator::EstimatorType refinementEstimator, Scalar* finalSqrError, Indices32* usedIndices)
{
	ocean_assert(pinholeCamera.isValid() && poses.size() == imagePoints.size() && poses.size() >= 2 && maximalSqrError >= 0);
	ocean_assert(iterations >= 1u);
	...
	Lines3 rays(poses.size());
	HomogenousMatrices4 posesIF(poses.size());
	for (size_t n = 0; n < poses.size(); ++n)
	{
		rays[n] = pinholeCamera.ray(distortImagePoints ? pinholeCamera.undistort<true>(imagePoints[n]) : imagePoints[n], poses[n]);
		posesIF[n] = PinholeCamera::standard2InvertedFlipped(poses[n]);
	}

	Scalar bestSqrError = Numeric::maxValue();
	size_t bestNumber = min(size_t(minValidCorrespondences), poses.size());

	Indices32 bestIndices;

	Indices32 indices;
	indices.reserve(posesIF.size());

	for (unsigned int i = 0u; i < iterations; ++i)
	{
		unsigned int index0, index1;
		RandomI::random(randomGenerator, (unsigned int)posesIF.size() - 1u, index0, index1);
		...
		if (rays[index0].nearestPoint(rays[index1], candidate))
		{
			...
			{
				if (!onlyFrontObjectPoint || PinholeCamera::isObjectPointInFrontIF(posesIF[n], candidate))
				{
					const Scalar localSqrError = imagePoints[n].sqrDistance(pinholeCamera.projectToImageIF<true>(posesIF[n], candidate, distortImagePoints));
	...
	if (refinementEstimator != Estimator::ET_INVALID)
	{
		...
		if (bestIndices.size() == posesIF.size())
		{
			if (Geometry::NonLinearOptimizationObjectPoint::optimizeObjectPointForFixedPosesIF(anyCamera, ConstArrayAccessor<HomogenousMatrix4>(posesIF), objectPoint, imagePoints, optimizedObjectPoint, 10u, refinementEstimator, Scalar(0.001), Scalar(5), onlyFrontObjectPoint, nullptr, finalSqrError))
			{
				objectPoint = optimizedObjectPoint;
			}
		}
		...
		{
			if (Geometry::NonLinearOptimizationObjectPoint::optimizeObjectPointForFixedPosesIF(anyCamera, ConstArraySubsetAccessor<HomogenousMatrix4, unsigned int>(posesIF, bestIndices), objectPoint, ConstIndexedAccessorSubsetAccessor<ImagePoint, unsigned int>(imagePoints, bestIndices), optimizedObjectPoint, 10u, refinementEstimator, Scalar(0.001), Scalar(5), onlyFrontObjectPoint, nullptr, finalSqrError))
			{
				objectPoint = optimizedObjectPoint;
			}
		}
...
bool RANSAC::objectPoint(const FisheyeCamera& fisheyeCamera, const ConstIndexedAccessor<HomogenousMatrix4>& poses_world_T_camera, const ConstIndexedAccessor<Vector2>& imagePoints, RandomGenerator& randomGenerator, Vector3& worldObjectPoint, const unsigned int iterations, const Scalar maximalSqrError, const unsigned int minValidCorrespondences, const bool onlyFrontObjectPoint, const Estimator::EstimatorType refinementEstimator, Scalar* finalSqrError, Indices32* usedIndices)
{
	ocean_assert(fisheyeCamera.isValid() && poses_world_T_camera.size() == imagePoints.size() && poses_world_T_camera.size() >= 2 && maximalSqrError >= 0);
	ocean_assert(minValidCorrespondences <= (unsigned int)poses_world_T_camera.size());
	ocean_assert(iterations >= 1u);

	if (poses_world_T_camera.size() <= 1)
	{
		return false;
	}

	Lines3 rays(poses_world_T_camera.size());
	HomogenousMatrices4 poses_flippedCamera_T_world(poses_world_T_camera.size());

	for (size_t n = 0; n < poses_world_T_camera.size(); ++n)
	{
		rays[n] = fisheyeCamera.ray(imagePoints[n], poses_world_T_camera[n]);
		poses_flippedCamera_T_world[n] = PinholeCamera::standard2InvertedFlipped(poses_world_T_camera[n]);
	}
	...
	Indices32 indices;
	indices.reserve(poses_flippedCamera_T_world.size());

	for (unsigned int i = 0u; i < iterations; ++i)
	{
		unsigned int index0, index1;
		RandomI::random(randomGenerator, (unsigned int)poses_flippedCamera_T_world.size() - 1u, index0, index1);

		Vector3 candidateObjectPoint;
		if (rays[index0].nearestPoint(rays[index1], candidateObjectPoint))
		{
			Scalar sqrError = 0;
			indices.clear();

			for (size_t n = 0; n < poses_flippedCamera_T_world.size(); ++n)
			{
				if (!onlyFrontObjectPoint || PinholeCamera::isObjectPointInFrontIF(poses_flippedCamera_T_world[n], candidateObjectPoint))
				{
					const Scalar localSqrError = imagePoints[n].sqrDistance(fisheyeCamera.projectToImageIF(poses_flippedCamera_T_world[n], candidateObjectPoint));
			...
			if (indices.size() >= size_t(minValidCorrespondences) && (indices.size() > bestIndices.size() || (indices.size() == bestIndices.size() && sqrError < bestSqrError)))
			{
				worldObjectPoint = candidateObjectPoint;
				bestSqrError = sqrError;
				bestIndices = std::move(indices);
			}
		}
	...
	if (refinementEstimator != Estimator::ET_INVALID)
	{
		...
		if (bestIndices.size() == poses_flippedCamera_T_world.size())
		{
			if (Geometry::NonLinearOptimizationObjectPoint::optimizeObjectPointForFixedPosesIF(AnyCameraFisheye(fisheyeCamera), ConstArrayAccessor<HomogenousMatrix4>(poses_flippedCamera_T_world), worldObjectPoint, imagePoints, optimizedObjectPoint, 10u, refinementEstimator, Scalar(0.001), Scalar(5), onlyFrontObjectPoint, nullptr, finalSqrError))
			{
				worldObjectPoint = optimizedObjectPoint;
			}
		}
		else
		{
			if (Geometry::NonLinearOptimizationObjectPoint::optimizeObjectPointForFixedPosesIF(AnyCameraFisheye(fisheyeCamera), ConstArraySubsetAccessor<HomogenousMatrix4, unsigned int>(poses_flippedCamera_T_world, bestIndices), worldObjectPoint, ConstIndexedAccessorSubsetAccessor<Vector2, unsigned int>(imagePoints, bestIndices), optimizedObjectPoint, 10u, refinementEstimator, Scalar(0.001), Scalar(5), onlyFrontObjectPoint, nullptr, finalSqrError))
			{
				worldObjectPoint = optimizedObjectPoint;
			}
		}
...
bool RANSAC::determineCameraCalibrationPlanar(const unsigned int width, const unsigned int height, const CameraCalibration::Patterns& calibrationPatterns, const Vector2& calibrationPatternBoxSize, const unsigned int testCandidates, PinholeCamera& pinholeCamera, const unsigned int iterations, Scalar* sqrAccuracy, Worker* worker, Indices32* usedIndices)
{
	...
	typedef std::vector<PinholeCamera> CameraVector;
	...
	CameraVector cameras(indexVectorSet.size());
	...
	if (worker)
	{
		...
		for (unsigned int n = 0u; n < indexVectorSet.size(); ++n)
			functions.push_back(Worker::Function::createStatic(&RANSAC::determineCameraCalibrationPlanarIteration, width, height, (const ConstIndexedAccessor<Vectors3>*)&objectPointGroupsAccessor, (const ConstIndexedAccessor<Vectors2>*)&imagePointGroupsAccessor, &indexVectorSet[n], &cameras[n], &sqrAccuracies[n]));
	...
	for (unsigned int n = 0u; n < objectPointGroups.size(); ++n)
	{
		...
		bool result = RANSAC::p3p(finalCamera, ConstArrayAccessor<ObjectPoint>(objectPoints), ConstArrayAccessor<ImagePoint>(imagePoints), randomGenerator, finalCamera.hasDistortionParameters(), roughPose);
		ocean_assert(result);
		if (!result)
			continue;
...
bool RANSAC::objectTransformationStereo(const FisheyeCamera& fisheyeCameraA, const FisheyeCamera& fisheyeCameraB, const HomogenousMatrix4& world_T_cameraA, const HomogenousMatrix4& world_T_cameraB, const ConstIndexedAccessor<Vector3>& objectPointsA, const ConstIndexedAccessor<Vector3>& objectPointsB, const ConstIndexedAccessor<Vector2>& imagePointsA, const ConstIndexedAccessor<Vector2>& imagePointsB, RandomGenerator& randomGenerator, HomogenousMatrix4& world_T_object, const unsigned int minimalValidCorrespondences, const bool refine, const unsigned int iterations, const Scalar sqrPixelErrorThreshold, Indices32* usedIndicesA, Indices32* usedIndicesB, Scalar* sqrAccuracy)
{
	...
	for (unsigned int nIteration = 0u; nIteration < adpativeIterations; ++nIteration)
	{
		...
		if (useCameraA)
		{
			...
			numberPoses = P3P::poses(fisheyeCameraA, permutationObjectPoints, permutationImagePoints, cameraPoses_object_T_camera);
		...
		else
		{
			...
			numberPoses = P3P::poses(fisheyeCameraB, permutationObjectPoints, permutationImagePoints, cameraPoses_object_T_camera);
		...
		for (unsigned int nPose = 0u; nPose < numberPoses; ++nPose)
		{
			...
			for (unsigned int nCandidateA = 0u; indicesA.size() + (correspondencesA - nCandidateA) + correspondencesB >= bestCorrespondences && nCandidateA < correspondencesA; ++nCandidateA)
			{
				...
				if (PinholeCamera::isObjectPointInFrontIF(flippedCameraA_T_object, memoryObjectPointsA[nCandidateA]))
				{
					const Vector2 projectedImagePoint(fisheyeCameraA.projectToImageIF(flippedCameraA_T_object, memoryObjectPointsA[nCandidateA]));
					const Vector2& imagePoint = memoryImagePointsA[nCandidateA];
			...
			for (unsigned int nCandidateB = 0u; indicesA.size() + indicesB.size() + (correspondencesB - nCandidateB) >= bestCorrespondences && nCandidateB < correspondencesB; ++nCandidateB)
			{
				...
				if (PinholeCamera::isObjectPointInFrontIF(flippedCameraB_T_object, memoryObjectPointsB[nCandidateB]))
				{
					const Vector2 projectedImagePoint(fisheyeCameraB.projectToImageIF(flippedCameraB_T_object, memoryObjectPointsB[nCandidateB]));
					const Vector2& imagePoint = memoryImagePointsB[nCandidateB];
	...
	if (refine)
	{
		...
		if (!NonLinearOptimizationTransformation::optimizeObjectTransformationStereoIF(fisheyeCameraA, fisheyeCameraB, flippedCamerasA_T_world, flippedCamerasB_T_world, internal_world_T_object, objectPointGroupsA, objectPointGroupsB, imagePointGroupsA, imagePointGroupsB, world_T_object, 20u, Estimator::ET_SQUARE, Scalar(0.001), Scalar(5), nullptr, sqrAccuracy))
		{
			return false;
		}
		...
		if (usedIndicesA)
		{
			...
			for (unsigned int nCandidateA = 0u; nCandidateA < correspondencesA; ++nCandidateA)
			{
				...
				// we accept only object points lying in front of the camera
				if (PinholeCamera::isObjectPointInFrontIF(flippedCameraA_T_object, objectPoint) && imagePoint.sqrDistance(fisheyeCameraA.projectToImageIF(flippedCameraA_T_object, objectPoint)) <= sqrPixelErrorThreshold)
				{
					bestIndicesA.push_back(nCandidateA);
				}
			}
		...
		if (usedIndicesB)
		{
			...
			for (unsigned int nCandidateB = 0u; nCandidateB < correspondencesB; ++nCandidateB)
			{
				...
				// we accept only object points lying in front of the camera
				if (PinholeCamera::isObjectPointInFrontIF(flippedCameraB_T_object, objectPoint) && imagePoint.sqrDistance(fisheyeCameraB.projectToImageIF(flippedCameraB_T_object, objectPoint)) <= sqrPixelErrorThreshold)
				{
					bestIndicesB.push_back(nCandidateB);
				}
			}
...
bool RANSAC::p3p(const HomogenousMatrix4* initialPose, const PinholeCamera& pinholeCamera, const ConstIndexedAccessor<ObjectPoint>& objectPointAccessor, const ConstIndexedAccessor<ImagePoint>& imagePointAccessor, RandomGenerator& randomGenerator, const bool useDistortionParameters, HomogenousMatrix4& pose, const Vector3* maxPositionOffset, const Scalar* maxOrientationOffset, const unsigned int minValidCorrespondences, const bool refine, const unsigned int iterations, const Scalar sqrPixelErrorThreshold, Indices32* usedIndices, Scalar* sqrAccuracy, const Scalar* weights)
{
	...
	for (unsigned int i = 0u; i < iterations; ++i)
	{
		...
		const unsigned int numberPoses = P3P::poses(pinholeCamera, permutationObjectPoints, permutationImagePoints, poses);
		ocean_assert(numberPoses <= 4u);
		...
		for (unsigned int n = 0u; n < numberPoses; ++n)
		{
			...
			{
				// we accept only object points lying in front of the camera
				if ((poseIF * objectPoints[c]).z() > Numeric::eps())
				{
					const Vector2 projectedImagePoint(pinholeCamera.projectToImageIF<true>(poseIF, objectPoints[c], useDistortionParameters));
					const ImagePoint& imagePoint = imagePoints[c];
	...
	if (refine)
	{
		...
		if (!NonLinearOptimizationPose::optimizePose(pinholeCamera, internalPose, ConstArraySubsetAccessor<Vector3, unsigned int>(objectPoints.data(), bestIndices), ConstArraySubsetAccessor<Vector2, unsigned int>(imagePoints.data(), bestIndices), useDistortionParameters, pose, 20u, Estimator::ET_SQUARE, Scalar(0.001), Scalar(5), nullptr, sqrAccuracy, invertedCovariances ? &invertedCovariances : nullptr))
			return false;
		...
		if (usedIndices && bestIndices.size() != correspondences)
		{
			...
			{
				// we accept only object points lying in front of the camera
				if ((poseIF * objectPoints[c]).z() > Numeric::eps() && imagePoints[c].sqrDistance(pinholeCamera.projectToImageIF<true>(poseIF, objectPoints[c], useDistortionParameters)) <= sqrPixelErrorThreshold)
					bestIndices.push_back(c);
			}

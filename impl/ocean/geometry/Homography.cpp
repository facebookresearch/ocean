...
bool Homography::homographyMatrixFromPointsAndLinesSVD(const Vector2* leftPoints, const Vector2* rightPoints, const size_t pointCorrespondences, const Line2* leftLines, const Line2* rightLines, const size_t lineCorrespondences, SquareMatrix3& right_H_left)
{
	...
	{
		const Vector2& left = leftPoints[nPoint];
		const Vector2& right = rightPoints[nPoint];

		Scalar* const row0 = matrix[row++];
	...
	{
		Vector3 left = leftLines[nLine].decomposeNormalDistance();
		Vector3 right = rightLines[nLine].decomposeNormalDistance();

		if (Numeric::isEqualEps(left.z()) || Numeric::isEqualEps(right.z()))
		{
	...
	const unsigned int lastColumn = (unsigned int)(v.columns()) - 1u;
	right_H_left = SquareMatrix3(v(0u, lastColumn), v(3u, lastColumn), v(6u, lastColumn), v(1u, lastColumn), v(4u, lastColumn), v(7u, lastColumn), v(2u, lastColumn), v(5u, lastColumn), v(8u, lastColumn));

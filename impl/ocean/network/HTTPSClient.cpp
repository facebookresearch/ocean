...
bool HTTPSClient::httpsGetRequest(const std::string& url, Buffer& data, const Port& port, const double timeout, bool* abort, const ProgressCallback& progressCallback)
{
	...
	curl_easy_setopt(curlHandle.object(), CURLOPT_URL, url.c_str());
	curl_easy_setopt(curlHandle.object(), CURLOPT_PORT, long(port.readable()));

	CurlSessionData curlSessionData(abort, progressCallback);

	curl_easy_setopt(curlHandle.object(), CURLOPT_WRITEFUNCTION, CurlSessionData::onNewData);
	curl_easy_setopt(curlHandle.object(), CURLOPT_WRITEDATA, &curlSessionData);

	curl_easy_setopt(curlHandle.object(), CURLOPT_XFERINFOFUNCTION, CurlSessionData::onProgress);
	curl_easy_setopt(curlHandle.object(), CURLOPT_XFERINFODATA, &curlSessionData);

	const CURLcode result = curl_easy_perform(curlHandle.object());

	if (result != CURLE_OK)
	{
		long responseCode = 0;
		curl_easy_getinfo(curlHandle.object(), CURLINFO_RESPONSE_CODE, &responseCode);
...
bool HTTPSClient::httpsPostRequest(const std::string& url, const uint8_t* requestData, const size_t requestDataSize, Buffer& data, const Port& port, const double timeout, const Strings& additionalHeaders)
{
	...
	curl_easy_setopt(curlHandle.object(), CURLOPT_POSTFIELDSIZE, long(requestDataSize));

	const char* charRequestData = (const char*)(requestData);
	curl_easy_setopt(curlHandle.object(), CURLOPT_POSTFIELDS, const_cast<char*>(charRequestData));
	...
	if (!additionalHeaders.empty())
	{
		...
		curl_easy_setopt(curlHandle.object(), CURLOPT_HTTPHEADER, headerList.object());
	}

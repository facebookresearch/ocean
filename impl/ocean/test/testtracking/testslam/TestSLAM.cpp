...
bool testSLAM(const double testDuration, Worker& /*worker*/, const std::string& testFunctions)
{
	ocean_assert(testDuration > 0.0);

	TestResult testResult("Ocean Tracking SLAM Library test");
	...
	if (TestSelector subSelector = selector.shouldRun("localizedobjectpoint"))
	{
		Log::info() << " ";
		Log::info() << " ";
		Log::info() << " ";
...
static void testSLAMAsynchronInternal(const double testDuration, const std::string testFunctions)
{
	ocean_assert(testDuration > 0.0);
	...
	try
	{
		testSLAM(testDuration, worker, testFunctions);
	}
...
void testSLAMAsynchron(const double testDuration, const std::string& testFunctions)
{
	TaskQueue::get().pushTask(TaskQueue::Task::createStatic(&testSLAMAsynchronInternal, testDuration, testFunctions));
}

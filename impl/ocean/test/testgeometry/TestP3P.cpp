...
bool TestP3P::test(const double testDuration)
{
	...
	allSucceeded = testRANSACP3PPinholeCamera(testDuration) && allSucceeded;

	Log::info() << " ";
	Log::info() << "-";
	Log::info() << " ";

	allSucceeded = testRANSACP3PZoom(testDuration) && allSucceeded;
...
TEST(TestP3P, RANSACP3PPinholeCamera)
{
	EXPECT_TRUE(TestP3P::testRANSACP3PPinholeCamera(GTEST_TEST_DURATION));
}

TEST(TestP3P, RANSACP3PZoom)
{
	EXPECT_TRUE(TestP3P::testRANSACP3PZoom(GTEST_TEST_DURATION));
}
...
bool TestP3P::testRANSACP3PPinholeCamera(const double testDuration)
{
	ocean_assert(testDuration > 0.0);
	...
	do
	{
		// determine random points inside a unit circle which are not colinear
		...
		{
			objectPoints.push_back(Vector3(Random::scalar(-1, 1), Random::scalar(Scalar(-0.1), Scalar(0.1)), Random::scalar(-1, 1)));
		}
		...
		{
			outlierIndices.insert(RandomI::random((unsigned int)objectPoints.size() - 1u));
		}

		for (IndexSet32::const_iterator i = outlierIndices.begin(); i != outlierIndices.end(); ++i)
		{
			imagePoints[*i] = Vector2(Random::scalar(0, Scalar(pinholeCamera.width())), Random::scalar(0, Scalar(pinholeCamera.height())));
		}
...
bool TestP3P::testRANSACP3PZoom(const double testDuration)
{
	ocean_assert(testDuration > 0.0);
	...
	do
	{
		...
		const Scalar perfectZoom = Random::scalar(Scalar(0.1), 10);
	...
	while (iterations == 0ull || startTimestamp + testDuration > Timestamp(true));

...
bool TestJacobian::test(const double testDuration)
{
	...
	allSucceeded = testAnyCameraPoseJacobian2x6(testDuration) && allSucceeded;
	...
	allSucceeded = testSphericalObjectPointOrientation2x3(testDuration) && allSucceeded;
...
TEST(TestJacobian, AnyCameraPoseJacobian2nx6)
{
	EXPECT_TRUE(TestJacobian::testAnyCameraPoseJacobian2x6(GTEST_TEST_DURATION));
}
...
TEST(TestJacobian, SphericalObjectPointOrientation2x3)
{
...
bool TestJacobian::testAnyCameraPoseJacobian2x6(const double testDuration)
{
	...
	Log::info() << "Testing any camera pose Jacobian rodrigues 2x6 for " << numberPoints << " points:";
	...
	for (const AnyCameraType anyCameraType : Utilities::realisticCameraTypes())
	{
		...
		do
		{
			...
			while (objectPoints.size() < numberPoints)
			{
				...
				const Line3 ray(anyCamera.ray(imagePoint, transformation));
				const Vector3 objectPoint(ray.point(Random::scalar(1, 5)));
...

			{
				...
				for (size_t n = 0; n < objectPoints.size(); ++n)
				{
					Geometry::Jacobian::calculatePoseJacobianRodrigues2x6IF(anyCamera, poseIF.transformation(), objectPoints[n], dwx, dwy, dwz, jacobian[n * 2 + 0], jacobian[n * 2 + 1]);
				}
			}

			{
				...
				{
					const Vector3 objectPoint = objectPoints[n];
					const Vector2 imagePoint(anyCamera.projectToImageIF(transformationIF, objectPoint));
					...
					for (unsigned int i = 0u; i < 6u; ++i)
					{
						...
						const Vector2 imagePointDelta(anyCamera.projectToImageIF(poseDelta.transformation(), objectPoint));
						const Vector2 derivative = (imagePointDelta - imagePoint) / Numeric::weakEps();
			...
			for (size_t n = 0; n < objectPoints.size(); ++n)
			{
...

				{
					...
					for (const Scalar epsilon : epsilons)
					{
						...
						const Vector2 imagePointWx(anyCamera.projectToImageIF(poseWx.transformation(), objectPoint));
						if (checkAccuracy(imagePoint, imagePointWx, epsilon, jacobianX[0], jacobianY[0]))
						{
							localAccuracy = true;
							break;
...

				{
					...
					for (const Scalar epsilon : epsilons)
					{
						...
						const Vector2 imagePointWy(anyCamera.projectToImageIF(poseWy.transformation(), objectPoint));
						if (checkAccuracy(imagePoint, imagePointWy, epsilon, jacobianX[1], jacobianY[1]))
						{
							localAccuracy = true;
							break;
...

				{
					...
					for (const Scalar epsilon : epsilons)
					{
						...
						const Vector2 imagePointWz(anyCamera.projectToImageIF(poseWz.transformation(), objectPoint));
						if (checkAccuracy(imagePoint, imagePointWz, epsilon, jacobianX[2], jacobianY[2]))
						{
							localAccuracy = true;
							break;
...

				{
					...
					for (const Scalar epsilon : epsilons)
					{
						...
						const Vector2 imagePointTx(anyCamera.projectToImageIF(poseTx.transformation(), objectPoint));
						if (checkAccuracy(imagePoint, imagePointTx, epsilon, jacobianX[3], jacobianY[3]))
						{
							localAccuracy = true;
							break;
...

				{
					...
					for (const Scalar epsilon : epsilons)
					{
						...
						const Vector2 imagePointTy(anyCamera.projectToImageIF(poseTy.transformation(), objectPoint));
						if (checkAccuracy(imagePoint, imagePointTy, epsilon, jacobianX[4], jacobianY[4]))
						{
							localAccuracy = true;
							break;
...

				{
					...
					for (const Scalar epsilon : epsilons)
					{
						...
						const Vector2 imagePointTz(anyCamera.projectToImageIF(poseTz.transformation(), objectPoint));
						if (checkAccuracy(imagePoint, imagePointTz, epsilon, jacobianX[5], jacobianY[5]))
						{
							localAccuracy = true;
							break;
...
bool TestJacobian::testSphericalObjectPointOrientation2x3(const double testDuration)
{
	...
	do
	{
		...
		const Scalar radius = std::is_same<Scalar, double>::value ? Random::scalar(Scalar(0.001), 100) : Random::scalar(Scalar(0.1), 10);
		const Vector3 defaultObjectPoint(defaultRotationDirection * radius);

		const SquareMatrix3 orientation(Random::rotation());
		const SquareMatrix3 invertedFlippedOrientation(PinholeCamera::standard2InvertedFlipped(orientation));

		const Vector2 testImagePoint(Random::scalar(5, width - 5u), Random::scalar(5, height - 5u));

		const Vector3 rotationDirection(camera.ray(camera.undistort<true>(testImagePoint), HomogenousMatrix4(orientation)).direction());
		ocean_assert(Numeric::isEqual(rotationDirection.length(), 1));

		const Vector3 testObjectPoint(rotationDirection * radius);
		...
		Geometry::Jacobian::calculateSphericalObjectPointOrientationJacobian2x3(jacobianX, jacobianY, camera, invertedFlippedOrientation, sphericalObjectPoint, radius, camera.hasDistortionParameters());

		const Vector3 objectPoint(sphericalObjectPoint.rotation() * Vector3(0, 0, -radius));
		ocean_assert_and_suppress_unused(objectPoint.isEqual(testObjectPoint, Numeric::weakEps()), testObjectPoint);

		const Vector2 imagePoint(camera.projectToImageIF<false>(HomogenousMatrix4(invertedFlippedOrientation), objectPoint, true));

		{
			...
			for (const Scalar epsilon : epsilons)
			{
				...
				const Vector2 imagePointWx(camera.projectToImageIF<false>(HomogenousMatrix4(invertedFlippedOrientation), objectPointWx, true));
...

		{
			...
			for (const Scalar epsilon : epsilons)
			{
				...
				const Vector2 imagePointWy(camera.projectToImageIF<false>(HomogenousMatrix4(invertedFlippedOrientation), objectPointWy, true));
...

		{
			...
			for (const Scalar epsilon : epsilons)
			{
				...
				const Vector2 imagePointWz(camera.projectToImageIF<false>(HomogenousMatrix4(invertedFlippedOrientation), objectPointWz, true));

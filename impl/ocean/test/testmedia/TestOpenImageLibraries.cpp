		...
		if (compressionSucceeded)
		{
			...
			if (targetFrame)
			{
				...
				else if (sourceFrame.frameType() == FrameType(targetFrame, FrameType::ORIGIN_LOWER_LEFT))
				{
					...
					Frame flippedFrame;
					if (CV::FrameConverter::Comfort::convert(targetFrame, FrameType(targetFrame, FrameType::ORIGIN_LOWER_LEFT), flippedFrame, false))
					{
				...
				else if (sourceFrame.pixelFormat() == FrameType::FORMAT_YUV24 && targetFrame.pixelFormat() == FrameType::FORMAT_RGB24)
				{
					...
					Frame convertedFrame;
					if (CV::FrameConverter::Comfort::convert(targetFrame, FrameType(targetFrame, FrameType::FORMAT_YUV24, sourceFrame.pixelOrigin()), convertedFrame, false))
					{
		...
		if (compressionSucceeded)
		{
			...
			if (targetFrame)
			{
				...
				else if (sourceFrame.frameType() == FrameType(targetFrame, FrameType::ORIGIN_LOWER_LEFT))
				{
					...
					if (CV::FrameConverter::Comfort::convert(targetFrame, FrameType(targetFrame, FrameType::ORIGIN_LOWER_LEFT), flippedTargetFrame, false))
					{
	...
	do
	{
		...
		if (compressionSucceeded)
		{
			...
			if (targetFrame)
			{
				...
				Frame convertedFrame;
				if (CV::FrameConverter::Comfort::convert(targetFrame, FrameType(targetFrame, sourceFrame.pixelFormat(), sourceFrame.pixelOrigin()), convertedFrame, false))
				{
					ocean_assert(convertedFrame.frameType() == sourceFrame.frameType());

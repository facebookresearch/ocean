...
bool test(const double testDuration, Worker& worker, const std::string& testFunctions)
{
	...
	Log::info() << "+++   Ocean Unified Features Library test:   +++";
	Log::info() << " ";
	...
	if (testSet.empty() || testSet.find("oceaharriscontainer") != testSet.end())
	{
		Log::info() << " ";
		Log::info() << " ";
		Log::info() << " ";
		Log::info() << " ";
		allSucceeded = TestOceanHarris::TestOceanHarrisContainer::test(testDuration) && allSucceeded;
	}
	...
	if (allSucceeded)
	{
		Log::info() << (testSet.empty() ? "Entire" : "Partial") << " Ocean Unified Features Library test succeeded.";
	}
	else
	{
		Log::info() << (testSet.empty() ? "Entire" : "Partial") << " Ocean Unified Features Library test FAILED!";
	}
...
static void testAsynchronInternal(const double testDuration, const std::string testFunctions)
{
	...
	Log::info() << "Ocean Framework test for the Unified Features library:";

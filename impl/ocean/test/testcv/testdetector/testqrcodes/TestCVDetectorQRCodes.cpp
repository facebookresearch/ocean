...
bool testCVDetectorQRCodes(const double testDuration, Worker& worker, const std::string& testFunctions)
{
	...
	if (testSet.empty() || testSet.find("microqrcodeencoder") != testSet.end())
	{
		...
		Log::info() << " ";
		Log::info() << " ";
		allSucceeded = TestMicroQRCodeEncoder::test(testDuration) && allSucceeded;
	}
	...
	if (testSet.empty() || testSet.find("microqrcodedetector2d") != testSet.end())
	{
		Log::info() << " ";
		Log::info() << " ";
		Log::info() << " ";
		Log::info() << " ";
		allSucceeded = TestMicroQRCodeDetector2D::test(testDuration, worker) && allSucceeded;
	}

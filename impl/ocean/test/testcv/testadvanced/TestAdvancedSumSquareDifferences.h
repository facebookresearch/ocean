...
#ifndef META_OCEAN_TEST_TESTCV_TESTADVANCED_TEST_ADVANCED_SUM_SQUARE_DIFFERENCES_H
#define META_OCEAN_TEST_TESTCV_TESTADVANCED_TEST_ADVANCED_SUM_SQUARE_DIFFERENCES_H
 ...
 */
class OCEAN_TEST_CV_ADVANCED_EXPORT TestAdvancedSumSquareDifferences
{
	protected:
	...
	public:
		...
		/**
		 * Tests the sum square differences function for two sub-pixel accurate patches in combination with a mask.
		 * @param testDuration Number of seconds for each test, with range (0, infinity)
		 * @return True, if succeeded
		 */
		...
		/**
		 * Tests the sum square differences function for two pixel accurate patches in combination with a rejecting mask.
		 * @param testDuration Number of seconds for each test, with range (0, infinity)
		 * @return True, if succeeded
		 */
	...
	private:
		 ...
		 * @param mask0 First mask to be used, must be valid, must be valid
		 * @param mask1 Second mask to be used, must be valid, must be valid
		 * @param center0 Center position in the first frame, with range (-infinity, infinity)x(-infinity, infinity)
		 * @param center1 Center position in the second frame, with range (-infinity, infinity)x(-infinity, infinity)
		 * @param pixelCenter The pixel center to be used
		 * @param patchSize The size of the square patch (the edge length) in pixel, with range [1, infinity), must be odd
		 * @param validMaskValue The pixel value for mask pixels which will be included in SSD calculation
		 * @return The resulting ssd
		 */
...
#endif // META_OCEAN_TEST_TESTCV_TESTADVANCED_TEST_ADVANCED_SUM_SQUARE_DIFFERENCES_H

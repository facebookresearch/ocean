...
bool TestFrameFilterGradient::test(const unsigned int width, const unsigned int height, const double testDuration, Worker& worker)
{
	...
	allSucceeded = testHorizontalVertical3Products1Channel8Bit(width, height, testDuration, worker) && allSucceeded;
...
bool TestFrameFilterGradient::testHorizontalVertical3Products1Channel8Bit(const unsigned int width, const unsigned int height, const double testDuration, Worker& worker)
{
	ocean_assert(width >= 3u && height >= 3u && testDuration > 0.0);

	Log::info() << "Testing horizontal and vertical 8 bit gradient with three response channels:";

	LegacyFrame frame8(FrameType(width, height, FrameType::FORMAT_Y8, FrameType::ORIGIN_UPPER_LEFT));
	CV::CVUtilities::randomizeFrame(frame8);

	const uint8_t* const source = frame8.constdata<uint8_t>();

	LegacyFrame targetFrame(FrameType(width, height, FrameType::genericPixelFormat<FrameType::DT_UNSIGNED_INTEGER_8, 6u>(), FrameType::ORIGIN_UPPER_LEFT));
	short* target = targetFrame.data<short>();
...

	{
		...
		{
			performance.start();
			CV::FrameFilterGradientBase::filterHorizontalVertical3Products1Channel8Bit(source, target, width, height);
			performance.stop();
		}
		while (startTimestamp + testDuration > Timestamp(true));

		Log::info() << "Standard performance: Best: " << performance.bestMseconds() << "ms, worst: " << performance.worstMseconds() << "ms, average: " << performance.averageMseconds() << "ms";

		if (validationHorizontalVertical3Products1Channel8Bit(source, target, width, height))
			Log::info() << "Validation: succeeded.";
		else
		{
			Log::info() << "Validation: FAILED!";
		...
		if (worker)
		{
			Log::info() << "Testing horizontal and vertical 8 bit gradient (multicore):";
			...
			do
			{
				...
				CV::FrameFilterGradientBase::filterHorizontalVertical3Products1Channel8Bit(source, target, width, height, &worker);
				performanceShared.stop();
			}
			while (startTimestampWorker + testDuration > Timestamp(true));

			Log::info() << "Standard performance: Best: " << performanceShared.bestMseconds() << "ms, worst: " << performanceShared.worstMseconds() << "ms, average: " << performanceShared.averageMseconds() << "ms";
			...
			Log::info() << "Multicore boost: Best: " << performance.best() / performanceShared.best() << ", worst: " << performance.worst() / performanceShared.worst() << ", average: " << performance.average() / performanceShared.average();
		}
	}
...

	{
		...
		{
			performance.start();
			CV::FrameFilterGradientSSE::filterHorizontalVertical3Products1Channel8Bit(source, target, width, height);
			performance.stop();
		}
		while (startTimestamp + testDuration > Timestamp(true));

		Log::info() << "SSE performance: Best: " << performance.bestMseconds() << "ms, worst: " << performance.worstMseconds() << "ms, average: " << performance.averageMseconds() << "ms";
		...
		if (worker)
		{
			...
			{
				performanceShared.start();
				CV::FrameFilterGradientSSE::filterHorizontalVertical3Products1Channel8Bit(source, target, width, height, &worker);
				performanceShared.stop();
			}
			while (startTimestampWorker + testDuration > Timestamp(true));

			Log::info() << "SSE performance: Best: " << performanceShared.bestMseconds() << "ms, worst: " << performanceShared.worstMseconds() << "ms, average: " << performanceShared.averageMseconds() << "ms";
...

	{
		...
		{
			performance.start();
			CV::FrameFilterGradientNEON::filterHorizontalVertical3Products1Channel8Bit(source, target, width, height);
			performance.stop();
		}
		while (startTimestamp + testDuration > Timestamp(true));

		Log::info() << "NEON performance: Best: " << performance.bestMseconds() << "ms, worst: " << performance.worstMseconds() << "ms, average: " << performance.averageMseconds() << "ms";
		...
		if (worker)
		{
			...
			{
				performanceShared.start();
				CV::FrameFilterGradientNEON::filterHorizontalVertical3Products1Channel8Bit(source, target, width, height, &worker);
				performanceShared.stop();
			}
			while (startTimestampWorker + testDuration > Timestamp(true));

			Log::info() << "NEON performance: Best: " << performanceShared.bestMseconds() << "ms, worst: " << performanceShared.worstMseconds() << "ms, average: " << performanceShared.averageMseconds() << "ms";
...

	{
		...
		{
			performance.start();
			CV::FrameFilterGradient::filterHorizontalVertical3Products1Channel8Bit(source, target, width, height);
			performance.stop();
		}
		while (startTimestamp + testDuration > Timestamp(true));

		Log::info() << "Default performance: Best: " << performance.bestMseconds() << "ms, worst: " << performance.worstMseconds() << "ms, average: " << performance.averageMseconds() << "ms";
		...
		if (worker)
		{
			...
			{
				performanceShared.start();
				CV::FrameFilterGradient::filterHorizontalVertical3Products1Channel8Bit(source, target, width, height, &worker);
				performanceShared.stop();
			}
			while (startTimestampWorker + testDuration > Timestamp(true));

			Log::info() << "Default performance: Best: " << performanceShared.bestMseconds() << "ms, worst: " << performanceShared.worstMseconds() << "ms, average: " << performanceShared.averageMseconds() << "ms";
...
bool TestFrameFilterGradient::validationHorizontalVertical3Products1Channel8Bit(const unsigned char* original, const short* filtered, const unsigned int width, const unsigned int height)
{
	ocean_assert(original && filtered && width >= 3u && height >= 3u);
	...
	for (unsigned int x = 0u; x < width; ++x)
	{
		...
		// bottom border
		if (filtered[3 * ((height - 1) * width + x) + 0] != 0)
			return false;

		if (filtered[3 * ((height - 1) * width + x) + 1] != 0)
			return false;

		if (filtered[3 * ((height - 1) * width + x) + 2] != 0)
			return false;
	}
	...
	{
		// left border
		if (filtered[3 * (y * width) + 0] != 0)
			return false;

		if (filtered[3 * (y * width) + 1] != 0)
			return false;

		if (filtered[3 * (y * width) + 2] != 0)
			return false;

		// left border
		if (filtered[3 * (y * width + width - 1) + 0] != 0)
			return false;

		if (filtered[3 * (y * width + width - 1) + 1] != 0)
			return false;

		if (filtered[3 * (y * width + width - 1) + 2] != 0)
			return false;
	}
	...
	for (unsigned int y = 1u; y < height - 1u; y++)
	{
		...
		{
			// [-1 0 1]
			const int horizontal = (original[y * width + x + 1] - original[y * width + x - 1]) / 2;

			// [-1]
			// [ 0]
			// [ 1]
			const int vertical = (original[(y + 1) * width + x] - original[(y - 1) * width + x]) / 2;

			bool foundMatch = false;

			for (int hDelta = -1; !foundMatch && hDelta <= 1; ++hDelta)
			{
				for (int vDelta = -1; !foundMatch && vDelta <= 1; ++vDelta)
				{
					const int testHorizontal = horizontal + hDelta;
					const int testVertical = vertical + vDelta;

					const int horizontal2 = testHorizontal * testHorizontal;
					const int vertical2 = testVertical * testVertical;
					const int horiverti = testHorizontal * testVertical;

					if (abs(horizontal2 - int(filtered[3 * (y * width + x) + 0])) <= 4
							&& abs(vertical2 - int(filtered[3 * (y * width + x) + 1])) <= 4
							&& abs(horiverti - int(filtered[3 * (y * width + x) + 2])) <= 4)
					{
						foundMatch = true;
					}
				}

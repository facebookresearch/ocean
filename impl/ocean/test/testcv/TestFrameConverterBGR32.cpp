...
bool TestFrameConverterBGR32::test(const unsigned int width, const unsigned int height, const double testDuration, Worker& worker)
{
...

	{
		...
		for (size_t n = 0; n < flags.size(); ++n)
		{
			...
			allSucceeded = testBGR32ToRGB24(width, height, flags[n], testDuration, worker) && allSucceeded;
		}
	}
...

	{
		...
		{
			Log::info().newLine(n != 0);
			allSucceeded = testBGR32ToRGBA32(width, height, flags[n], testDuration, worker) && allSucceeded;
		}
	}

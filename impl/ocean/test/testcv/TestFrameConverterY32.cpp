...
bool TestFrameConverterY32::test(const unsigned int width, const unsigned int height, const double testDuration, Worker& worker)
{
...

	{
		...
		for (size_t n = 0; n < flags.size(); ++n)
		{
			...
			allSucceeded = testY32ToY8(width, height, flags[n], testDuration, worker) && allSucceeded;
		}
	}
...

	{
		...
		{
			Log::info().newLine(n != 0);
			allSucceeded = testY32ToY16(width, height, flags[n], testDuration, worker) && allSucceeded;
		}
	}

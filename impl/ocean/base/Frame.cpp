...
/**
 * This class implements a manager for frame memory.
 * Managing the frame on our own can be suitable if the OS memory management system does not provide the same memory buffer again (after a buffer has been released) although the size of the memory has not changed.<br>
 * Therefore, this manager ensures that successive allocation (while freeing the buffer in-between) of the same buffer (with same buffer size) provides the same buffer pointer.<br>
 * This manager has an own thread and frees not used memory after a specified time frame.<br>
 * This object is thread-safe.
 * @ingroup base
 */
class LegacyFrame::MemoryManager :
	...
	private Thread
{
...
#ifdef OCEAN_DEBUG
		/// True, to activate the buffer overflow check.
		static const bool managerBufferOverflowCheck = true;
#endif

		/**
		 * Definition of a pair combining a timestamp and a buffer pointer.
		 */
		typedef std::pair<Timestamp, void*> BufferPair;

		/**
		 * Definition of a vector holding buffer pairs.
		 */
		typedef std::vector<BufferPair> BufferPairs;

		/**
		 * Definition of a map mapping buffer sizes to buffer pairs.
		 */
		typedef std::map<size_t, BufferPairs> BufferPairsMap;

		/**
		 * Definition of a map mapping buffer points to buffer sizes.
		 */
		typedef std::map<void*, size_t> BufferSizeMap;

	public:

		/**
		 * Allocates a new buffer with specified buffer size.
		 * This function is equivalent to a standard malloc() call.
		 * @param size The size of the requested buffer in bytes, with range [0, infinity)
		 * @return The requested buffer if a buffer could be allocated, otherwise nullptr
		 */
		void* allocateBuffer(const size_t size);

		/**
		 * Frees a previously allocated buffer.
		 * This function is equivalent to a standard free() call.
	...
	protected:
		...
		/**
		 * Releases all managed memory buffers.
		 * Ensure that no buffer is used by an external instance anymore before calling this function.
		 */
		void release();
		 ...
		 * @return The allocated buffer, nullptr if the buffer could not be allocated
		 */
		static inline void* internalMalloc(const size_t size);

#ifdef OCEAN_DEBUG

		/**
		 * Checks the consistency of a buffer.
		 * Beware: Do not call this function if 'managerBufferOverflowCheck' is False.
		 * @param realBuffer The real buffer to be checked (including the extra bytes)
		 * @param realSize The real size of the buffer (including the extra bytes), with range [2, infinity)
		 * @return True, if succeeded
		 */
		static inline bool checkConsistency(void* realBuffer, const size_t realSize);

		/**
		 * Internal free function.
		 * @param buffer The buffer to be freed
		 * @param size The size of the buffer to be freed in bytes
		 */
		static inline void internalFree(void* buffer, const size_t size);
		 ...
		 * @param buffer The buffer to be freed
		 */
		static inline void internalFree(void* buffer);

#endif

	protected:

		/// The map of currently available buffers.
		BufferPairsMap managerAvailableBuffersMap;

		/// The map of buffers currently used (and thus not available).
		BufferSizeMap managerUsedBuffersMap;

		/// The time an unused buffer can exist until the buffer is removed by the manager, in seconds, with range (0, infinity)
		double managerBufferIdleTime;

		/// The lock of the manager.
		Lock managerLock;
};

LegacyFrame::MemoryManager::MemoryManager() :
	managerBufferIdleTime(10.0)
{
	startThread();
}
...
void* LegacyFrame::MemoryManager::allocateBuffer(const size_t size)
{
	if (size == 0)
	{
		return nullptr;
	...
	TemporaryScopedLock temporaryScopedLock(managerLock);

	BufferPairsMap::iterator i = managerAvailableBuffersMap.find(size);

	if (i == managerAvailableBuffersMap.end() || i->second.empty())
	{
		// we do not have a free buffer with the requested size
		// so we create a new buffer, but outside the manager's lock

		temporaryScopedLock.release();

		void* buffer = internalMalloc(size);

		// now we enter the manager's lock again and add the buffer

		const ScopedLock scopedLock(managerLock);

		ocean_assert(managerUsedBuffersMap.find(buffer) == managerUsedBuffersMap.end());
		managerUsedBuffersMap[buffer] = size;

		return buffer;
	}

	// we take the most recent buffer object
	void* buffer = i->second.back().second;

	// and we remove the buffer from the list of available buffers
	i->second.pop_back();
...
void LegacyFrame::MemoryManager::freeBuffer(void* buffer)
{
	if (buffer == nullptr)
	{
		return;
	...
	const BufferSizeMap::const_iterator iU = managerUsedBuffersMap.find(buffer);
	ocean_assert(iU != managerUsedBuffersMap.end());

	const size_t bufferSize = iU->second;

	// we remove the buffer from the list of used buffers
	managerUsedBuffersMap.erase(iU);

	// we either insert new pairs or access the existing ones
	BufferPairs& availableBufferPairs = managerAvailableBuffersMap[bufferSize];

#ifdef OCEAN_DEBUG

	// we ensure that the buffer is not registered as available/free buffer
	for (BufferPairs::const_iterator iB = availableBufferPairs.begin(); iB != availableBufferPairs.end(); ++iB)
	{
		ocean_assert(iB->second != buffer);
	}

	if (managerBufferOverflowCheck)
	{
		ocean_assert(checkConsistency((void*)(((const unsigned char*)buffer) - 1), bufferSize + 2) && "The buffer has been manipulated outside the allowed memory space!");
	}

#endif

	// now we add the buffer to the list of available buffers
	availableBufferPairs.push_back(BufferPair(Timestamp(true), buffer));
}

void LegacyFrame::MemoryManager::release()
{
	const ScopedLock scopedLock(managerLock);

	ocean_assert(managerUsedBuffersMap.empty());

	for (BufferPairsMap::iterator iA = managerAvailableBuffersMap.begin(); iA != managerAvailableBuffersMap.end(); ++iA)
	{
		for (BufferPairs::const_iterator i = iA->second.cbegin(); i != iA->second.cend(); ++i)
		{
			ocean_assert(i->second != nullptr);

#ifdef OCEAN_DEBUG
			internalFree(i->second, iA->first);
#else
			internalFree(i->second);
#endif
		}
	}

	managerAvailableBuffersMap.clear();
}

void LegacyFrame::MemoryManager::threadRun()
{
	ocean_assert(managerBufferIdleTime > 0.0);

#ifdef OCEAN_DEBUG
	std::vector<std::pair<void*, size_t>> buffersToFree;
#else
	std::vector<void*> buffersToFree;
#endif
	buffersToFree.reserve(10);

	BufferPairs tempBufferPairs;
	tempBufferPairs.reserve(100);
	...
	while (!shouldThreadStop())
	{
		...
		Timestamp removeTimestamp(currentTimestamp - managerBufferIdleTime);

		TemporaryScopedLock temporaryScopedLock(managerLock);

		for (BufferPairsMap::iterator iA = managerAvailableBuffersMap.begin(); iA != managerAvailableBuffersMap.end(); /* noop */)
		{
			tempBufferPairs.clear();

			for (BufferPairs::const_iterator i = iA->second.cbegin(); i != iA->second.cend(); ++i)
			{
				if (i->first < removeTimestamp)
				{
					// the buffer is old and hasn't been requested for a while, so we free the bufffer

#ifdef OCEAN_DEBUG
					buffersToFree.push_back(std::make_pair(i->second, iA->first));
#else
					buffersToFree.push_back(i->second);
#endif
				}
				else
				{
					// the buffer is not old and may be requested in the future
					tempBufferPairs.push_back(*i);
				}
			}

			// we replace the existing buffer list with our temporary list
			std::swap(iA->second, tempBufferPairs);

			// we finally remove all pairs with zero entries
			if (iA->second.empty())
			{
				iA = managerAvailableBuffersMap.erase(iA);
			}
			else
			{
				++iA;
		...
		for (size_t n = 0; n < buffersToFree.size(); ++n)
		{
#ifdef OCEAN_DEBUG
			internalFree(buffersToFree[n].first, buffersToFree[n].second);
#else
			internalFree(buffersToFree[n]);
#endif
		}
...
inline void* LegacyFrame::MemoryManager::internalMalloc(const size_t size)
{
#ifdef OCEAN_DEBUG

	if (managerBufferOverflowCheck)
	{
		// we allocate two additional bytes (at the front and at the end of the buffer) and we set these bytes to a constant value
		void* buffer = frameAlignedMalloc(size + 2);

		((uint8_t*)buffer)[0] = 0xED;
		((uint8_tr*)buffer)[size + 1] = 0xED;

		return (void*)(((uint8_t*)buffer) + 1);
	}
	else
	{
		return frameAlignedMalloc(size);
	}
...
inline bool LegacyFrame::MemoryManager::checkConsistency(void* realBuffer, const size_t realSize)
{
	ocean_assert(managerBufferOverflowCheck);
	ocean_assert(realBuffer && realSize >= 2);

	// we check whether the last byte in the buffer still has the constant value
	return ((const uint8_t*)realBuffer)[0] == 0xED && ((const uint8_t*)realBuffer)[realSize - 1] == 0xED;
}

inline void LegacyFrame::MemoryManager::internalFree(void* buffer, const size_t size)
{
	ocean_assert(buffer && size != 0);

	if (managerBufferOverflowCheck)
	{
		void* realBuffer = (void*)(((const uint8_t*)buffer) - 1);
		ocean_assert(checkConsistency(realBuffer, size + 2) && "The buffer has been manipulated outside the allowed memory space!");

		frameAlignedFree(realBuffer);
	}
	else
	{
		frameAlignedFree(buffer);
	}
}

#else // OCEAN_DEBUG

inline void LegacyFrame::MemoryManager::internalFree(void* buffer)
{
	frameAlignedFree(buffer);
}
...
#endif // OCEAN_USE_FRAME_MEMORY_MANAGER

LegacyFrame::LegacyFrame() :
	FrameType(),
	allocatedData_(nullptr),
	frameData(nullptr),
	constFrameData(nullptr)
{
	// nothing to do here
}

LegacyFrame::LegacyFrame(const LegacyFrame& frame) :
	FrameType(frame),
	allocatedData_(nullptr),
	frameData(frame.frameData),
	constFrameData(frame.constFrameData),
	frameTimestamp(frame.frameTimestamp),
	frameRelativeTimestamp(frame.frameRelativeTimestamp)
{
	if (frame.allocatedData_ != nullptr)
	{
		const unsigned int bytes = frame.size();

		allocatedData_ = frameMalloc(bytes, frameData);
		...
		if (frameData)
		{
			if (frame.frameData)
			{
				memcpy(frameData, frame.frameData, bytes);
			}
			else
			{
				memcpy(frameData, frame.constFrameData, bytes);
			}
		}
	}
...
LegacyFrame::LegacyFrame(LegacyFrame&& frame) noexcept :
	FrameType(frame.frameType()),
	allocatedData_(frame.allocatedData_),
	frameData(frame.frameData),
	constFrameData(frame.constFrameData),
	frameTimestamp(frame.frameTimestamp),
	frameRelativeTimestamp(frame.frameRelativeTimestamp)
{
	dynamic_cast<FrameType&>(frame) = FrameType();

	frame.allocatedData_ = nullptr;
	frame.frameData = nullptr;
	frame.constFrameData = nullptr;
}

LegacyFrame::LegacyFrame(Frame&& frame) noexcept :
	LegacyFrame()
{
	*this = std::move(frame);
}

LegacyFrame::LegacyFrame(const Frame& frame, const FrameCopyMode frameCopyMode) noexcept :
	LegacyFrame()
{
	if (frame.isValid())
	{
		FrameType::operator=(frame.frameType());

		if (frameCopyMode == FCM_COPY_REMOVE_PADDING_LAYOUT || frame.numberPlanes() != 1u || !frame.isContinuous())
		{
			allocatedData_ = frameMalloc(size(), frameData);
			ocean_assert(frameData != nullptr);

			unsigned int dataOffset = 0u;

			for (const Frame::Plane& plane : frame.planes_)
			{
				for (unsigned int y = 0u; y < plane.height(); ++y)
				{
					ocean_assert(dataOffset + plane.widthBytes() <= size());

					memcpy(frameData + dataOffset, (const uint8_t*)(plane.constData_) + y * plane.strideBytes(), plane.widthBytes());
					dataOffset += plane.widthBytes();
				}
			}

			ocean_assert(dataOffset == size());
		}
		else
		{
			ocean_assert(frame.planes_.size() == 1);
			const Frame::Plane& plane = frame.planes_[0];

			ocean_assert(allocatedData_ == nullptr);

			if (plane.data_ != nullptr)
			{
				frameData = (unsigned char*)plane.data_;
			}
			else
			{
				constFrameData = (const unsigned char*)plane.constData_;
			}
		}

		frameTimestamp = frame.timestamp_;
		frameRelativeTimestamp = frame.relativeTimestamp_;
	}
}

LegacyFrame::LegacyFrame(const LegacyFrame& frame, const bool copyData) :
	FrameType(frame),
	allocatedData_(nullptr),
	frameData(nullptr),
	constFrameData(nullptr),
	...
	frameRelativeTimestamp(frame.frameRelativeTimestamp)
{
	if (copyData)
	{
		...
		if (bytes != 0u)
		{
			...
			if (frameData)
			{
				ocean_assert(frame.frameData || frame.constFrameData);
...
LegacyFrame::LegacyFrame(const FrameType& type) :
	FrameType(type),
	allocatedData_(nullptr),
	frameData(nullptr),
	constFrameData(nullptr)
...
LegacyFrame::LegacyFrame(const FrameType& type, const Timestamp timestamp) :
	FrameType(type),
	allocatedData_(nullptr),
	frameData(nullptr),
	constFrameData(nullptr),
...
LegacyFrame::LegacyFrame(const FrameType& type, const unsigned char* data, const bool copyData) :
	FrameType(type),
	allocatedData_(nullptr),
	frameData(nullptr),
	constFrameData(nullptr)
...
LegacyFrame::LegacyFrame(const FrameType& type, unsigned char* data, const bool copyData) :
	FrameType(type),
	allocatedData_(nullptr),
	frameData(nullptr),
	constFrameData(nullptr)
...
LegacyFrame::LegacyFrame(const FrameType& type, const Timestamp timestamp, const unsigned char* data, const bool copyData) :
	FrameType(type),
	allocatedData_(nullptr),
	frameData(nullptr),
	constFrameData(nullptr),
...
LegacyFrame::LegacyFrame(const FrameType& type, const Timestamp timestamp, unsigned char* data, const bool copyData) :
	FrameType(type),
	allocatedData_(nullptr),
	frameData(nullptr),
	constFrameData(nullptr),
...
LegacyFrame::~LegacyFrame()
{
	...
	if (allocatedData_ != nullptr)
	{
		frameFree(allocatedData_);
		allocatedData_ = nullptr;
	}
}

bool LegacyFrame::copy(const LegacyFrame& source)
{
	ocean_assert(source);
	ocean_assert(this != &source);
	...
	if (frameType() != source.frameType() || !isOwner())
	{
		set(source.frameType(), true, true);
	}

	ocean_assert(size() == source.size());
	memcpy(frameData, source.constdata<void>(), size());

	frameTimestamp = source.frameTimestamp;
	frameRelativeTimestamp = source.frameRelativeTimestamp;

	return true;
}

void LegacyFrame::set(const FrameType& type, const bool forceOwner, const bool forceWritable)
{
	if (!type.isValid())
	{
		if (allocatedData_ != nullptr)
		...
		frameRelativeTimestamp.toInvalid();
		frameTimestamp.toInvalid();

		return;
	}

	if ((isOwner() || !forceOwner) && (frameData != nullptr || !forceWritable))
	{
		if (frameType() == type)
		{
			return;
		...
		// Do not reallocate frame buffer if it has equal or sufficient size:
		if (size() >= type.frameTypeSize())
		{
			(FrameType&)(*this) = type;
	...
	// we explicitly make a copy of the frame type as we cannot guarantee that: this != &type
	const FrameType frameTypeCopy(type);
	...
	const unsigned int newBytes = frameTypeCopy.frameTypeSize();

	if (allocatedData_ != nullptr) // Frame owns buffer, needs reallocation
	{
		frameFree(allocatedData_);
		allocatedData_ = nullptr;
		...
		ocean_assert(newBytes != 0u);
		allocatedData_ = frameMalloc(newBytes, frameData);
		ocean_assert(frameData != nullptr);
	}
	else // Frame does not own buffer, needs allocation of a new buffer
	{
		ocean_assert(!isOwner());
	...
	(FrameType&)(*this) = frameTypeCopy;
}

void LegacyFrame::set(const FrameType& type, const Timestamp timestamp, const unsigned char* data, const bool copyData)
{
	ocean_assert(data != nullptr);
...
void LegacyFrame::set(const FrameType& type, const Timestamp timestamp, unsigned char* data, const bool copyData)
{
	ocean_assert(data != nullptr);
...
bool LegacyFrame::hasAlphaPixel() const
{
	if (!formatHasAlphaChannel(pixelFormat()))
	{
		return false;
	}
	...
	switch (pixelFormat())
	{
		...
		case FORMAT_BGRA32:
		{
			const uint8_t* pointer = constdata<uint8_t>() + 3;
		...
		case FORMAT_RGBA64:
		{
			const unsigned short* pointer = constdata<unsigned short>() + 3;
		...
		case FORMAT_YA16:
		{
			const uint8_t* pointer = constdata<uint8_t>() + 1;
...
bool LegacyFrame::haveIntersectingMemory(const LegacyFrame& frame) const
{
	ocean_assert(isValid() && frame.isValid());
	...
	const size_t thisMemoryStart = size_t(constdata<void>());
	const size_t thisMemoryEnd = thisMemoryStart + size(); // exclusive

	const size_t frameMemoryStart = size_t(frame.constdata<void>());
	const size_t frameMemoryEnd = frameMemoryStart + frame.size(); // exclusive

	// thisMemory       [start                ]end
	// frameMemory                                          [start      ]end

	return !(frameMemoryEnd <= thisMemoryStart || frameMemoryStart >= thisMemoryEnd);
}

LegacyFrame& LegacyFrame::operator=(LegacyFrame&& right) noexcept
{
	if (this != &right)
	{
		release();

		dynamic_cast<FrameType&>(*this) = right.frameType();

		allocatedData_ = right.allocatedData_;
		frameData = right.frameData;
		constFrameData = right.constFrameData;

		frameTimestamp = right.frameTimestamp;
		frameRelativeTimestamp = right.frameRelativeTimestamp;

		dynamic_cast<FrameType&>(right) = FrameType();
...
LegacyFrame& LegacyFrame::operator=(Frame&& frame) noexcept
{
	release();
	...
	if (frame.isValid())
	{
		...
		{
			// the given Frame is compatible with this Frame object, so that we can move the data

			ocean_assert(frame.planes_.size() == 1);
			Frame::Plane& plane = frame.planes_[0];
		...
		{
			// the given Frame is not with this Frame object, so that we have to copy the data
...
LegacyFrame& LegacyFrame::operator=(const LegacyFrame& right)
{
	if (this != &right)
	{
		if (right)
		{
			set((FrameType&)right, right.timestamp(), right.constdata<uint8_t>(), right.isOwner());
		}
		else
		{
			release();
...
void* LegacyFrame::frameMalloc(const size_t size, unsigned char*& alignedData)
{
	ocean_assert(memoryAlignment >= size_t(1));
...
#ifdef OCEAN_USE_FRAME_MEMORY_MANAGER
		void* allocatedData = MemoryManager::get().allocateBuffer(size + memoryAlignment);
#else
		void* allocatedData = malloc(size + memoryAlignment);
#endif
		ocean_assert(allocatedData != nullptr);

		if (allocatedData != nullptr)
		{
			const size_t alignmentOffset = (memoryAlignment - (size_t(allocatedData) % memoryAlignment)) % memoryAlignment;

			ocean_assert(alignmentOffset < memoryAlignment);
			ocean_assert((size_t(allocatedData) + alignmentOffset) % memoryAlignment == size_t(0));

			alignedData = ((uint8_t*)allocatedData) + alignmentOffset;
			ocean_assert(alignedData >= allocatedData);
...
void LegacyFrame::frameFree(void* buffer)
{
#ifdef OCEAN_USE_FRAME_MEMORY_MANAGER

	return MemoryManager::get().freeBuffer(buffer);

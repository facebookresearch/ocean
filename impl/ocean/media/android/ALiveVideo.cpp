...
void ALiveVideo::onCaptureCompleted(ACameraCaptureSession* session, ACaptureRequest* request, const ACameraMetadata* result)
{
	...
	TemporaryScopedLock scopedLock(lock_);
		...
		const bool frameCreated = frameFromImage(image, frame, timestampNs);

		// the frame owns a copy of the image data, so the image is returned to the reader while the lock still guarantees the reader exists
		// deleting it further below would race with a stop() deleting the reader, which invalidates every image acquired from it

		NativeMediaLibrary::get().AImage_delete(image);

		if (frameCreated)
		{
			// the lock is released before the frame is delivered, the callbacks the delivery invokes must not run under this lock

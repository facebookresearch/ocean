...
HRESULT STDMETHODCALLTYPE MFMedium::EventCallback::Invoke(IMFAsyncResult* pAsyncResult)
{
	...
	if (S_OK == eventGenerator_.EndGetEvent(pAsyncResult, &mediaEvent.resetObject()))
	{
		...
		mediaEvent.object()->GetType(&eventType);
		mediaEvent.object()->GetStatus(&eventStatus);

		switch (eventType)
		{
			case MESessionTopologySet:
			{
				ScopedMediaFoundationObject<IMFTopology> topology = Utilities::eventObject<IMFTopology>(mediaEvent.object());
				ocean_assert(topology.object() != nullptr);

				if (topology.object() && callbackTopologySet_)
				{
					callbackTopologySet_(topology.object());
				}
			...
			{
				TOPOID nodeId = 0ull;
				if (S_OK == mediaEvent.object()->GetUINT64(MF_EVENT_OUTPUT_NODE, &nodeId) && callbackFormatTypeChanged_)
				{
...
MFMedium::~MFMedium()
{
	...
	if (eventCallback_.object() != nullptr)
	{
		eventCallback_.object()->deactivate();
	}
...
{
	if (mediaSession_.object() != nullptr)
	{
		return true;
	}
	...
	ocean_assert(eventCallback_.object() == nullptr);
	...
	mediaSession_.object()->BeginGetEvent(eventCallback_.object(), nullptr);
...
void MFMedium::releaseMediaSession()
{
	...
	{
		if (S_OK == mediaSession_.object()->Close())
		{
			const Timestamp startTimestamp(true);

			while (!eventCallback_.object()->isMediaSessionClosed())
			{
...
void MFMedium::releaseTopology()
{
	if (mediaSession_.object() != nullptr)
	{
		mediaSession_.object()->ClearTopologies();
	}
...
{
	if (mediaSource_.object() != nullptr)
	{
		return true;
	}
	...
	return mediaSource_.object() != nullptr;
}
...
bool MFMedium::startMediaSession()
{
	...
	if (S_OK != mediaSession_.object()->Start(&GUID_NULL, &startPosition))
	{
		return false;
	}
...
bool MFMedium::pauseMediaSession()
{
	...
	if (S_OK != mediaSession_.object()->Pause())
	{
		return false;
	}
...
bool MFMedium::stopMediaSession()
{
	...
	if (S_OK != mediaSession_.object()->Stop())
	{
		return false;
	}

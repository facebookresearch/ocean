...
double MFFiniteMedium::normalDuration() const
{
	...
	if (S_OK != mediaSource_.object()->CreatePresentationDescriptor(&presentationDescriptor))
	{
		return 0.0;
	}
...
double MFFiniteMedium::position() const
{
	...
	if (S_OK != mediaSession_.object()->GetClock(&clock.resetObject()))
	{
		return -1.0;
	}
	...
	if (S_OK != clock.object()->GetCorrelatedTime(0, &clockTime, &systemTime))
	{
		return -1.0;
	}
...
bool MFFiniteMedium::setSpeed(const float speed)
{
	...
	if (S_OK == MFGetService(mediaSession_.object(), MF_RATE_CONTROL_SERVICE, IID_IMFRateControl, (void**)(&rateControl.resetObject())))
	{
		if (S_OK == rateControl.object()->SetRate(false, speed))
		{
			success = true;
			speed_ = speed;
		}

...
float MFSoundMedium::soundVolume() const
{
	...
	if (S_OK == MFGetService(mediaSession_.object(), MR_STREAM_VOLUME_SERVICE, IID_IMFAudioStreamVolume, (void**)(&audioStreamVolume.resetObject())))
	{
		float level = 0.0f;
		UINT32 channels = 0;

		if (S_OK == audioStreamVolume.object()->GetChannelCount(&channels))
		{
			for (UINT32 i = 0; i < channels; ++i)
			{
				float cLevel = 0.0;

				if (S_OK == audioStreamVolume.object()->GetChannelVolume(i, &cLevel))
				{
...
bool MFSoundMedium::setSoundVolume(const float volume)
{
	...
	if (S_OK == MFGetService(mediaSession_.object(), MR_STREAM_VOLUME_SERVICE, IID_IMFAudioStreamVolume, (void**)(&audioStreamVolume.resetObject())))
	{
		...
		if (S_OK == audioStreamVolume.object()->GetChannelCount(&channels))
		{
			for (UINT32 i = 0; i < channels; ++i)
			{
				success = S_OK == audioStreamVolume.object()->SetChannelVolume(i, level) && success;
			}
		}
	}
...
bool MFSoundMedium::buildSoundTopology()
{
	...
	if (!Media::MediaFoundation::Utilities::connectSelectedStream(topology_.object(), mediaSource_.object(), sinkActivate.object(), MFMediaType_Audio))
	{
		return false;
	}

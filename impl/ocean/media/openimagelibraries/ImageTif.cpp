...
Frame ImageTif::decodeImage(const void* buffer, const size_t size)
{
	...
	if (TIFFGetField(scopedTiff.object(), TIFFTAG_IMAGEWIDTH, &width) != 1 || TIFFGetField(scopedTiff.object(), TIFFTAG_IMAGELENGTH, &height) != 1 || TIFFGetField(scopedTiff.object(), TIFFTAG_SAMPLESPERPIXEL, &samplesPerPixel) != 1 || width == 0u || height == 0u || samplesPerPixel == 0u)
	{
		return Frame();
	}

	unsigned short orientation = (unsigned short)(-1);
	if (TIFFGetField(scopedTiff.object(), TIFFTAG_ORIENTATION, &orientation) != 1 || (orientation != ORIENTATION_TOPLEFT && orientation != ORIENTATION_BOTLEFT))
	{
		return Frame();
	}
	...
	if (TIFFReadRGBAImageOriented(scopedTiff.object(), width, height, result.data<uint32_t>(), orientation, 1) != 1)
	{
		return Frame();
	}
...
bool ImageTif::encodeImage(const Frame& frame, std::vector<uint8_t>& buffer, const bool allowConversion, bool* hasBeenConverted)
{
	...
	if (TIFFSetField(scopedTiff.object(), TIFFTAG_IMAGEWIDTH, outputFrame->width()) != 1 || TIFFSetField(scopedTiff.object(), TIFFTAG_IMAGELENGTH, outputFrame->height()) != 1)
	{
		return false;
	}
	...
	if (TIFFSetField(scopedTiff.object(), TIFFTAG_SAMPLESPERPIXEL, tiffSamplesPerPixel) != 1 || TIFFSetField(scopedTiff.object(), TIFFTAG_BITSPERSAMPLE, tiffBitsPerSample) != 1)
	{
		return false;
	}
	...
	if (outputFrame->pixelFormat() == FrameType::FORMAT_RGBA32)
	{
		...
		if (TIFFSetField(scopedTiff.object(), TIFFTAG_EXTRASAMPLES, 1, tt) != 1)
		{
			return false;
		}
	}
	...
	if (TIFFSetField(scopedTiff.object(), TIFFTAG_ORIENTATION, orientation) != 1|| TIFFSetField(scopedTiff.object(), TIFFTAG_PLANARCONFIG, PLANARCONFIG_CONTIG) != 1 || TIFFSetField(scopedTiff.object(), TIFFTAG_PHOTOMETRIC, PHOTOMETRIC_RGB) != 1)
	{
		return false;
	}

	if (TIFFSetField(scopedTiff.object(), TIFFTAG_ROWSPERSTRIP, 1u) != 1 || TIFFSetField(scopedTiff.object(), TIFFTAG_COMPRESSION, COMPRESSION_NONE) != 1)
	{
		return false;
	}
	...
	{
		TIFFWriteEncodedStrip(scopedTiff.object(), y, const_cast<void*>(outputFrame->constrow<void>(y)), outputFrame->planeWidthBytes(0u));
	}

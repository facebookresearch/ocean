...
{
	ocean_assert(inputPin_.object() != nullptr);
	return inputPin_.object()->specifyMediaType(mediaType);
}
...
bool DSSampleSinkFilter::establishedMediaType(DSMediaType& mediaType)
{
	...
	mediaType = DSMediaType(inputPin_.object()->m_mt);

	return inputPin_.object()->m_Connected != nullptr;
}

bool DSSampleSinkFilter::respectPlaybackTime() const
{
	ocean_assert(inputPin_.object() != nullptr);
	return inputPin_.object()->respectPlaybackTime();
}

bool DSSampleSinkFilter::setRespectPlaybackTime(const bool state)
{
	ocean_assert(inputPin_.object() != nullptr);
	return inputPin_.object()->setRespectPlaybackTime(state);
}

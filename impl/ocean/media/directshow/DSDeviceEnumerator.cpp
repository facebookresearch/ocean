...
bool DSDeviceEnumerator::refresh()
{
	...
	if (S_OK != systemDeviceEnumerator.object()->CreateClassEnumerator(classIdentifier_, &deviceEnumerator.resetObject(), 0))
	{
		return false;
	}
	...
	while (deviceEnumerator.object()->Next(1, &moniker.resetObject(), &fetched) == S_OK)
	{
		ScopedIPropertyBag propertyBag;
		if (S_OK == moniker.object()->BindToStorage(0, 0, IID_IPropertyBag, (void**)(&propertyBag.resetObject())))
		{
			...
			if (S_OK == propertyBag.object()->Read(L"FriendlyName", &friendlyName, nullptr))
			{
			...
			if (S_OK == propertyBag.object()->Read(L"Description", &description, nullptr))
			{
				object.deviceDescription_ = String::toAString(description.bstrVal);
			}

			if (S_OK == propertyBag.object()->Read(L"DevicePath", &devicePath, nullptr))
			{
...
ScopedIMoniker DSDeviceEnumerator::moniker(const std::string& name) const
{
	for (const Object& object : objects_)
	{
		if (object.friendlyName_ == name)
		{
			IMoniker* copyMoniker = object.moniker_.object();
			copyMoniker->AddRef();

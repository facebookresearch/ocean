...
{
	ocean_assert(outputPin_.object() != nullptr);

	return outputPin_.object()->lockBufferToFill(buffer, size, respectFrameFrequency);
}
...
void DSSampleSourceFilter::unlockBufferToFill(const size_t size)
{
	...
	outputPin_.object()->unlockBufferToFill(size);
}
...
bool DSSampleSourceFilter::setFormat(const std::string& compression, const FrameType& type, const double frequency)
{
	...
	try
	{
		...
		ocean_assert(outputPin_.object() != nullptr);
		return outputPin_.object()->specifyMediaType(mediaType, frequency);
	}

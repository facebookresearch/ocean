...
bool DSFrameMedium::collectFrameFormats(IPin* pin, FrameTypes& frameTypes)
{
	...
	while (S_OK == enumerator.object()->Next(1, &type, nullptr))
	{
...
bool DSFrameMedium::createFrameSampleSinkFilter()
{
	...
	const ScopedIPin videoRendererFilterInputPin = firstPin(videoRendererFilter.object(), PTYPE_INPUT, CTYPE_CONNECTED);
	const ScopedIPin outputPin = connectedPin(videoRendererFilterInputPin.object());

	if (videoRendererFilterInputPin.object() == nullptr || outputPin.object() == nullptr)
	{
	...
	if (S_OK != filterGraph_.object()->RemoveFilter(videoRendererFilter.object()))
	{
	...
	if (!insertFrameSampleSinkFilter(outputPin.object(), preferredFrameType_.pixelFormat()))
	{
...
bool DSFrameMedium::insertFrameSampleSinkFilter(IPin* outputPin, FrameType::PixelFormat /*pixelFormat*/)
{
	...
	if (sampleSinkFilter_.object() != nullptr)
	{
		return true;
	}
	...
	sampleSinkFilter_.object()->AddRef();
	...
	if (S_OK != filterGraph_.object()->AddFilter(sampleSinkFilter_.object(), L"Frame sample sink filter"))
	{
	...
	if (preferredFrameType_.pixelFormat() != FrameType::FORMAT_UNDEFINED)
	{
		...
		sampleSinkFilter_.object()->specifyMediaType(requestedMediaType.type());
	}
	...
	if (!connectFilter(outputPin, sampleSinkFilter_.object()))
	{
		if (!connectFilter(outputPin, sampleSinkFilter_.object()))
		{
	...
	if (sampleSinkFilter_.object()->establishedMediaType(establishedMediaType) == false)
	{
...
void DSFrameMedium::releaseFrameSampleSinkFilter()
{
	if (sampleSinkFilter_.object() != nullptr)
	{
		...
		filterGraph_.object()->RemoveFilter(sampleSinkFilter_.object());
	}

...
Library::Definitions AVFLibrary::selectableMedia(const Medium::Type type) const
{
	...
	if (type == Medium::LIVE_VIDEO)
	{
		...
		definitions.resize(devices.size());

		for (size_t n = 0; n < devices.size(); ++n)
		{
			definitions[n].url = String::toAString(devices[n].friendlyName());
			definitions[n].type = Medium::LIVE_VIDEO;
			definitions[n].library = name();
		}
	}

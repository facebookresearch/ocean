...
#ifndef META_OCEAN_MEDIA_AVF_VIDEO_ENCODER_H
#define META_OCEAN_MEDIA_AVF_VIDEO_ENCODER_H
 ...
 * The encoder uses Apple's VideoToolbox framework (VTCompressionSession) for hardware-accelerated encoding.
 *
 * Usage:
 * @code
...
class VideoEncoder
{
	...
	protected:
		 ...
		 */
		static inline void releaseVTCompressionSession(VTCompressionSessionRef session);

		/**
		* Definition of a scoped object holding a VTCompressionSessionRef object.
		* The wrapped VTCompressionSessionRef object will be invalidated and released automatically once the scoped object does not exist anymore.
		*/
		using ScopedVTCompressionSessionRef = ScopedObjectCompileTimeT<VTCompressionSessionRef, VTCompressionSessionRef, void, releaseVTCompressionSession>;
	...
	protected:
		 ...
		 * @param outputCallbackRefCon Reference to this encoder
		 * @param sourceFrameRefCon Reference containing the presentation time
		 * @param status The status of the compression operation
		 * @param infoFlags Information flags
		 * @param sampleBuffer The encoded sample buffer, may be nullptr on error
		 */
...
inline void VideoEncoder::releaseVTCompressionSession(VTCompressionSessionRef session)
{
	if (session != nullptr)
	{
...
inline bool VideoEncoder::isInitialized() const
{
	...
	return compressionSession_.isValid();
}
...
#endif // META_OCEAN_MEDIA_AVF_VIDEO_ENCODER_H

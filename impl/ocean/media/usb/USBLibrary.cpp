...
std::string USBLibrary::resolveUrl(const std::string& url, std::string& deviceName) const
{
	...
	{
		Log::debug() << n << ": " << availableDevices_[n].url << ", " << availableDevices_[n].uniqueIdentifier_;
	}
#endif

	for (size_t n = 0; n < availableDevices_.size(); ++n)
	{
		if (index == n || url == availableDevices_[n].url || url == availableDevices_[n].uniqueIdentifier_)
		{
			deviceName = availableDevices_[n].uniqueIdentifier_;

	...
	LiveVideo(url)
{
	...
	if (jniEnv != nullptr)
	{
		if (System::USB::Android::OceanUSBManager::get().requestPermission(jniEnv, deviceName, std::bind(&USBLiveVideo::onPermission, this, std::placeholders::_1, std::placeholders::_2)))
		{
...
void USBLiveVideo::onPermission(const std::string& deviceName, const bool permissionGranted)
{
	...
	hasPermission_ = permissionGranted;
}

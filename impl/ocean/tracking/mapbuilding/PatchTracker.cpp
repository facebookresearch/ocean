...
bool PatchTracker::trackVRSFile(const std::string& vrsFile, Database& database, SharedAnyCamera& anyCamera, std::shared_ptr<UnifiedDescriptorMap>& descriptorMap, const std::shared_ptr<UnifiedDescriptorExtractor>& unifiedDescriptorExtractor)
{
	...
	while (true)
	{
		...
		yCurrentFramePyramid->replace8BitPerChannel(yFrame.constdata<uint8_t>(), yFrame.width(), yFrame.height(), yFrame.channels(), yFrame.pixelOrigin(), pyramidLayers, yFrame.paddingElements(), WorkerPool::get().scopedWorker()(), yFrame.timestamp());
...
void PatchTracker::trackUnlocatedPoints(const Index32 frameIndex, const SharedFramePyramid& yCurrentFramePyramid, Geometry::SpatialDistribution::OccupancyArray& occupancyArray, Worker* worker)
{
	...
	if (yPreviousFramePyramid && !previousImagePoints.empty())
	{
		...
		{
			// we need to remove all object points from the database which we lost in this tracking iterationdatabase

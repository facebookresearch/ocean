	...
	private Thread
{
	...
	private:
		...
		/**
		 * Determines the camera pose if no reliable knowledge is given from any previous camera frame (a very rough pose may be provided from a rough pose estimation unit).
		 * @param frame The frame which will be used for pose determination, must be valid
		 * @param pinholeCamera The pinhole camera profile defining the projection, must be valid
		 * @param pose Resulting pose, if succeeded
		 * @param roughPose Optional rough (may be very rough) pose to be used for initialization
		 * @param worker Optional worker object to distribute the computation
		 * @return True, if succeeded
		 */
		bool determinePoseWithoutKnowledgeBlob(const Frame& frame, const PinholeCamera& pinholeCamera, HomogenousMatrix4& pose, const HomogenousMatrix4* roughPose = nullptr, Worker* worker = nullptr);

...
void RMVFeatureTracker6DOF::setFeatureMap(const RMVFeatureMap& featureMap, RandomGenerator& randomGenerator, const bool autoUpdateMaxPositionOffset)
{
	...
	if (initializationBoundingBox.isValid() && !initializationObjectPoints.empty() && trackerMaximalPoseProjectionFeatureNumber_ != 0)
	{
		...
		{
			trackerPoseProjectionSet_.addPoseProjection(Tracking::PoseProjection(randomPose, featureMap.initializationCamera(), initializationObjectPoints.data(), min(initializationObjectPoints.size(), trackerMaximalPoseProjectionFeatureNumber_)));
		}
	}
}
...
bool RMVFeatureTracker6DOF::determinePoses(const Frame& frame, const PinholeCamera& camera, const bool frameIsUndistorted, TransformationSamples& transformations, const Quaternion& /*absoluteOrientation*/, Worker* worker)
{
	ocean_assert(frame.isValid() && camera.isValid());
	...
	if (internDeterminePose(frame, AnyCameraPinhole(camera), frameIsUndistorted, world_T_camera, worker))
	{
...
bool RMVFeatureTracker6DOF::internDeterminePose(const Frame& frame, const AnyCamera& camera, const bool frameIsUndistorted, HomogenousMatrix4& world_T_camera, Worker* worker)
{
	ocean_assert(frame && camera.isValid());
	ocean_assert(frame.width() == camera.width() && frame.height() == camera.height());
	...
	if (motionModel_)
	{
		...
		const Box2 projectedBox = RMVFeatureMap::projectToImage(camera, world_T_roughCamera, trackerFeatureMap_.boundingBox());

		ocean_assert(imagePoints.empty());
		imagePoints = detectFeatures(yFrame, frameIsUndistorted, projectedBox, worker);
		...
		if (trackerFeatureMap_.recentStrongObjectPointIndices().size() >= 5 && trackerFeatureMap_.recentStrongObjectPointIndices().size() + trackerFeatureMap_.recentSemiStrongObjectPointIndices().size() >= 20)
		{
			...
			if (determinePoseWithStrongPreviousCorrespondences(world_T_roughCamera, camera, imagePoints, world_T_camera, worker))
			{
				return true;
			}
		}

		if (determinePoseWithAnyPreviousCorrespondences(world_T_roughCamera, camera, imagePoints, world_T_camera, worker))
		{
			return true;
		}
		...
		if (determinePoseWithRoughPose(world_T_roughCamera, camera, imagePoints, world_T_camera, worker))
		{
			return true;
		}
	}
	...
	if (RMVFeatureDetector::needPyramidInitialization(trackerFeatureMap_.initializationDetectorType()))
	{
		...
		imagePoints = detectFeatures(yFrame, frameIsUndistorted, RMVFeatureMap::projectToImage(camera, world_T_roughCamera, trackerFeatureMap_.boundingBox()), worker);
	...
	else
	{
		...
		if (!determinePoseWithoutKnowledgeDefault(camera, imagePoints, world_T_camera, worker))
		{
			return false;
		}
	}
...
bool RMVFeatureTracker6DOF::determinePoseWithoutKnowledgePyramid(const Frame& frame, const AnyCamera& camera, HomogenousMatrix4& world_T_camera, Worker* worker)
{
	...
	ocean_assert(adjustedFrame.width() == camera.width() && adjustedFrame.height() == camera.height());
...

	{
		...
		roughFlippedCameras_T_world = Camera::standard2InvertedFlipped(world_T_roughCameras.data(), world_T_roughCameras.size());
	}

	const Scalar resolutionFactor = cameraResolutionFactor(camera);
	...
	if (RandomModelVariation::optimizedPoseFromPointCloudsWithSeveralInitialPosesIF<true>(roughFlippedCameras_T_world.data(), roughFlippedCameras_T_world.size(), camera, objectPoints.data(), objectPoints.size(), imagePoints.data(), numberImagePoints, numberImagePoints * 65u / 100u, trackerRandomGenerator_, resultingFlippedCamera_T_world, Geometry::Error::ED_APPROXIMATED, maxPixelError0, Vector3(Scalar(0.3), Scalar(0.3), Scalar(0.3)), Numeric::deg2rad(30), Scalar(0.01), nullptr, nullptr, worker))
	{
		HomogenousMatrix4 initialFlippedCamera_T_world = resultingFlippedCamera_T_world;
		if (RandomModelVariation::optimizedPoseFromPointCloudsWithOneInitialPoseIF<true>(initialFlippedCamera_T_world, camera, objectPoints.data(), objectPoints.size(), imagePoints.data(), numberImagePoints, numberImagePoints * 55u / 100u, trackerRandomGenerator_, resultingFlippedCamera_T_world, Geometry::Error::ED_APPROXIMATED, maxPixelError1, Vector3(Scalar(0.3), Scalar(0.3), Scalar(0.3)), Numeric::deg2rad(30), Scalar(0.01), nullptr, nullptr, nullptr, worker))
		{
			initialFlippedCamera_T_world = resultingFlippedCamera_T_world;

			if (RandomModelVariation::optimizedPoseFromPointCloudsWithOneInitialPoseIF<true>(initialFlippedCamera_T_world, camera, objectPoints.data(), objectPoints.size(), imagePoints.data(), numberImagePoints, numberImagePoints * 55u / 100u, trackerRandomGenerator_, resultingFlippedCamera_T_world, Geometry::Error::ED_AMBIGUOUS, maxPixelError2, Vector3(Scalar(0.25), Scalar(0.25), Scalar(0.25)), Numeric::deg2rad(25), Scalar(0.01), nullptr, nullptr, nullptr, worker))
			{
				initialFlippedCamera_T_world = resultingFlippedCamera_T_world;

				if (RandomModelVariation::optimizedPoseFromPointCloudsWithOneInitialPoseIF<true>(initialFlippedCamera_T_world, camera, objectPoints.data(), objectPoints.size(), imagePoints.data(), numberImagePoints, numberImagePoints * 50u / 100u, trackerRandomGenerator_, resultingFlippedCamera_T_world, Geometry::Error::ED_UNIQUE, maxPixelError3, Vector3(Scalar(0.2), Scalar(0.2), Scalar(0.2)), Numeric::deg2rad(20), Scalar(0.05), nullptr, nullptr, nullptr, worker))
				{
					world_T_camera = Camera::invertedFlipped2Standard(resultingFlippedCamera_T_world);
...
bool RMVFeatureTracker6DOF::determinePoseWithoutKnowledgeDefault(const AnyCamera& camera, const Vectors2& imagePoints, HomogenousMatrix4& world_T_camera, Worker* worker)
{
	const Vectors3& objectPoints = trackerFeatureMap_.initializationObjectPoints();
	...
	ocean_assert(trackerPoseProjectionSet_.width() == camera.width() && trackerPoseProjectionSet_.height() == camera.height());
	...
	HomogenousMatrix4 resultingFlippedCamera_T_world;
	if (RandomModelVariation::optimizedPoseFromPointCloudsWithSeveralInitialPosesIF<true>(roughFlippedCameras_T_world.data(), roughFlippedCameras_T_world.size(), camera, objectPoints.data(), objectPoints.size(), imagePoints.data(), numberImagePoints, numberImagePoints * 60u / 100u, trackerRandomGenerator_, resultingFlippedCamera_T_world, Geometry::Error::ED_APPROXIMATED, maxPixelError0, Vector3(Scalar(0.1), Scalar(0.1), Scalar(0.1)), Numeric::deg2rad(15), timeout, &resultingError, nullptr, worker))
	{
		...
		if (RandomModelVariation::optimizedPoseFromPointCloudsWithOneInitialPoseIF<true>(roughFlippedCamera_T_world, camera, objectPoints.data(), objectPoints.size(), imagePoints.data(), numberImagePoints, numberImagePoints * 60u / 100u, trackerRandomGenerator_, resultingFlippedCamera_T_world, Geometry::Error::ED_APPROXIMATED, maxPixelError1, Vector3(Scalar(0.05), Scalar(0.05), Scalar(0.05)), Numeric::deg2rad(12.5), timeout, nullptr, nullptr, nullptr, worker))
		{
			...
			if (RandomModelVariation::optimizedPoseFromPointCloudsWithOneInitialPoseIF<true>(roughFlippedCamera_T_world, camera, objectPoints.data(), objectPoints.size(), imagePoints.data(), numberImagePoints, numberImagePoints * 60u / 100u, trackerRandomGenerator_, resultingFlippedCamera_T_world, Geometry::Error::ED_AMBIGUOUS, maxPixelError2, Vector3(Scalar(0.02), Scalar(0.02), Scalar(0.02)), Numeric::deg2rad(10), timeout, nullptr, &correspondences, nullptr, worker))
			{
				...
				if (Geometry::NonLinearOptimizationPose::optimizePoseIF(camera, roughFlippedCamera_T_world, ConstArrayAccessor<Vector3>(validObjectPoints), ConstArrayAccessor<Vector2>(validImagePoints), resultingFlippedCamera_T_world, 30u, Geometry::Estimator::ET_HUBER, Scalar(0.001), Scalar(10), nullptr, nullptr))
				{
					...
					if (refinePoseIF(roughFlippedCamera_T_world, camera, imagePoints, trackerFeatureMap_.objectPoints(), resultingFlippedCamera_T_world, 200, 400, Scalar(10) * resolutionFactor, Scalar(30 * 30), Numeric::maxValue(), worker) < 5u)
					{
						return false;
					}
					...
					if (refinePoseIF(roughFlippedCamera_T_world, camera, imagePoints, trackerFeatureMap_.objectPoints(), resultingFlippedCamera_T_world, 400, 800, Scalar(4) * resolutionFactor, Scalar(2 * 2), Numeric::maxValue(), worker) < 5u)
					{
						return false;
					}
...
bool RMVFeatureTracker6DOF::determinePoseWithStrongPreviousCorrespondences(const HomogenousMatrix4& world_T_roughCamera, const AnyCamera& camera, Vectors2& imagePoints, HomogenousMatrix4& world_T_camera, Worker* worker)
{
	ocean_assert(world_T_roughCamera.isValid() && camera.isValid());
	...
	const HomogenousMatrix4 roughFlippedCamera_T_world(Camera::standard2InvertedFlipped(world_T_roughCamera));

	HomogenousMatrix4 resultingFlippedCamera_T_world;
	if (!refinePoseWithStrongPreviousCorrespondencesIF(roughFlippedCamera_T_world, camera, imagePoints, resultingFlippedCamera_T_world))
	{
		return false;
	}
	...
	const unsigned int validCorrespondencesFirst = refinePoseIF(flippedCamera_T_world, camera, imagePoints, trackerFeatureMap_.objectPoints(), resultingFlippedCamera_T_world, 200u, 400u, Scalar(10) * resolutionFactor, Scalar(30 * 30), Numeric::maxValue(), worker);
	...
	const unsigned int validCorrespondencesSecond = refinePoseIF(flippedCamera_T_world, camera, imagePoints, trackerFeatureMap_.objectPoints(), resultingFlippedCamera_T_world, 400u, 600u, Scalar(4) * resolutionFactor, Scalar(2 * 2), Numeric::maxValue(), worker);
	...
	{
		const unsigned int validCorrespondencesThird = refinePoseIF(flippedCamera_T_world, camera, imagePoints, trackerFeatureMap_.objectPoints(), resultingFlippedCamera_T_world, 800u, 1000u, Scalar(4) * resolutionFactor, Scalar(1.5 * 1.5), Numeric::maxValue(), worker);
	...
	startAsynchronousDataProcessingIF(resultingFlippedCamera_T_world, camera, std::move(imagePoints));
...
bool RMVFeatureTracker6DOF::determinePoseWithAnyPreviousCorrespondences(const HomogenousMatrix4& world_T_roughCamera, const AnyCamera& camera, Vectors2& imagePoints, HomogenousMatrix4& world_T_camera, Worker* worker)
{
	ocean_assert(world_T_roughCamera.isValid() && camera.isValid());
	...
	camera.projectToImageIF(roughFlippedCamera_T_world, previousUsedObjectPoints.data(), previousUsedObjectPoints.size(), previousUsedProjectedObjectPoints.data());
	...
	const unsigned int bins = std::max(1u, std::min(camera.width(), camera.height()) / 10u); // each bin will have approx. 10^2 pixels

	const Geometry::SpatialDistribution::DistributionArray distributionImagePoints(Geometry::SpatialDistribution::distributeToArray(imagePoints.data(), numberImagePoints, Scalar(0), Scalar(0), Scalar(camera.width()), Scalar(camera.height()), bins, bins));
	...
	do
	{
		...
		{
			// we select one object previously used object points which hasn't been selected in this iteration yet
			...
			const Indices32& neighbors = neighborGroups[randomUsedObjectPointIndex];
			...
			if (!neighbors.empty())
			{
				...
				const unsigned int candidateNeighborIndex = RandomI::random(trackerRandomGenerator_, (unsigned int)(neighbors.size()) - 1u);
				ocean_assert(candidateNeighborIndex < neighbors.size());

				ocean_assert(neighbors[candidateNeighborIndex] < imagePoints.size());
				const Vector2& iterationImagePoint = imagePoints[neighbors[candidateNeighborIndex]];
				ocean_assert_and_suppress_unused(iterationProjectedObjectPoint == camera.projectToImageIF(roughFlippedCamera_T_world, iterationObjectPoint), iterationProjectedObjectPoint);
		...
		if (Geometry::NonLinearOptimizationPose::optimizePoseIF(camera, roughFlippedCamera_T_world, ConstArrayAccessor<Vector3>(iterationObjectPoints), ConstArrayAccessor<Vector2>(iterationImagePoints), optimizedRoughFlippedCamera_T_world, 20u, Geometry::Estimator::ET_SQUARE, Scalar(0.001), Scalar(10)))
		{
			// now we count the number of perfect/good matches

			Vectors2 iterationProjectedObjectPoints(previousUsedObjectPoints.size());
			camera.projectToImageIF(optimizedRoughFlippedCamera_T_world, previousUsedObjectPoints.data(), previousUsedObjectPoints.size(), iterationProjectedObjectPoints.data());
...
#ifdef OCEAN_DEBUG
	{
		...
		const bool debugResult = Geometry::Error::determinePoseErrorIF<ConstTemplateArraySubsetAccessor<Vector3, Index32>, ConstTemplateArraySubsetAccessor<Vector2, Index32>, true>(bestFlippedCamera_T_world, camera, ConstTemplateArraySubsetAccessor<Vector3, Index32>(previousUsedObjectPoints, bestObjectPointIndices), ConstTemplateArraySubsetAccessor<Vector2, Index32>(imagePoints.data(), bestImagePointIndices), sqrAverageError, sqrMinimalError, sqrMaximalError);
	...
	const unsigned int validCorrespondencesFirst = refinePoseIF(bestFlippedCamera_T_world, camera, imagePoints, trackerFeatureMap_.recentUsedObjectPoints(), optimizedFlippedCamera_T_world, 150u, 100u, Scalar(5), Scalar(3 * 3), Scalar(1.5 * 1.5), worker);
	...
	const unsigned int validCorrespondencesSecond = refinePoseIF(bestFlippedCamera_T_world, camera, imagePoints, trackerFeatureMap_.objectPoints(), optimizedFlippedCamera_T_world, 400u, 200u, Scalar(5), Scalar(3 * 3), Scalar(1.5 * 1.5), worker);
	...
	startAsynchronousDataProcessingIF(optimizedFlippedCamera_T_world, camera, std::move(imagePoints));

	world_T_camera = Camera::invertedFlipped2Standard(optimizedFlippedCamera_T_world);

	return true;
}

bool RMVFeatureTracker6DOF::determinePoseWithRoughPose(const HomogenousMatrix4& world_T_roughCamera, const AnyCamera& camera, Vectors2& imagePoints, HomogenousMatrix4& world_T_camera, Worker* worker)
{
	ocean_assert(world_T_roughCamera.isValid() && camera.isValid());
	...
	if (numberImagePoints < numberObjectPoints)
	{
		if (!RandomModelVariation::optimizedPoseFromPointCloudsWithOneInitialPoseIF<true>(roughFlippedCamera_T_world, camera, objectPoints.data(), numberObjectPoints, imagePoints.data(), numberImagePoints, numberImagePoints * 60u / 100u, trackerRandomGenerator_, resultingFlippedCamera_T_world, Geometry::Error::ED_APPROXIMATED, maxPixelError0, Vector3(maximalTranslationSmall, maximalTranslationSmall, maximalTranslationSmall), Numeric::deg2rad(5), timeout, nullptr, &correspondences, nullptr, worker))
		{
			...
			if (!RandomModelVariation::optimizedPoseFromPointCloudsWithOneInitialPoseIF<true>(roughFlippedCamera_T_world, camera, objectPoints.data(), numberObjectPoints, imagePoints.data(), numberImagePoints, numberImagePoints * 60u / 100u, trackerRandomGenerator_, resultingFlippedCamera_T_world, Geometry::Error::ED_APPROXIMATED, maxPixelError0, Vector3(maximalTranslationSmall, maximalTranslationSmall, maximalTranslationSmall) * Scalar(2), Numeric::deg2rad(10), timeout * 2.0, nullptr, &correspondences, nullptr, worker))
			{
				return false;
			}
		}
	...
	{
		if (!RandomModelVariation::optimizedPoseFromPointCloudsWithOneInitialPoseIF<false>(roughFlippedCamera_T_world, camera, objectPoints.data(), numberObjectPoints, imagePoints.data(), numberImagePoints, numberObjectPoints * 60u / 100u, trackerRandomGenerator_, resultingFlippedCamera_T_world, Geometry::Error::ED_APPROXIMATED, maxPixelError0, Vector3(maximalTranslationSmall, maximalTranslationSmall, maximalTranslationSmall), Numeric::deg2rad(5), timeout, nullptr, &correspondences, nullptr, worker))
		{
			ocean_assert(correspondences.empty());

			// we give it a second chance with slightly simpler conditions
			if (!RandomModelVariation::optimizedPoseFromPointCloudsWithOneInitialPoseIF<false>(roughFlippedCamera_T_world, camera, objectPoints.data(), numberObjectPoints, imagePoints.data(), numberImagePoints, numberObjectPoints * 60u / 100u, trackerRandomGenerator_, resultingFlippedCamera_T_world, Geometry::Error::ED_APPROXIMATED, maxPixelError0, Vector3(maximalTranslationSmall, maximalTranslationSmall, maximalTranslationSmall) * Scalar(2), Numeric::deg2rad(10), timeout * 2.0, nullptr, &correspondences, nullptr, worker))
			{
				return false;
			}
		}
	...
	if (!Geometry::NonLinearOptimizationPose::optimizePoseIF(camera, initialFlippedCamera_T_world, ConstArrayAccessor<Vector3>(validObjectPoints), ConstArrayAccessor<Vector2>(validImagePoints), resultingFlippedCamera_T_world, 30u, Geometry::Estimator::ET_HUBER, Scalar(0.001), Scalar(10), nullptr, nullptr))
	{
		return false;
	}
	...
	if (!Geometry::NonLinearOptimizationPose::optimizePoseIF(camera, flippedCamera_T_world, ConstArrayAccessor<Vector3>(validObjectPoints), ConstArrayAccessor<Vector2>(validImagePoints), resultingFlippedCamera_T_world, 30u, Geometry::Estimator::ET_TUKEY, Scalar(0.001), Scalar(10), nullptr, nullptr))
	{
		return false;
	}
	...
	if (refinePoseIF(flippedCamera_T_world, camera, imagePoints, trackerFeatureMap_.objectPoints(), resultingFlippedCamera_T_world, 400u, 800u, Scalar(4) * resolutionFactor, Scalar(2 * 2), Numeric::maxValue(), worker) < 10u)
	{
		return false;
	}
...
bool RMVFeatureTracker6DOF::refinePoseWithStrongPreviousCorrespondencesIF(const HomogenousMatrix4& roughFlippedCamera_T_world, const AnyCamera& camera, const Vectors2& imagePoints, HomogenousMatrix4& flippedCamera_T_world)
{
	ocean_assert(roughFlippedCamera_T_world.isValid() && camera.isValid() && imagePoints.size() >= 10);
	...
	addUniqueCorrespondencesIF(roughFlippedCamera_T_world, camera, imagePoints.data(), min(imagePoints.size(), size_t(200)), ConstArraySubsetAccessor<Vector3, Index32>(trackerFeatureMap_.objectPoints(), trackerFeatureMap_.recentStrongObjectPointIndices()), trackerStrongCorrespondencesEmptyAreaRadius_ * resolutionFactor, Scalar(4 * 4), candidateObjectPoints, candidateImagePoints);
	...
	if (candidateObjectPoints.size() <= 10 && !trackerFeatureMap_.recentSemiStrongObjectPointIndices().empty())
	{
		...
		addUniqueCorrespondencesIF(roughFlippedCamera_T_world, camera, imagePoints.data(), min(imagePoints.size(), size_t(300)), ConstArraySubsetAccessor<Vector3, Index32>(trackerFeatureMap_.objectPoints(), trackerFeatureMap_.recentSemiStrongObjectPointIndices()), trackerSemiStrongCorrespondencesEmptyAreaRadius_ * resolutionFactor, Scalar(2.5 * 2.5), candidateObjectPoints, candidateImagePoints);
		ocean_assert(candidateObjectPoints.size() == candidateImagePoints.size());
	}
	...
	HomogenousMatrix4 optimizedFlippedCamera_T_world;
	if (!Geometry::NonLinearOptimizationPose::optimizePoseIF(camera, roughFlippedCamera_T_world, ConstArrayAccessor<Vector3>(candidateObjectPoints), ConstArrayAccessor<Vector2>(candidateImagePoints), optimizedFlippedCamera_T_world, 10u, Geometry::Estimator::ET_HUBER))
	{
		return false;
	}
	...
	if (!Geometry::NonLinearOptimizationPose::optimizePoseIF(camera, optimizedFlippedCamera_T_world, ConstArrayAccessor<Vector3>(candidateObjectPoints), ConstArrayAccessor<Vector2>(candidateImagePoints), flippedCamera_T_world, 5u, Geometry::Estimator::ET_TUKEY))
	{
		return false;
	}
...
void RMVFeatureTracker6DOF::startAsynchronousDataProcessingIF(const HomogenousMatrix4& flippedCamera_T_world, const AnyCamera& camera, Vectors2&& imagePoints)
{
	const ScopedLock scopedLock(trackerAsynchronousDataProcessingLock_);
	...
	ocean_assert(!trackerAsynchronousDataProcessingCamera_);
	trackerAsynchronousDataProcessingCamera_ = camera.clone();
...
void RMVFeatureTracker6DOF::threadRun()
{
	while (!shouldThreadStop())
	{
		...
		trackerAsynchronousDataProcessingCamera_ = nullptr;
		trackerAsynchronousDataProcessingImagePoints_.clear();
	}
}

bool RMVFeatureTracker6DOF::determineUsedFeaturesIF(const HomogenousMatrix4& fineFlippedCamera_T_world, const AnyCamera& camera, const Vectors2& imagePoints, const size_t minimalStrongObjectPoints, Indices32& strongObjectPointIndices, Indices32& moderateObjectPointIndices, Indices32& usedObjectPointIndices)
{
	ocean_assert(fineFlippedCamera_T_world.isValid() && camera.isValid());
	...
	camera.projectToImageIF(fineFlippedCamera_T_world, trackerFeatureMap_.objectPoints().data(), projectedObjectPoints.size(), projectedObjectPoints.data());
	...
	const unsigned int bins = std::max(1u, std::min(camera.width(), camera.height()) / 20u); // each bin will have approx. 20^2 pixels

	const Geometry::SpatialDistribution::DistributionArray distributionProjectedObjectPoints(Geometry::SpatialDistribution::distributeToArray(projectedObjectPoints.data(), projectedObjectPoints.size(), Scalar(0), Scalar(0), Scalar(camera.width()), Scalar(camera.height()), bins, bins));
	const Geometry::SpatialDistribution::DistributionArray distributionImagePoints(Geometry::SpatialDistribution::distributeToArray(imagePoints.data(), maximalImagePoints, Scalar(0), Scalar(0), Scalar(camera.width()), Scalar(camera.height()), bins, bins));
...
{
	ocean_assert(roughFlippedCamera_T_world.isValid() && camera.isValid());
	...
	{
		projectedObjectPoints.push_back(camera.projectToImageIF(roughFlippedCamera_T_world, objectPointAccessor[n]));
	}
	...
	for (const PointCorrespondences::RedundantCorrespondence& redundantCorrespondence : redundantCorrespondences)
	{
		if (redundantCorrespondence.isUnique(uniquenessSqrFactor))
		{
			...
			if (Camera::isObjectPointInFrontIF(roughFlippedCamera_T_world, objectPoint))
			{
...
unsigned int RMVFeatureTracker6DOF::refinePoseIF(const HomogenousMatrix4& roughFlippedCamera_T_world, const AnyCamera& camera, const Vectors2& imagePoints, const Vectors3& objectPoints, HomogenousMatrix4& flippedCamera_T_world, const unsigned int useNumberImagePoints, const unsigned int useNumberObjectPoints, const Scalar searchWindow, const Scalar uniquenessSqrFactor, const Scalar maxSqrDistance, Worker* /*worker*/)
{
	ocean_assert(roughFlippedCamera_T_world.isValid() && camera.isValid());
	...
	camera.projectToImageIF(roughFlippedCamera_T_world, objectPoints.data(), numberObjectPoints, projectedObjectPoints.data());

	// we determine possible correspondence candidates (and also store a second candidate for each point)
	const PointCorrespondences::RedundantCorrespondences redundantCorrespondences(PointCorrespondences::determineNearestCandidates(imagePoints.data(), numberImagePoints, projectedObjectPoints.data(), numberObjectPoints, camera.width(), camera.height(), searchWindow));
	...
	if (!Geometry::NonLinearOptimizationPose::optimizePoseIF(camera, roughFlippedCamera_T_world, ConstArrayAccessor<Vector3>(correspondenceObjectPoints), ConstArrayAccessor<Vector2>(correspondenceImagePoints), flippedCamera_T_world, 5u, Geometry::Estimator::ET_HUBER, Scalar(0.001), Scalar(10)))
	{
		return 0u;
	}
	...
	if (!Geometry::NonLinearOptimizationPose::optimizePoseIF(camera, optimizedFlippedCamera_T_world, ConstArrayAccessor<Vector3>(correspondenceObjectPoints), ConstArrayAccessor<Vector2>(correspondenceImagePoints), flippedCamera_T_world, 5u, Geometry::Estimator::ET_TUKEY, Scalar(0.001), Scalar(10)))
	{
		return 0u;
	}

...
bool RMVFeatureTracker6DOF::internDeterminePose(const Frame& frame, const PinholeCamera& pinholeCamera, const bool frameIsUndistorted, HomogenousMatrix4& pose, Worker* worker)
{
	...
	if (motionModel_)
	{
		...
		{
			// in the case we have enough strong feature points from the previous frame we first try to determine the camera pose based ont these feature points as this would be very simple and would provide a reliable tracking result
	...
	if (trackerFeatureMap.initializationDetectorType() == RMVFeatureDetector::DT_BLOB_FEATURE)
	{
		HomogenousMatrix4 resultingRoughPose;
		if (!determinePoseWithoutKnowledgeBlob(yFrame, pinholeCamera, resultingRoughPose, initialRoughPose, worker))
		{
			return false;
		}

		pose = resultingRoughPose;
...
bool RMVFeatureTracker6DOF::determinePoseWithoutKnowledgeBlob(const Frame& frame, const PinholeCamera& pinholeCamera, HomogenousMatrix4& pose, const HomogenousMatrix4* optionalRoughPose, Worker* worker)
{
	ocean_assert(frame && pinholeCamera);

	ocean_assert(!trackerFeatureMap.initializationObjectPoints().empty());
	const Vectors3& objectPoints = trackerFeatureMap.initializationObjectPoints();
	const unsigned int numberObjectPoints = (unsigned int)objectPoints.size();

	unsigned int numberImagePoints = numberObjectPoints * 70u / 100u;
	const Vectors2 imagePoints(RMVFeatureDetector::detectFeatures(frame, RMVFeatureDetector::DT_BLOB_FEATURE, Scalar(40), numberImagePoints, false, worker));
	numberImagePoints = (unsigned int)imagePoints.size();

	if (numberImagePoints < 10u)
	{
		return false;
	}

#ifdef OCEAN_HARDWARE_REDUCED_PERFORMANCE
	unsigned int numberInitialPoses = 4u;
	Scalar timeout = Scalar(0.10);
#else
	unsigned int numberInitialPoses = 8u;
	Scalar timeout = Scalar(0.15);
#endif
	...
	HomogenousMatrices4 initialPosesIF;

	if (!optionalRoughPose)
	{
		HomogenousMatrices4 initialPoses(numberInitialPoses);

		numberInitialPoses = trackerPoseProjectionSet.findPosesWithMinimalError<Geometry::Estimator::ET_TUKEY>(imagePoints.data(), imagePoints.size(), (unsigned int)imagePoints.size() * 55u / 100u, Geometry::Error::ED_APPROXIMATED, initialPoses.size(), initialPoses.data(), nullptr, worker);

		initialPosesIF = PinholeCamera::standard2InvertedFlipped(initialPoses.data(), initialPoses.size());
	}

	const Scalar resolutionFactor = cameraResolutionFactor(pinholeCamera);

	const Scalar maxPixelError0 = Numeric::sqr(Scalar(6.5) * resolutionFactor);
	const Scalar maxPixelError1 = Numeric::sqr(Scalar(5.8) * resolutionFactor);
	const Scalar maxPixelError2 = Numeric::sqr(Scalar(3.2) * resolutionFactor);

	Scalar resultingSqrError0 = -1;
	Scalar resultingSqrError1 = -1;
	Scalar resultingSqrError2 = -1;

	HomogenousMatrix4 resultingPoseIF;
	if (initialPosesIF.size() > 1)
	{
		if (!RandomModelVariation::optimizedPoseFromPointCloudsWithSeveralInitialPosesIF<true>(initialPosesIF.data(), initialPosesIF.size(), pinholeCamera, objectPoints.data(), objectPoints.size(), imagePoints.data(), numberImagePoints, numberImagePoints * 50u / 100u, trackerRandomGenerator, resultingPoseIF, Geometry::Error::ED_APPROXIMATED, maxPixelError0, Vector3(Scalar(0.3), Scalar(0.3), Scalar(0.3)), Numeric::deg2rad(30), timeout, &resultingSqrError0, nullptr, worker))
		{
			return false;
		}
	}
	...
	{
		ocean_assert(initialPosesIF.empty() || optionalRoughPose);
		const HomogenousMatrix4 roughPoseIF = optionalRoughPose ? PinholeCamera::standard2InvertedFlipped(*optionalRoughPose) : initialPosesIF.front();

		if (!RandomModelVariation::optimizedPoseFromPointCloudsWithOneInitialPoseIF<true>(roughPoseIF, pinholeCamera, objectPoints.data(), objectPoints.size(), imagePoints.data(), numberImagePoints, numberImagePoints * 50u / 100u, trackerRandomGenerator, resultingPoseIF, Geometry::Error::ED_APPROXIMATED, maxPixelError0, Vector3(Scalar(0.3), Scalar(0.3), Scalar(0.3)), Numeric::deg2rad(30), timeout, &resultingSqrError1, nullptr, nullptr, worker))
		{
			return false;
		}
	}
	...
	if (RandomModelVariation::optimizedPoseFromPointCloudsWithOneInitialPoseIF<true>(initialPoseIF, pinholeCamera, objectPoints.data(), objectPoints.size(), imagePoints.data(), numberImagePoints, numberImagePoints * 60u / 100u, trackerRandomGenerator, resultingPoseIF, Geometry::Error::ED_APPROXIMATED, maxPixelError1, Vector3(Scalar(0.3), Scalar(0.3), Scalar(0.3)), Numeric::deg2rad(30), timeout, &resultingSqrError2, nullptr, nullptr, worker))
	{
		initialPoseIF = resultingPoseIF;

		IndexPairs32 correspondences;
		if (RandomModelVariation::optimizedPoseFromPointCloudsWithOneInitialPoseIF<true>(initialPoseIF, pinholeCamera, objectPoints.data(), objectPoints.size(), imagePoints.data(), numberImagePoints, numberImagePoints * 55u / 100u, trackerRandomGenerator, resultingPoseIF, Geometry::Error::ED_UNIQUE, maxPixelError2, Vector3(Scalar(0.1), Scalar(0.1), Scalar(0.1)), Numeric::deg2rad(10), timeout, nullptr, &correspondences, nullptr, worker) && !correspondences.empty())
		{
			initialPoseIF = resultingPoseIF;
			...
			for (IndexPairs32::const_iterator i = correspondences.begin(); i != correspondences.end(); ++i)
			{
				...
				ocean_assert(i->second < objectPoints.size());

				validImagePoints.push_back(imagePoints[i->first]);
				validObjectPoints.push_back(objectPoints[i->second]);
			}

			ocean_assert(!validObjectPoints.empty());
			...
			if (Geometry::NonLinearOptimizationPose::optimizePoseIF(pinholeCamera, initialPoseIF, ConstArrayAccessor<Vector3>(validObjectPoints), ConstArrayAccessor<Vector2>(validImagePoints), false, resultingPoseIF, 30u, Geometry::Estimator::ET_HUBER, Scalar(0.001), Scalar(10), nullptr, nullptr))
			{
				...
				if (Geometry::NonLinearOptimizationPose::optimizePoseIF(pinholeCamera, initialPoseIF, ConstArrayAccessor<Vector3>(validObjectPoints), ConstArrayAccessor<Vector2>(validImagePoints), false, resultingPoseIF, 30u, Geometry::Estimator::ET_TUKEY, Scalar(0.001), Scalar(10), nullptr, nullptr))
				{
	...
	Log::info() << resultingSqrError0 << ", " << resultingSqrError1 << ", " << resultingSqrError2;
...
bool RMVFeatureTracker6DOF::determinePoseWithoutKnowledgeDefault(const PinholeCamera& pinholeCamera, const Vectors2& imagePoints, HomogenousMatrix4& pose, Worker* worker)
{
	...
	{
		// second, we try to optimize the best resulting pose further (with slightly stonger conditions)
		...
		if (RandomModelVariation::optimizedPoseFromPointCloudsWithOneInitialPoseIF<true>(roughPoseIF, pinholeCamera, objectPoints.data(), objectPoints.size(), imagePoints.data(), numberImagePoints, numberImagePoints * 60u / 100u, trackerRandomGenerator, resultingPoseIF, Geometry::Error::ED_APPROXIMATED, maxPixelError1, Vector3(Scalar(0.05), Scalar(0.05), Scalar(0.05)), Numeric::deg2rad(12.5), timeout, nullptr, nullptr, nullptr, worker))
		{
			...
			if (RandomModelVariation::optimizedPoseFromPointCloudsWithOneInitialPoseIF<true>(roughPoseIF, pinholeCamera, objectPoints.data(), objectPoints.size(), imagePoints.data(), numberImagePoints, numberImagePoints * 60u / 100u, trackerRandomGenerator, resultingPoseIF, Geometry::Error::ED_AMBIGUOUS, maxPixelError2, Vector3(Scalar(0.02), Scalar(0.02), Scalar(0.02)), Numeric::deg2rad(10), timeout, nullptr, &correspondences, nullptr, worker))
			{
				...
				{
					// now we have a quite good pose based on the gathered points coorespondences
					// now we apply a last finetuning with more points as we now should be able to find significant more feature coorespondences
...
bool RMVFeatureTracker6DOF::determinePoseWithRoughPose(const HomogenousMatrix4& roughPose, const PinholeCamera& pinholeCamera, Vectors2& imagePoints, HomogenousMatrix4& pose, Worker* worker)
{
	...
	// finally we refine the current pose based on the entire set of image and object points, first with a small amout of point coorespondences and week uniqueness constraints

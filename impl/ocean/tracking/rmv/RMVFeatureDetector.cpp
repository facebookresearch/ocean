...
Vectors2 RMVFeatureDetector::detectFeatures(const Frame& frame, const DetectorType detectorType, const Scalar threshold, const bool frameIsUndistorted, Worker* worker)
{
	...
	switch (detectorType)
	{
		...
		case DT_BLOB_FEATURE:
		{
			CV::Detector::Blob::BlobFeatures blobFeatures;
			CV::Detector::Blob::BlobFeatureDetector::detectFeatures(frame, CV::Detector::Blob::BlobFeatureDetector::SAMPLING_NORMAL, threshold, frameIsUndistorted, blobFeatures, worker);
			std::sort(blobFeatures.begin(), blobFeatures.end());

			return CV::Detector::Blob::BlobFeature::features2imagePoints(blobFeatures);
		}
...
Vectors2 RMVFeatureDetector::detectFeatures(const Frame& frame, const Box2& boundingBox, const DetectorType detectorType, const Scalar threshold, const bool frameIsUndistorted, Worker* worker)
{
	...
	switch (detectorType)
	{
		...
		{
			CV::Detector::Blob::BlobFeatures blobFeatures;
			CV::Detector::Blob::BlobFeatureDetector::detectFeatures(frame, left, top, right - left, bottom - top, CV::Detector::Blob::BlobFeatureDetector::SAMPLING_NORMAL, threshold, frameIsUndistorted, blobFeatures, worker);
			std::sort(blobFeatures.begin(), blobFeatures.end());
...
Vectors2 RMVFeatureDetector::detectFeatures(const Frame& frame, const DetectorType detectorType, const Scalar approximatedThreshold, const size_t numberFeatures, const bool frameIsUndistorted, Worker* worker)
{
	...
	switch (detectorType)
	{
		...
		{
			CV::Detector::Blob::BlobFeatures blobFeatures;
			CV::Detector::Blob::BlobFeatureDetector::detectFeatures(frame, CV::Detector::Blob::BlobFeatureDetector::SAMPLING_NORMAL, approximatedThreshold, frameIsUndistorted, blobFeatures, worker);

			if (blobFeatures.size() < numberFeatures)
			{
				for (Scalar threshold = approximatedThreshold; threshold >= 5; threshold -= 5)
				{
					blobFeatures.clear();
					CV::Detector::Blob::BlobFeatureDetector::detectFeatures(frame, CV::Detector::Blob::BlobFeatureDetector::SAMPLING_NORMAL, threshold, frameIsUndistorted, blobFeatures, worker);
			...
			std::sort(blobFeatures.begin(), blobFeatures.end());
			if (blobFeatures.size() > numberFeatures)
				blobFeatures.resize(numberFeatures);
...
Vectors2 RMVFeatureDetector::detectFeatures(const Frame& frame, const Box2& boundingBox, const DetectorType detectorType, const Scalar approximatedThreshold, const size_t numberFeatures, const bool frameIsUndistorted, Worker* worker)
{
	...
	switch (detectorType)
	{
		...
		{
			CV::Detector::Blob::BlobFeatures blobFeatures;
			CV::Detector::Blob::BlobFeatureDetector::detectFeatures(frame, left, top, right - left, bottom - top, CV::Detector::Blob::BlobFeatureDetector::SAMPLING_NORMAL, approximatedThreshold, frameIsUndistorted, blobFeatures, worker);

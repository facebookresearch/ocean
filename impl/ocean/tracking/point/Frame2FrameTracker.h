...
class Frame2FrameTracker
{
	public:
		 ...
		 * @param world_T_previousCamera The known camera pose for the previous camera frame, with default camera pointing towards the negative z-axis with y-axis upwards, must be valid
		 ...
		 * @param previousObjectPoints The 3D object point locations corresponding to the image points in the previous frame, define in world, at least 3
		 ...
		 * @param world_T_currentCamera The resulting camera pose of the current frame
...
template <unsigned int tSize>
bool Frame2FrameTracker::determinePose(const AnyCamera& camera, const HomogenousMatrix4& world_T_previousCamera, const Frame& previousFrame, const Frame& currentFrame, const Vectors3& previousObjectPoints, const Vectors2& previousImagePoints, const unsigned int maximalOffset, HomogenousMatrix4& world_T_currentCamera, Indices32* validIndices, Worker* worker)
{
	ocean_assert(camera.isValid() && world_T_previousCamera.isValid());
	...
	if (!CV::Advanced::AdvancedMotionT<>::trackPointsBidirectionalSubPixelMirroredBorder<tSize>(previousFrame, currentFrame, maximalOffset, 2u, copyPreviousImagePoints, currentImagePoints, Scalar(0.9 * 0.9), CV::FramePyramid::DM_FILTER_14641, worker, &internalValidIndices))
	{
		return false;
	}
	...
	if (!Geometry::NonLinearOptimizationPose::optimizePose(camera, world_T_previousCamera, ConstArraySubsetAccessor<Vector3, Index32>(previousObjectPoints, internalValidIndices), ConstArraySubsetAccessor<Vector2, Index32>(currentImagePoints, internalValidIndices), world_T_currentCamera))
	{
		return false;
	}

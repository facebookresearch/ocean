...
#include "application/ocean/test/tracking/testtracking/testslam/TestSLAM.h"
...
#endif
{
	...
	commandArguments.registerParameter("functions", "f", "The optional subset of functions to test, e.g., \"localizedobjectpoint\"");
	...
	const std::string outputFilename = commandArguments.value<std::string>("output", std::string(), false);
	const std::string functionList = commandArguments.value<std::string>("functions", std::string(), false);
	...
	Log::info() << "Ocean Framework test for the Tracking SLAM library:";
	Log::info() << " ";
	...
	const uint64_t startVirtualMemory = System::Memory::processVirtualMemory();
	...
	try
	{
		if (Test::TestTracking::TestSLAM::testSLAM(testDuration, worker, functionList))
		{
			resultValue = 0;
		}
	}
	...
	const uint64_t stopVirtualMemory = System::Memory::processVirtualMemory();

...
#include "application/ocean/test/unifiedfeatures/testunifiedfeatures/TestUnifiedFeatures.h"
...
#endif
{
	...
	if (argc >= 2)
	{
		...
		{
			std::cout << "Ocean Framework test for the Unified Features library:" << std::endl << std::endl;
			std::cout << "Optional arguments: " << std::endl;
			std::cout << "Parameter 1: [log output file e.g., \"log.txt\" or default \"\"]" << std::endl;
			std::cout << "Parameter 2: [functions to test e.g., \"database\" or default \"\"]" << std::endl;
			...
			std::cout << "output.log \"database\"" << std::endl;
	...
	try
	{
		if (Test::TestUnifiedFeatures::test(testDuration, worker, functionList))
			resultValue = 0;
	}
	catch (...)
	{

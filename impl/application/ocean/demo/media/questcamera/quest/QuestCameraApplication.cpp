...
QuestCameraApplication::QuestCameraApplication(struct android_app* androidApp) :
...
XrSpace QuestCameraApplication::baseSpace() const
{
...
void QuestCameraApplication::onAndroidPermissionGranted(const std::string& permission)
{
	VRNativeApplicationAdvanced::onAndroidPermissionGranted(permission);

	if (permission == "horizonos.permission.HEADSET_CAMERA")
	{
...
void QuestCameraApplication::onFramebufferInitialized()
{
...
void QuestCameraApplication::onFramebufferReleasing()
{
...
void QuestCameraApplication::onReleaseResources()
{
...
void QuestCameraApplication::onPreRender(const XrTime& xrPredictedDisplayTime, const Timestamp& predictedDisplayTime)
{
	...
	switch (applicationState_)
	{
			...
			if (vrTableMenu_.isShown())
			{
				...
				if (vrTableMenu_.onPreRender(trackedController(), predictedDisplayTime, menuEntryUrl))
				{
					if (!menuEntryUrl.empty())
					{
						...
						{
							Log::info() << "Selected camera device: " << menuEntryUrl;
		...
		case AS_WAITING_FOR_SUPPORTED_STREAMS:
		{
			...
			else
			{
				...
				{
					Log::warning() << "Failed to enumerate available stream configurations";
					renderingTextCamera_->setText("Failed to enumerate available stream configurations");
				}
			}
...
void QuestCameraApplication::onButtonPressed(const OpenXR::TrackedController::ButtonType buttons, const Timestamp& timestamp)
{
...
void QuestCameraApplication::onButtonReleased(const OpenXR::TrackedController::ButtonType buttons, const Timestamp& timestamp)
{
...
void QuestCameraApplication::enumerateCameras()
{
	...
	std::string deviceGroupName = menuGroupEntries.empty() ? "No Quest camera found" : "Select a Quest camera";
...
bool QuestCameraApplication::enumerateStreamConfigurations()
{
	ocean_assert(liveVideo_);
	...
	for (const Media::LiveVideo::StreamType streamType : streamTypes)
	{
		...
		for (size_t n = 0; n < streamConfigurations.size(); ++n)
		{
			...
			std::string menuEntryText = String::toAString(streamConfiguration.width_) + "x" + String::toAString(streamConfiguration.height_);

			if (frameRate != 0.0)
			{
				menuEntryText + ", " + String::toAString(frameRate, 1u) + "fps";
			}
...
std::string QuestCameraApplication::translateStreamType(const Media::LiveVideo::StreamType streamType, const FrameType::PixelFormat pixelFormat, const Media::LiveVideo::CodecType codecType)
{
	switch (streamType)
	{

...
FBMessengerCodesWrapper::FBMessengerCodesWrapper() :
	pixelFormat_(FrameType::FORMAT_RGB24)
{
	// Nothing to do here
}

FBMessengerCodesWrapper::FBMessengerCodesWrapper(FBMessengerCodesWrapper&& fbMessengerCodesWrapper) noexcept
{
	*this = std::move(fbMessengerCodesWrapper);
}

FBMessengerCodesWrapper::FBMessengerCodesWrapper(const std::vector<std::wstring>& commandArguments) :
	pixelFormat_(FrameType::FORMAT_RGB24)
{
#if 0
	...
	{
		Log::info() << "usage: oceandemofbmessengercodes source_image_or_video dimensions pixelFormat [no]loop";
...
void FBMessengerCodesWrapper::release()
{
...
bool FBMessengerCodesWrapper::detectAndDecode(Frame& frame, double& time, std::vector<std::string>& messages, bool* lastFrameReached)
{
	if (lastFrameReached)
	{
	...
	if (fbMessengerCodesPerformance_.measurements() % 20u == 0u)
	{
		fbMessengerCodesPerformance_.reset();
	}

	bool codeDetected = false;

	HighPerformanceStatistic::ScopedStatistic scopedPerformance(fbMessengerCodesPerformance_);
	...
	time = fbMessengerCodesPerformance_.average();
...
FBMessengerCodesWrapper& FBMessengerCodesWrapper::operator=(FBMessengerCodesWrapper&& fbMessengerCodesWrapper) noexcept
{
	if (this != &fbMessengerCodesWrapper)
	{
		// Only one instance of this class may exist at the same time
		ocean_assert(frameMedium_.isNull());

		frameMedium_ = std::move(fbMessengerCodesWrapper.frameMedium_);
		pixelFormat_ = fbMessengerCodesWrapper.pixelFormat_;
		timestamp_ = fbMessengerCodesWrapper.timestamp_;
	}

...
#include "application/ocean/demo/geometry/cameracalibration/win/CameraCalibration.h"
...
{
	Messenger::get().setFileOutput("camera_calibration_output.txt");
	Messenger::get().setOutputType(Messenger::MessageOutput(Messenger::OUTPUT_DEBUG_WINDOW | Messenger::OUTPUT_FILE));
	...
	try
	{
		CameraCalibrationMainWindow mainWindow(hInstance, String::toWString(std::string("Camera calibration ,") + Build::buildString()), mediaFile);

...
#include "application/ocean/demo/tracking/similaritytracker/macos/AppDelegate.h"
...
- (void)applicationDidFinishLaunching:(NSNotification*)aNotification
{
	...
	applicationSimilarityTrackerWrapper = SimilarityTrackerWrapper(Platform::Apple::MacOS::Utilities::commandArguments());
...
- (void)timerTicked:(NSTimer*)timer
{
	...
	const Vector2 recentTouchPosition = applicationCustomView.recentMousePositionAndReset();

	Frame resultingFrame;
	if (applicationSimilarityTrackerWrapper.trackNewFrame(resultingFrame, resultingPerformance, recentTouchPosition) && resultingFrame.isValid())
	{

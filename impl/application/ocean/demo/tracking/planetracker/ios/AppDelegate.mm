...
@interface AppDelegate ()
{
	...
	/// A text label showing the performance of the tracker.
	UILabel* textLabel_;
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
	...
	[viewController_.view addSubview:textLabel_];
	...
	if (pixelImage_)
	{
		pixelImage_->setDevice_T_camera(planeTrackerWrapper_.frameMedium()->device_T_camera());
		pixelImage_->start();
...
- (void)timerTicked:(NSTimer*)timer
{
	...
	if (planeTrackerWrapper_.trackNewFrame(resultingFrame, resultingPerformance, recentTouchPosition) && resultingFrame.isValid())
	{
		...
		{
			textLabel_.text = StringApple::toNSString(String::toAString(resultingPerformance * 1000.0) + " ms");
		}
		else
		{
			textLabel_.text = StringApple::toNSString("Select a plane to track.");
		}

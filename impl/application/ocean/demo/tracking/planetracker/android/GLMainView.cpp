...
#include "application/ocean/demo/tracking/planetracker/android/GLMainView.h"
...
void GLMainView::initializePlaneTracker(const std::string& inputMedium, const std::string& resolution)
{
	...
	planeTrackerWrapper_ = PlaneTrackerWrapper(commandLines);
...
void GLMainView::threadRun()
{
	...
	while (shouldThreadStop() == false)
	{
		...
		if (planeTrackerWrapper_.trackNewFrame(resultingTrackerFrame, resultingTrackerPerformance, localRecentTouchPosition) && resultingTrackerFrame.isValid())
		{
...
jboolean Java_com_facebook_ocean_app_demo_tracking_planetracker_android_PlaneTrackerActivity_initializePlaneTracker(JNIEnv* env, jobject javaThis, jstring inputMedium, jstring resolution)
{
	...
	GLMainView::get<GLMainView>().initializePlaneTracker(inputMediumValue, resolutionValue);

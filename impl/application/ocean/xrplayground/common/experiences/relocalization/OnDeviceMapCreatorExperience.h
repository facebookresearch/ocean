...
#ifndef FACEBOOK_APPLICATION_OCEAN_XRPLAYGROUND_COMMON_EXPERIENCES_ON_DEVICE_MAP_CREATOR_H
#define FACEBOOK_APPLICATION_OCEAN_XRPLAYGROUND_COMMON_EXPERIENCES_ON_DEVICE_MAP_CREATOR_H
...
/**
 * This class implements an experience allowing to create a mpa on device.
 * @ingroup xrplayground
 */
class OnDeviceMapCreatorExperience : public XRPlaygroundExperience
{
	protected:
    ...
    public:
		 ...
		 */
    	~OnDeviceMapCreatorExperience() override;
		...
		/**
		 * Starts the map creation.
		 * @return True, if succeeded
		 */
		bool start();

		/**
		 * Stops the map creation.
		 * @return True, if succeeded
		 */
		bool stop();
		 ...
		 * @param filename The file to which the map will be written
		 * @param optimized True, to store the map optimized (with e.g., bundle adjustment); False, to store the map raw
		 * @return True, if succeeded
		 */
		bool save(const std::string& filename, const bool optimized);
	...
	protected:
		...
		/// The 6-DOF tracker doing the map creation.
		Devices::SceneTracker6DOFRef mapCreatorTracker6DOF_;

		/// The subscription for samples events from the world tracker.
...
#endif // FACEBOOK_APPLICATION_OCEAN_XRPLAYGROUND_COMMON_EXPERIENCES_ON_DEVICE_MAP_CREATOR_H
